...
1. add optional adapters for specific installed solvers only when available in the environment;
2. add certificate-size and verification-time plots from generated report JSON files;
3. add controlled-width benchmark batches with scripted sweeps, still without performance claims unless external solvers are actually run;
4. prepare an artifact manifest and reproducibility instructions for eventual repository deposit.
