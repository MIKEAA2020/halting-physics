...
1. add a sparsification certificate checker for the affine interval formula;
2. replace the trivial one-bag decomposition emitter with a small supplied-decomposition library and nontrivial path/tree examples;
3. add SAT/CP-SAT/MILP encodings only after the certificate-producing DP and FE-row layers remain stable.
