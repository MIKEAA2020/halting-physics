...
The next changes that are genuinely worth doing are now implementation-level rather than prose-level:

1. add a small tree-decomposition DP verifier to `/home/user/finite_ecd_prototype/`;
2. emit and verify `TREE-DECOMP`, `DP-REFUTATION`, and `DP-OPTIMUM` certificates on tiny instances;
3. add a minimal affine row generator producing `FE-ROW` certificates for exact arithmetic spring/truss examples;
4. add a sparsification certificate checker for the affine interval formula;
5. only after those gates, add SAT/CP-SAT/MILP encodings for baseline comparison.
