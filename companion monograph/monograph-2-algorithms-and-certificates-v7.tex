...
\]
The \texttt{DP-OPTIMUM} certificates of Section~\ref{sec:width-dp} certify
the minimum of \(\kappa\) over \(\Gamma(\mathcal P)\); the benchmark suite of
...
For an observation value \(z\in O\), the observation fibre over \(z\) is
...
the retained scopes \(J_i\) lie in the corresponding sparse relations.  The
extrema below are over the finite numeric domains, as in all instances of
this article.
...
report the minimum number of active actuators.
...
instances, carry certified influence scopes and widths but no constructible
instance: their relation tables exceed the enumeration cap and are marked
...
chain \(n{=}8\), same / split & 8 / 16 & 16 & 0 & \texttt{UNSAT} / \texttt{OPT} & 0.000 & 0.000\\
...
the artifact report contains all twenty cases and timings).  Every constructed
...
rungs; the artifact report contains the complete ladder data).  Retained
...
chain-pretension ablation, a dense-chain ladder of five radii, and lattice
...
harness agreement counter is now gated on verified cross-check evidence,
which makes the accounting identity structural.  An earlier version of
this article reported a ``corrected'' accounting of 32/4/66 on the
...
Two follow-up studies of this release consolidate the recorded comparative
...
rebuilt or pinned to the recorded versions: minisat built from the public
mirror at commit 16982a5 (gcc 14.2, \texttt{-O3}, the standard
compiler-compatibility patch), kissat 4.0.4 at commit 8af8e56 (gcc 14.2,
\texttt{-O3}), z3 5.1.0, CP-SAT from OR-Tools 9.15.6755, and HiGHS 1.15.1.
...
agreements --- and a field-by-field comparison that excludes
hardware-dependent timing and size fields finds every structural field
identical; the only differences are certificate file names, because the
replication's native runner re-emits byte-identical duplicate certificates
(SHA-256 equality verified on dense, ten-bar, and lattice spot checks).
...
observation variants, with the recorded phase-4 driver and the fixed
...
system OOM killer at about 3.75\,GiB resident --- and the case is recorded
...
dense \(n{=}18\), same & 524123 & 31.6 & \texttt{TMO} & 52.18 & \texttt{TMO} & \texttt{UNK} & \texttt{TMO}\\
...
re-verification; \texttt{TMO} marks runs beyond the fixed 60-second limit;
\texttt{UNK} marks CP-SAT runs that returned \texttt{UNKNOWN} within their
limit; \texttt{MEM} marks the bounded native run that raised
\texttt{MemoryError} at the 3\,GiB address-space cap; all three markers are
recorded as inconclusive.  Every status-bearing external answer is
...
The recorded observations are per-instance and carry no superiority claim.
minisat exceeds the limit on all four extension instances; CP-SAT returns
unknown within its limit on the three unsatisfiable or encoding-heaviest
variants, whose forbidden-tuple encodings comprise 137.7--388.1\,MB of JSON
so that model construction consumes a substantial share of its budget;
...
extension runs records seven verified satisfiable models, three verified
unsatisfiable answers, ten inconclusive runs, no rejections, and seven
...
artifacts above the repository's 100\,MB per-file limit --- the dense
\(n{=}17\) and \(n{=}18\) encodings and DP certificates --- are archived
by SHA-256 checksum and size in the manifest
\texttt{dense\_\allowbreak extension\_\allowbreak large\_\allowbreak artifacts.json} under \texttt{reports/}
and are exactly
regenerable from the archived instances and decompositions by the recorded
drivers; the small artifacts (instances, models, certified row batches,
decompositions, native run records, and the \(n{=}17\) split policy
certificate) are deposited with their checksums.  An archival DOI will be
...
\input{companion-artifact-appendix-v5.tex}
...
under the standard parameterizations~\cite{downey-fellows99}.  None of these
