...
instances; all limits, relation-table boundaries, and unresolved brackets are
...
study covers twenty rungs --- ten-bar and dense-chain ladders of five radii
each, the single-rung chain ablation, and lattice ladders of four, three,
and two radii at column counts one, two, and three --- with fourteen
...
dense \(n{=}16\), same & 16 & 130942 & 7.18 & \texttt{TMO} & 3.775 & 1.648 & \texttt{UNK} & \texttt{TMO}\\
...
exceeded the fixed 60-second limit; \texttt{UNK} marks the CP-SAT run that
returned \texttt{UNKNOWN} within its limit; both are recorded as
inconclusive.  Native
...
32 external unsatisfiable answers are cross-checked against verified
\texttt{DP-REFUTATION} certificates; three external runs exceed the limit
and one CP-SAT run returns \texttt{UNKNOWN} within its limit, so four runs
are recorded as inconclusive; nothing is rejected; and all 66 status-bearing
external answers agree with the native
certificates.\footnote{The recorded summary file of the artifact counts the
CP-SAT \texttt{UNKNOWN} run among the cross-checked unsatisfiable answers,
reporting 33 cross-checked unsatisfiable answers, 3 inconclusive runs, and 67
agreements.  The per-instance records and the status discipline of
Appendix~\ref{app:artifact} place a solver status without a certified claim
among the inconclusive runs, so the corrected accounting is reported here;
the harness counter is to be re-audited at the next artifact release.}
