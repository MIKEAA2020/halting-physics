...
value costs \(\kappa\) of the article's value-cost remark (Remark~\ref{rem:costs})
for optimum certificates. The complexity and certificate claims made
...
of the article's status discipline (Remark~\ref{rem:status-discipline}) ---
\texttt{VERIFIED}, \texttt{REJECTED}, \texttt{COMPLETE}, and
\texttt{RELATION-TABLE-LIMIT} --- which record the
...
for retained scopes $J\subseteq J'$: coarse inner cylinders (a cylinder is
the set of completions of one retained assignment) must refine into fine
...
The mechanics layer adds declared structural models in the sense of the
article's mechanics front end (Subsection~\ref{subsec:structural-front-end}),
in two kinds. A truss model lists nodes with rational coordinates and support
flags, members with declared rational axial stiffnesses, actuators (node-force
...
Instances derived from structural models come in the two observation variants
(\texttt{same} and \texttt{split}) of the article's benchmark convention
(Section~\ref{sec:mechanics-benchmarks}). Decompositions follow the
deterministic minimum-fill heuristic and tree-decomposition validation of the
same suite.
...
\end{center}
\noindent The protocol is the fixed external-baseline protocol of the article
(Subsection~\ref{subsec:external-baselines}). In the mechanics study
...
from the phase~4 report --- the recorded report of the mechanics-benchmark
generation phase --- rather than charged to either side. Solvers are located on the path or through the
...
\end{center}
with no solver available the emitted report is \texttt{INCONCLUSIVE}. External
answers are cross-checked under the conservative policy above. The build
...
The recorded comparative scaling study was replicated in full under the
version-exact solver provenance of the article's baseline configuration
(Subsection~\ref{subsec:external-baselines}); the replicated report is
archived as
...
dense-extension reports of Subsection~\ref{subsec:dense-extension} are recorded
