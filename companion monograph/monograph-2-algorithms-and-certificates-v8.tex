...
memory, glibc 2.41, kernel 5.10.134, and harness
Python 3.12.14 (the recorded protocol block of the artifact reports exactly
these fields); minisat (2.2 series, built from the public mirror at commit
...
system OOM killer with an anonymous resident set of 3\,750\,608\,kB, about
...
interpreter startup and model construction, so the \(n{=}18\) same CP-SAT
run is killed at the hard cap, while the \(n{=}18\) split CP-SAT run
returns \texttt{UNKNOWN} only after its soft solver limit is overshot (73\,s
of solve time).  Every status-bearing external answer is
...
The recorded observations are per-instance and carry no superiority claim.
minisat exceeds the limit on all four extension instances; CP-SAT ends
...
\input{companion-artifact-appendix-v7.tex}
