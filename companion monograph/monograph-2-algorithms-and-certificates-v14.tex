...
descent (ECD).  The foundational monograph (version~47) supplies the presentation-relative
...
W[2]-hard in the number of added sensors; the unrestricted formulation is
classified alongside --- degenerate under free sensor alphabets, and under a
fixed value alphabet an exact reduction to per-fibre transversal bounds with
the same hardnesses.  Sixth, we connect exact affine mechanical
...
Minimum-cardinality unrestricted synthesis (fixed value alphabet) & NP-complete\\
\bottomrule
\end{tabular}
\end{center}
...
and Appendix~\ref{app:reductions}).  The unrestricted formulation, with
sensors not drawn from a declared list, is degenerate under free value
alphabets and reduces exactly to per-fibre transversal bounds under a fixed
value alphabet, with the same NP-completeness and W[2]-hardness in the sensor
count (Proposition~\ref{prop:unrestricted-synthesis}).
...
(Proposition~\ref{prop:sensor-design-hardness}), and unrestricted synthesis
under a fixed value alphabet is W[2]-hard in the sensor count
(Proposition~\ref{prop:unrestricted-synthesis}), so the design-side barrier
is not an artifact of the candidate list.  Evaluation width and design
...
The unrestricted formulation --- sensors not drawn from a declared candidate
list --- admits a complete classification, which also identifies what the
candidate list contributes.

\begin{proposition}[Unrestricted sensor synthesis]
\label{prop:unrestricted-synthesis}
Fix one-shot data \(W,o,A,(S_w)\) with \(S_w\ne\varnothing\) for every
scenario \(w\).
\begin{enumerate}
\item Under unrestricted sensor value sets, minimum-cardinality synthesis is
degenerate: the empty family repairs when the one-shot problem already admits
a selector, and otherwise a single sensor --- one taking distinct values on
the scenarios of each observation fibre --- repairs, so the minimum is \(0\)
or \(1\).
\item For a fixed value alphabet of size \(a\ge2\), a family of \(m\)
sensors repairs if and only if, for every realised observation value \(z\),
the admissible family \((S_w)_{w\in o^{-1}(z)}\) admits a transversal --- a
set of actions meeting every \(S_w\) --- of cardinality at most \(a^m\).
The minimum sensor count is therefore
\(\max_z\lceil\log_a\tau_z\rceil\), where \(\tau_z\) is the transversal
number of the fibre family and the maximum is over realised observation
values.
\item Deciding whether \(m\) sensors with values in a fixed alphabet of size
\(a\ge2\) exist is NP-complete and W[2]-hard under the parameterisation by
\(m\), already with one observation fibre and \(a=2\).
\end{enumerate}
\end{proposition}

\begin{proof}
(1) The empty family is the refining family of the original problem, which
admits a selector exactly under the fibre criterion of
Theorem~\ref{thm:fibre-criterion}.  Otherwise some fibre fails.  A sensor
taking distinct values on the scenarios of each fibre is nonconstant on every
inclusion-minimal conflict core: every core lies in one fibre and has at
least two scenarios, since \(S_w\ne\varnothing\) rules out singleton cores.
By Theorem~\ref{thm:sensor-repair} the singleton family repairs, and no
nonempty family has fewer members.

(2) Let \(T\) be a repairing family \(s_1,\dots,s_m\) with values in the
fixed \(a\)-element set.  Within one observation fibre \(o^{-1}(z)\), the
nonempty refined fibres partition the scenarios into at most \(a^m\) classes,
each with nonempty admissible intersection; one action chosen from each
nonempty class yields a transversal of cardinality at most \(a^m\), since
every scenario's admissible set contains the action of its class.
Conversely, suppose each fibre family admits a transversal \(T_z\) with
\(|T_z|\le a^m\).  Fix an order on each \(T_z\), assign every
\(w\in o^{-1}(z)\) an element \(t(w)\in T_z\cap S_w\), and encode the rank
of \(t(w)\) in \(m\) digits to base \(a\); the \(j\)-th digits of the codes
define \(s_j\).  Scenarios of one fibre assigned the same element receive
identical codes, and scenarios assigned different elements receive different
ones, so each nonempty refined fibre consists exactly of the scenarios of one
fibre assigned to one element, whose admissible intersection contains that
element.  The displayed minimum follows by taking logarithms; the empty
family is the case \(a^0=1\), which holds exactly when every fibre family
has a one-element transversal --- the fibre criterion itself.

(3) Membership in NP is witnessed by the \(m\) sensor tables together with a
selector for the refined problem, checkable by a scan over the scenario list.
Hardness, both the NP-completeness and the W[2]-hardness in the sensor count,
is the transversal padding construction of
Appendix~\ref{app:reductions}, which reduces hitting set to the decision
problem with a single observation fibre and \(a=2\).
\end{proof}
...
sensor repair with its conditional approximation and ETH corollaries, and the
classification of unrestricted sensor synthesis
(Propositions~\ref{prop:sensor-design-hardness}
and~\ref{prop:unrestricted-synthesis}), are the design-side results this
article records --- problems rather than a layer.  Those statements require
...
are not drawn from a declared list, is classified by
Proposition~\ref{prop:unrestricted-synthesis} and
Subsection~\ref{subsec:unrestricted-synthesis}.

\subsection{Unrestricted sensor synthesis}
\label{subsec:unrestricted-synthesis}

Proposition~\ref{prop:unrestricted-synthesis} reduces unrestricted synthesis
under a fixed value alphabet to per-fibre transversal bounds.  For the
hardness claim in part~(3), reduce from hitting set.  Given a family
\(F_1,\dots,F_n\) of nonempty subsets of a universe \(A_0\) and a bound
\(k\ge2\), let \(t=\lceil\log_2 k\rceil\) and \(K_0=2^t-k\), so
\(0\le K_0<2^{t-1}\).  Build one-shot data with a single observation value,
action set \(A=A_0\cup\{p_1,\dots,p_{K_0}\}\) with fresh actions \(p_j\),
one scenario \(w_i\) per set with \(S_{w_i}=F_i\), and \(K_0\) further
scenarios \(u_j\) with \(S_{u_j}=\{p_j\}\).  Every transversal of the
constructed family contains every \(p_j\), and its \(A_0\)-part must meet
every \(F_i\), so the transversal number of the constructed family is
exactly \(\tau(F_1,\dots,F_n)+K_0\).  By
Proposition~\ref{prop:unrestricted-synthesis}(2), \(t\) sensors with values
in a two-element set repair if and only if this number is at most
\(2^t=k+K_0\), that is, if and only if the original family admits a
transversal of cardinality at most \(k\).  Hitting set is NP-complete and
W[2]-complete under the parameterisation by the number of chosen elements,
in each case by the standard duality with set
cover~\cite{karp72,downey-fellows99}; the construction is a parameterised
reduction with sensor count \(t\le\log_2 k+1\), so the decision problem is
NP-complete and W[2]-hard in the sensor count.  The single-channel
presentation of the constructed instance again has one unary factor per
scenario and an edgeless primal graph, so the hardness is present at width
zero on the unrestricted side as well.  For fixed \(m\) the characterisation
is algorithmic: enumerating the at most \(|A|^{a^m}\) candidate transversals
per fibre decides whether \(m\) sensors exist, without enumerating the
exponentially many sensor tables.

\subsection{Conditional statements for listed repair}
\label{subsec:conditional-repair}

Two standard statements transfer to listed repair through the
budget-preserving bijection of the set-cover construction in
Subsection~\ref{subsec:hard-rows}, under which repairing subfamilies of at
most \(k\) candidates correspond exactly to covers of at most \(k\)
covering sets.  First, minimum-cardinality listed repair, on instances whose
inclusion-minimal core list, of size \(N\), is part of the finite master
problem, is approximable within the harmonic bound \(H_N\le1+\ln N\) by the
greedy set-cover algorithm applied to the cores~\cite{johnson74}; and unless
\(\mathrm P=\mathrm{NP}\) no \((1-\eps)\ln N\)-approximation exists, since
such an algorithm would transfer back through the bijection to set cover on
the constructed instances~\cite{dinursteurer14}.  Second, dominating set
admits no \(f(k)\,n^{o(k)}\) algorithm under the exponential-time
hypothesis~\cite{impagliazzo-paturi01,cygan15}; the closed-neighbourhood
encoding maps a dominating-set instance with parameter \(k\) to a set-cover
instance with the same parameter and polynomial size, and the construction
in Subsection~\ref{subsec:hard-rows} preserves the budget, so an
\(|I|^{o(k)}\) algorithm for listed repair would compose with this chain to
violate the dominating-set barrier.  Under the exponential-time hypothesis,
therefore, no \(|I|^{o(k)}\) algorithm decides listed repair; the trivial
enumeration of subfamilies of size at most \(k\) runs in
\(|I|^{\mathrm O(k)}\), so the barrier is tight up to the constant in the
exponent.  Both statements are conditional and single-problem; no
approximation or ETH frontier is claimed for any other layer of the
explicit-table model.
...
\bibitem{cygan15} M.~Cygan, F.~V. Fomin, \L.~Kowalik, D.~Lokshtanov,
D.~Marx, M.~Pilipczuk, M.~Pilipczuk, and S.~Saurabh. \emph{Parameterized
Algorithms}. Springer, 2015.

\bibitem{dechter03} R.~Dechter. \emph{Constraint Processing}.
Morgan Kaufmann, 2003.

\bibitem{dinursteurer14} I.~Dinur and D.~Steurer. Analytical approach to
parallel repetition. In \emph{Proceedings of the 46th Annual ACM Symposium
on Theory of Computing (STOC)}, pages 501--510, 2014.
...
\bibitem{johnson74} D.~S. Johnson. Approximation algorithms for combinatorial
problems. \emph{Journal of Computer and System Sciences}, 9(3):256--278,
1974.
