...
controlled-width benchmark generation, baseline encoders, optional
external-solver adapters, external-result cross-checking, report summaries, and
SVG plot generation.  The repository-ready entry point is
\begin{verbatim}
make reproduce
\end{verbatim}
...
Missing external solver & \texttt{INCONCLUSIVE}\\
\bottomrule
\end{tabular}
\end{center}

Controlled-width benchmark families include paths, binary trees,
banded chains, and small grids with supplied decompositions.  The reports record
instance size, ordinary width, weighted width, certificate size, and verification
time.  These are certificate and reproducibility checks, not external-solver
performance claims.

The artifact containing the finite ECD prototype and reproduction scripts will
be deposited in a public repository before publication, and the release
manifest will be finalized upon deposit.
...
\input{companion-artifact-appendix-v2.tex}
