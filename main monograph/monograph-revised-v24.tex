...
realise exactly the finite local behaviours. Global information recall makes
pure action recall capability-redundant; a height-two countable arena transfers
the DNR/PA obstruction to causal form.
\end{abstract}
...
\section{Functorial and compositional programme}\label{sec:functorial-programme}

The preceding chapters give object-level presentations and preservation results.
A monograph treatment can also incorporate the companion morphism-level
programme. The aim is to identify the maps between presentations that preserve
capability, the stronger maps that preserve diagnostic labels, and the still
stronger maps that transport certificates.

\begin{definition}[Effective causal section presentation]
\label{def:effective-causal-section-presentation}
An effective causal section presentation consists of a context category
$\mathcal C$, an assignment functor $\mathcal A:\mathcal C\to\mathbf{Set}$,
tolerance-indexed local admissibility sets $\LocalAdm_\eps(c)$, effective and
bounded implementation classes, and realisation maps whose images define
\[
\Gamma_{\mathrm{set}}^\eps
\supseteq
\Gamma_{\mathrm{eff}}^\eps
...
\Gamma_{\mathcal B}^\eps.
\]
A structured version separates local validity from structural admissibility by
writing the valid global space as
$\Gamma(\mathcal C,\mathsf A\cap\mathsf V^\eps)$.
\end{definition}

\begin{definition}[Morphism levels]
\label{def:morphism-levels}
A capability morphism preserves the policy or strategy space and the worldwise
loss functional. A diagnostic morphism preserves the obstruction datum, including
the local-success bit and the nested global spaces. A certificate morphism
additionally preserves the objectwise local sets, restriction maps, and the
incidence of local elements with compatible global families.
\end{definition}

\begin{programme}[Monograph theorem targets]
\label{prog:monograph-targets}
The morphism-level theory has the following theorem targets.
\begin{enumerate}[label=(\roman*)]
\item The global-family assignment is a functor from constrained presentations
and admissibility-preserving natural transformations to nested triples.
\item Terminal compression is an idempotent functor, or reflector, onto terminal
presentations after the relevant presentation category is fixed.
\item Empirical-support packaging is functorial for support isomorphisms and for
specified non-invertible reindexing maps.
\item Reduced finite distributed selectors and blind-order one-shot arenas form
equivalent groupoids under structure-preserving isomorphisms.
\item Arena patch sheaves and activated-validity subsheaves are functorial under
arena isomorphisms preserving worlds, information sets, choices, nature maps, and
losses.
\item Constraint systems compose by conjunction, disjoint union, pullback over
shared interfaces, and hiding of internal variables; the corresponding solution
spaces compose by intersections, products, fibre products, and projections.
\item Parallel, sequential, and guarded-feedback composition preserve capability
and success spectra under explicit independence and resource-overhead hypotheses.
\item Broader source-domain placements, including protocol complexes,
decentralised control, Gödel-style diagonal reductions, and quantum process
restrictions, require declared source categories, translation maps, and success
predicates before diagnostic claims are made.
\end{enumerate}
\end{programme}

\begin{remark}
This programme is included in the monograph version because it organises the
companion work naturally. The article-level results above remain independent of
which non-invertible morphisms are eventually chosen.
\end{remark}
