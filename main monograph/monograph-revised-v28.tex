...
Gödel-style diagonals & representation target & theoremhood/truth convention, consistency or soundness assumptions, representability data\\
No-cloning/no-broadcasting & representation target & process theory, tensor product, states, channels, and physical success predicate\\
...
\begin{enumerate}[label=(\roman*)]
\item A finite tabular representation theorem identifying finite constrained
presentations with finite verifier games up to prediction-relevant behavioural
equivalence.
...
existence.
\item Protocol-complex representations of distributed-computing tasks, where
carried chromatic decision maps are global sections and protocol legality is an
...
\item Gödel-style diagonal placements formulated as reductions from candidate
deciders with explicit distinctions between theoremhood, truth, consistency,
soundness, and internal representability.
\item No-cloning and no-broadcasting placements in a declared process theory,
where physical channels form an implementation or physical-realisation layer
rather than a local fixed-point contradiction.
...
A no-cloning or no-broadcasting placement requires a symmetric monoidal process
theory with states, channels, tensor product, discarding, and a success equation.
The set-theoretic map $\rho\mapsto\rho\otimes\rho$ exists; the obstruction is to
realisation by allowed physical channels. A separate physical-realisation stratum
$\Gamma_{\mathrm{phys}}$ may therefore be appropriate between
$\Gamma_{\mathrm{set}}$ and $\Gamma_{\mathrm{eff}}$.

\paragraph{Logical diagonalisation.}
A Gödel-style chapter should distinguish theoremhood from truth, consistency
...
representability and correctness assumptions, construct a selector instance on
which it fails.
...
A full minimax tower requires a specified loss doctrine and attainment
conditions. Resource composition requires a chosen ordered monoid and
implementation-pairing, projection, or bind operations. Without those data, the
framework gives filtrations and success spectra, not additive physical
decompositions.
