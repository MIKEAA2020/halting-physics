...
compositional extension theory. Focused articles can be extracted from the
...
companion morphism theory: global-family functoriality, diagnostic and
...
Computability examples yield the binary DNR \(\Pi^0_1\) class, the PA oracle
spectrum, \(\mathsf{WKL}\) Weihrauch degrees, and a \(\Sigma^0_3\)-complete
...
integrations is $\#\mathrm P$-complete.
...
fails: in the binary-input, binary-output PR behaviour, outputs satisfy
...
theorem therefore does not apply. The reduced prefix category uses the same
restriction-arrow convention as the patch categories: longer prefixes restrict to
shorter prefixes, just as larger patches restrict to smaller patches. The exact
...
\begin{proof}
Let $s\in\Gamma_{\mathrm{set}}(P)$. Local admissibility is preserved by the
hypothesis on $\alpha$. For an arrow $f:c\to c'$, naturality gives
\[
\mathcal A^Q(f)(\alpha_c(s_c))
=\alpha_{c'}(\mathcal A^P(f)(s_c))
=\alpha_{c'}(s_{c'}),
\]
so $\Gamma(\alpha)s$ is compatible. The effective and bounded components are
preserved by the image hypotheses. Identities and composition are inherited from
natural transformations, giving a functor.
\end{proof}
...
For behaviorally annotated presentations, a capability morphism preserves the
...
worldwise loss functionals. A \emph{diagnostic isomorphism} is an isomorphism of
obstruction data, hence a bijection of the set-theoretic global spaces carrying
the effective and bounded subspaces onto their counterparts and preserving the
local-success bit. A \emph{certificate isomorphism} is a diagnostic isomorphism
induced by an isomorphism of the underlying local-incidence presentation,
...
The monograph-level structure can be summarized as a single spine: sections are
...
Let $T$ be a computably axiomatized theory whose theorem set
\[
...
For every computably axiomatized theory $T$ with undecidable theorem set, the
...
consistent effectively axiomatized extensions of enough arithmetic, this is the
...
algorithmic monograph (version~10, with its artifact appendix, version~7).
...
the present repository is the companion artifact's \texttt{make reproduce}
