...
\makeatletter
\newcommand{\LocalAdm}{\@ifnextchar{_}{\@LocalAdmSub}{\@LocalAdmPlain}}
\newcommand{\@LocalAdmPlain}{\mathcal{A}_{\mathrm{loc}}}
\def\@LocalAdmSub_#1{\mathcal{A}_{\mathrm{loc},#1}}
\makeatother
\newcommand{\eps}{\varepsilon}

\title{Effective Causal Descent:\\ Uniform Selection, Contextual Gluing, and Causal Histories\\[1mm]
...
\paragraph{Monograph scope.}
This monograph develops the section-theoretic core, the presentation-relative
diagnostic calculus, the physical Bell repair module, and the functorial and
compositional extension theory. The finite algorithmic realisation --- input
formats, typed certificate verifiers, width-based dynamic programs, the
mechanics front end, and the prototype artifact --- is delegated to the
companion algorithmic monograph. Focused articles can be extracted from the
selector/diagnostic core, the Bell--contextuality module, the computability
module, and the causal-arena module.

\begin{abstract}
We develop effective causal descent as a section-theoretic account of
decision-making under partial observation. The basic constraint is observational
uniformity: an interface cannot support different responses to worlds it does not
distinguish. From this arise four presentation-relative diagnostics: local
validity, compatibility of a declared diagram, effective realisability, and
bounded realisability. The labels are not invariants of bare capability
predicates; they record where success first fails in a chosen local-to-global
presentation. The static kernel is a fibrewise selector lemma. Compactness,
Helly, join-tree, parity, support-contextuality, source-integration, and
Bell-coupling results provide finite certificate and gluing tools. The
obstruction datum records objectwise local success and the nested spaces
...
while erasing precisely the \(\mathsf D\)-gap. The monograph develops the
accompanying morphism theory: global-family functoriality, diagnostic and
certificate morphisms, terminal compression as an idempotent functor,
support-packaging functoriality, distributed/one-shot groupoid equivalence,
arena patch-sheaf functoriality, and composition laws for constraints, static
compilations, sequential composition, parallel composition, and guarded feedback.
Computability examples yield the binary diagonally nonrecursive (DNR)
\(\Pi^0_1\) class, the Peano-arithmetic (PA) oracle spectrum, \(\mathsf{WKL}\)
(weak K\"onig's lemma) Weihrauch degrees, and a \(\Sigma^0_3\)-complete
primitive-recursive capability index set. The causal tier proves backward
induction, active-sensing presentation relativity, confluent causal-poset
execution, global-recall action-recall redundancy, and a height-two causal
DNR/PA transfer. The Bell module adds local-causal interpretation, Farkas
certificates, finite-sample testing, and repair profiles.
\end{abstract}

\noindent\textbf{Keywords:} effective causal descent; presentation-relative diagnostics; constraint satisfaction and sheaf semantics; Bell nonlocality and contextuality; computable selection and Weihrauch reducibility; extensive-form games and causal protocols.


\begin{table}[htbp]
\centering
\caption{Principal notation for the monograph.}
\label{tab:notation}
\begin{tabular}{@{}p{0.24\linewidth}p{0.68\linewidth}@{}}
\toprule
Symbol & Meaning\\
\midrule
$\mathcal C,\mathcal A$ & context category and assignment functor; arrows are restriction arrows\\
$\LocalAdm_\eps(c)$ & local admissibility set at context $c$ and tolerance $\eps$\\
$\Gamma_{\mathrm{set}}^\eps$ & compatible locally admissible global families\\
$\Gamma_{\mathrm{eff}}^\eps,\Gamma_{\mathcal B}^\eps$ & effective and bounded realised subspaces of $\Gamma_{\mathrm{set}}^\eps$\\
$\lambda_\eps$ & local-success bit: all local admissibility sets are nonempty\\
$\mathsf L,\mathsf D,\mathsf K,\mathsf R,\checkmark$ & local failure, compatibility failure, effectivity failure, resource failure, bounded success\\
$T_{\mathsf L},T_{\mathsf D},T_{\mathsf K},T_{\mathsf R}$ & tolerance-success spectra for the four filters\\
$\mathfrak O_\eps$ & obstruction datum: $\lambda_\eps$ plus the nested global spaces\\
$\operatorname{ExtComp}$ & terminal extensional compression\\
$G_{\mathsf D}$ & compatibility gap $T_{\mathsf L}\setminus T_{\mathsf D}$\\
$v_c(p;q)$ & Bell critical visibility of $p$ relative to local noise $q$\\
$\kappa_{\mathrm{comm}},\kappa_{\mathrm{meas}},\eta_c$ & communication, measurement-dependence, and detector-efficiency repair coordinates\\
\bottomrule
\end{tabular}
\end{table}
...
selector problem begins. Table~\ref{tab:notation} collects the principal
notation used throughout the monograph.
...
same underlying decision problem can be organised in more than one mathematical
...
The monograph proceeds in four stages. First, the minimal selector kernel isolates
...
observation contexts are organised as assignment spaces with restriction maps.
Objectwise validity gives a constrained diagram; when validity is preserved by
restriction to smaller subcontexts, it gives a descent system
(Definition~\ref{def:Av} below). Set-theoretic, effective,
...
Third, the same typed language is used to locate standard quantitative, finite,
computable, and causal obstructions. A quantitative interlude separates
pointwise, observation-uniform, effective, and bounded worst-case optima, with a
Chebyshev-radius formula in passive metric prediction. The finite gluing theory
then includes compactness and Helly bounds for local certificates, join-tree
gluing for acyclic context structures, a six-object parity obstruction,
support-level strong contextuality, finite source integration, Bell coupling,
and physical Bell repair. Constraint-datum isomorphisms preserve compatible
families; affine systems have a complete cokernel obstruction; bijective
constraint networks are governed by holonomy fixed points; Bell coupling
infeasibility has a Farkas dual certificate; finite-sample and repair variants
interpret this certificate operationally; and exact counting of global
integrations is $\#\mathrm P$-complete. Past factorisation, finite propagation,
and the light-cone obstruction complete the static-local tier.
...
$\mathsf K$ presentation of path selection. The diagonally nonrecursive (DNR)
instance has the Peano-arithmetic (PA) oracle
spectrum; its input-uniform path-selection operator, and uniform binary fibre
selection, have Weihrauch degree $\mathsf{WKL}$ --- path selection for nonempty
infinite prefix-closed binary trees, fixed with its representation in
Section~\ref{sec:effect}. Recursive capability is
...
Fourth, the morphism level makes the classification itself transportable.
Capability, diagnostic, and certificate morphisms separate what each level of
comparison preserves, and terminal compression is an idempotent functor that
erases precisely the $\mathsf D$-gap while preserving the nested global
spaces. Distributed and one-shot arenas agree up to groupoid equivalence, and
composition laws cover constraints, static compilations, sequential and
parallel composition, and guarded feedback. Finite tabular presentations are
represented as verifier games, in which certificates and refutations are
winning strategies and an unfinished play decides nothing. A dependency
ledger then governs source-domain placement under the topological-order
development principle --- dependency order is a constraint, and feasibility
orders the remaining work --- and the finite algorithmic theory is delegated
to the companion algorithmic monograph, which realises it in typed
certificates, width-based dynamic programs, and a prototype artifact.
...
satisfaction (CSP). Its multi-context form connects directly with relational database
...
On the distributed side, the information-structure tradition of
decentralised control studies synthesis and decision making when agents hold
different observations \cite{witsenhausen71,papadimitrioutsitsiklis86,
bernstein02}. The distributed-selector tier of Section~\ref{sec:distributed}
and the one-shot team arenas of Section~\ref{sec:feedback} treat the same
source problems presentation-relatively, as compatibility of a diagram over
...
selector formalism. For this reason the text cites, rather than reproves, several
external results: Farkas separation, Hoeffding concentration, König's lemma, the
Low and hyperimmune-free basis theorems, standard Weihrauch equivalences for
closed choice, deterministic time hierarchy theorems, and standard finite CSP
complexity classifications where invoked. The contribution is the common typed
comparison: local validity, compatibility, effective realisation, resource
bounds, and their preservation or failure under translation are represented in
one language.
...
Operationally, $\Adm_V(z)$ is the menu of choices that work for every world
...
objects on which the rest of the monograph builds.
\end{remark}
...
may have implementations with different costs. The qualifier \emph{represented}
is fixed in Definition~\ref{def:represented-realisation} below; the effective
and bounded classes are relative to that representation.
\end{definition}
...
Corollary~\ref{cor:worldpair} below, but the query cannot be one fixed function of the
...
The discussion broadens from one observation channel to many overlapping
...
declared diagram and are not invariants of a bare capability predicate. Table~\ref{tab:presentations} summarises the structural cases used in the monograph; several rows list constructions appearing in Sections~\ref{sec:distributed}, \ref{sec:Dtri}, and \ref{sec:feedback}. Repackaging all constraints into one object can move a failure
...
\begin{remark}[Success-set notation]
The notation $T_{\mathsf L},T_{\mathsf D},T_{\mathsf K},T_{\mathsf R}$ records
success of the successive filters, not the tolerances at which the corresponding
failure labels occur. For example, $\eps\in T_{\mathsf L}$ means all local sets
are nonempty; the failure outcome $\mathsf L_\eps$ occurs when
$\eps\notin T_{\mathsf L}$. For the terminal-category static compression,
$T_{\mathrm{fib}}=T_{\mathsf L}$ in ZFC; the equality uses choice when the
observation image is infinite. In the fibre-indexed discrete presentation,
$T_{\mathrm{fib}}$ is objectwise local success by definition.
\end{remark}
...
For each presentation language used in the monograph, a raw presentation is a code for the
...
$(0,0,0,0)$. Table~\ref{tab:compression-signature} tabulates this action on
the well-formed signatures.
\end{remark}

\begin{table}[htbp]
\centering
\caption{Fixed-tolerance action of terminal compression on diagnostic signatures.}
\label{tab:compression-signature}
\begin{tabular}{@{}c c c@{}}
\toprule
Original signature $(\lambda,s,e,b)$ & Original outcome & Compressed outcome\\
\midrule
$(0,0,0,0)$ & $\mathsf L$ & $\mathsf L$\\
$(1,0,0,0)$ & $\mathsf D$ & $\mathsf L$\\
$(1,1,0,0)$ & $\mathsf K$ & $\mathsf K$\\
$(1,1,1,0)$ & $\mathsf R$ & $\mathsf R$\\
...
Figure~\ref{fig:diagnostic-flow} displays the same cascade as a decision flow,
and the canonical presentations collected in Table~\ref{tab:presentations}
return throughout Sections~\ref{sec:distributed}--\ref{sec:feedback}.
\end{remark}
...
\[
\begin{array}{ccl}
\text{well-formed?} & \text{no} & \mathsf U\\
\downarrow\text{ yes} &&\\[-1mm]
\forall c\;\LocalAdm_\eps(c)\ne\varnothing? & \text{no} & \mathsf L_\eps\\
\downarrow\text{ yes} &&\\[-1mm]
\Gamma_{\mathrm{set}}^\eps\ne\varnothing? & \text{no} & \mathsf D_\eps\\
\downarrow\text{ yes} &&\\[-1mm]
\Gamma_{\mathrm{eff}}^\eps\ne\varnothing? & \text{no} & \mathsf K_\eps\\
\downarrow\text{ yes} &&\\[-1mm]
\Gamma_{\mathcal B}^\eps\ne\varnothing? & \text{no} & \mathsf R_\eps\\
\downarrow\text{ yes} && \checkmark_\eps
\end{array}
\]
\caption{Diagnostic decision flow at fixed tolerance. The labels identify the
first failed filter in the chosen presentation, not an invariant physical cause.}
\label{fig:diagnostic-flow}
\end{figure}
...
For hardness, reduce 3SAT, which is NP-complete \cite{karp}. Use one agent
per Boolean variable, a singleton
...
problem of Corollary~\ref{cor:team-decision} has nonempty local relations but no
global member, so its outcome is $\mathsf D$, not $\mathsf K$.
Section~\ref{sec:effect} supplies a distinct infinite represented
...
Identifying $x,y$ with Clauser--Horne--Shimony--Holt (CHSH) settings and
$f(x),g(y)$ with deterministic local
...
team example is therefore the support-level deterministic core of the CHSH
game: it has locally satisfiable input relations, and, as noted after
Corollary~\ref{cor:team-decision}, no single deterministic response table
satisfies all of them, the probability-valued coupling diagram of
Definition~\ref{def:bellpack} being their convex relaxation. The join-tree
theorem gives the acyclic, separator-consistent positive case for the scope
hypergraph.
...
\item $\eps\in T_{\mathsf R}$ gives outcome $\checkmark_\eps$, i.e. bounded capability.
\end{enumerate}
\end{corollary}

\begin{proof}
Apply the diagnostic partition of Proposition~\ref{prop:diagnostic} to the four
nested success conditions at tolerance $\eps$. Moving down the nested chain
$T_{\mathsf L}\supseteq T_{\mathsf D}\supseteq T_{\mathsf K}\supseteq
T_{\mathsf R}$, each case records the first failed success test; if none fails,
the outcome is $\checkmark_\eps$.
\end{proof}

\begin{proposition}[Robustness under class enlargement]\label{prop:robust}
Local emptiness and set-theoretic compatibility failure are invariant under
enlargement of the implementation class. Effectivity and resource outcomes are
relative to the corresponding realisation images.
\end{proposition}

\begin{proof}
The local sets $\LocalAdm_\eps(c)$ and the set-theoretic global space
$\Gamma_{\mathrm{set}}^\eps$ are defined before an implementation class is
imposed. Changing that class cannot populate an empty local set and cannot change
whether locally admissible families glue set-theoretically. By contrast,
...
\]
Use objects $D\in\downarrow M$, arrows $D\to D'$ for $D'\subseteq D$, and
\[
...
construction $e\mapsto\mathrm{Pack}(e)$ on noninvertible morphisms of empirical
models remains Conjecture~\ref{conj:context}.
\end{remark}
...
$(U_i,R_i)_{i=1}^m$ with $U_i\subseteq X$ and
\[
R_i\subseteq\prod_{x\in U_i}O_x.
\]
A global record is an element $t\in\prod_{x\in X}O_x$, and it integrates the
sources when $t|_{U_i}\in R_i$ for every $i$. The integration count is
\[
N_{\mathrm{int}}=\#\{t\in\prod_{x\in X}O_x:t|_{U_i}\in R_i\text{ for all }i\}.
\]
For each realised source scope $U\subseteq X$, let
\[
\bar R_U=\bigcap_{i:U_i=U}R_i
\]
be the aggregated relation for all sources with that scope. Then global
records integrate the original sources exactly when their restrictions lie in
$\bar R_U$ for every realised source scope $U$.

The aggregated source relations are the local tables of the canonical incidence
presentation. An integrating record is a global row whose projections lie in all
of those tables. The $\mathsf D$ outcome captures the situation in which every
aggregated local table is nonempty but the natural join is empty; if some
aggregated relation is empty, the obstruction is local in this presentation.
...
each source. The active scope at $w_i$ is $U_i$, and the world relation is
...
$(a_x)_{x\in U_i}\in R_i$. Theorem~\ref{thm:distributed-equivalence} identifies
...
For the canonical integration presentation at tolerance zero, if at least one
aggregated realised-source-scope relation $\bar R_U$ is empty, the outcome is
$\mathsf L_0$; nonempty aggregated relations with no integrating global record
give outcome
$\mathsf D_0$; and a global record gives set-theoretic capability. Effective and
bounded diagnostics are obtained by intersecting the global-record space with the
declared realisation images.
\end{corollary}

\begin{proof}
In the distributed presentation of Proposition~\ref{prop:source-integration-distributed},
all worlds with the same realised source scope have the same realised scope. The local
relation attached to that scope in Definition~\ref{def:distributed-csp} is
therefore the intersection of the corresponding source relations, namely
$\bar R_U$. The three cases are then exactly the local, compatibility, and
set-theoretic-success cases of the diagnostic partition.
\end{proof}
...
domains, source scopes of size at most three, and nonempty source relations.
\end{theorem}
...
For hardness, reduce parsimoniously from $\#3\mathrm{SAT}$, which is
$\#\mathrm P$-complete \cite{valiant}. Given a 3CNF formula,
use its variables as Boolean attributes. For each clause, make a source whose
scope is the set of variables appearing in the clause and whose relation is
...
\begin{proof}
Membership in $\#\mathrm P$ follows by nondeterministically guessing one value
for each active variable and checking all explicitly tabulated realised-scope
relations. Hardness follows from the source-integration subclass: by
Theorem~\ref{thm:distributed-equivalence}, valid distributed strategies, CSP
solutions, and compatible admissible families of the canonical incidence diagram
are cardinality-preservingly identified. Apply
Theorem~\ref{thm:source-integration-sharpP}.
\end{proof}

\begin{remark}[Counting and presentation]
The number of compatible families is invariant under isomorphism of the explicit
relational constraint datum, and under the canonical CSP--incidence bijection of
Theorem~\ref{thm:distributed-equivalence}. It is not an invariant of an
arbitrary repackaging: terminal compression and support saturation can preserve
existence while changing local incidence and certificate structure.
\end{remark}
...
specified by affine linear equations in those coordinates. Assume that a
compatible family is induced by a single global assignment $x\in\mathbb F^X$,
as holds for ordinary agreement on overlaps when the context diagram enforces all
shared variable identifications. Pad each local matrix with zero columns outside its context and stack the
...
unique path in $K$. Choose one orientation $e:u\to v$ for each non-tree edge. For such an oriented
non-tree edge, put
\[
...
\]
with the empty intersection understood as $C_{x_0}$. The result is independent
of the chosen orientations because a permutation and its inverse have the same
fixed-point set. Equivalently, it is the common fixed-point set of all
...
$\rho_i:C_i\to C_{i+1}$, with indices read modulo $n$ so that $C_n=C_0$, the
global solution set is in bijection with
\[
...
$p_{\mathbf x}\in\Delta(\prod_iO_i)$ for every setting tuple. Write
$p_{\mathbf x}(\mathbf o)$ equivalently as $p(\mathbf o\mid\mathbf x)$. Put
...
\]
This is a constrained assignment diagram, not generally a descent subfunctor: a
measure at $\top$ restricts into the singleton local set at every setting object
exactly when it lies in the coupling set below. Its compatible-family space is
determined by
\[
...
contributes normalised factors one, leaving
\[
...
\item there is $\gamma\in\mathbb R^Q$ such that
\[
A^{\mathsf T}\gamma\ge0,
\qquad
\langle\gamma,p\rangle<0.
\]
\end{enumerate}
...
rational, $\gamma$ may be chosen rational.
\end{theorem}
...
\quad\text{and}\quad
\exists\gamma\ (A^{\mathsf T}\gamma\ge0,
\ \langle\gamma,p\rangle<0)
\]
...
=\langle A^{\mathsf T}\gamma,\nu\rangle\ge0.
\]
Conversely, nonnegativity on every deterministic local table is precisely the
coordinate inequality $A^{\mathsf T}\gamma\ge0$. For rational $A$ and $p$, the
...
\subsection{Physical Bell obstruction, robustness, and repair}
\label{subsec:physical-bell}

The preceding coupling theorem answers a structural question: do the observed
setting-indexed probability tables have a common hidden-variable coupling? This
subsection gives that question its physical reading. The global object is a
hidden-variable space of complete response tables; the setting objects are the
experimentally observed marginals. A \(\mathsf D\) outcome in this presentation
means that the observed behaviour cannot be embedded in the local-causal model
without relaxing one of its assumptions. The same formal obstruction then has
three operational faces: a Bell inequality, a finite-sample statistical test, and
a family of possible physical repairs.

\begin{definition}[Local-causal Bell model]
\label{def:local-causal-bell}
For a finite Bell scenario, a local-causal model of a behaviour \(p\) consists of
a finite hidden-variable space \(\Lambda\), a probability distribution
\(\mu\in\Delta(\Lambda)\) independent of the setting tuple, and local response
kernels
\[
p_i(o_i\mid x_i,\lambda)
\qquad(i=1,\ldots,n)
\]
such that
\[
p(\mathbf o\mid\mathbf x)
=
\sum_{\lambda\in\Lambda}
\mu(\lambda)
\prod_i p_i(o_i\mid x_i,\lambda).
\]
The independence of \(\mu\) from \(\mathbf x\) is the measurement-independence, or
freedom-of-choice, assumption. For finite scenarios this stochastic formulation
is equivalent to a probability measure on deterministic response tables
\(\Omega=\prod_iO_i^{X_i}\), by the product-table construction in the proof of
Theorem~\ref{thm:bell}.
\end{definition}

\begin{proposition}[Local causality implies no-signalling]
\label{prop:local-nosignalling}
Every finite local-causal Bell behaviour is nonsignalling. That is, for every
party \(i\), setting \(x_i\), local outcome \(o_i\), and settings
\(\mathbf x_{-i},\mathbf x'_{-i}\) of the other parties,
\[
\sum_{\mathbf o_{-i}}p(o_i,
\mathbf o_{-i}\mid x_i,
\mathbf x_{-i})
=
\sum_{\mathbf o_{-i}}p(o_i,
...
\begin{proof}
Using the local-causal representation and summing over \(\mathbf o_{-i}\), all
factors belonging to parties other than \(i\) sum to one. The marginal becomes
\[
\sum_{\lambda\in\Lambda}
\mu(\lambda)
 p_i(o_i\mid x_i,\lambda),
\]
which is independent of \(\mathbf x_{-i}\). This proves the displayed equality.
\end{proof}

\begin{remark}[Scope of no-signalling]
The conclusion is the operational no-signalling condition. It follows from
measurement independence together with local response factorisation. The converse
fails: in the binary-input, binary-output Popescu--Rohrlich (PR) box
behaviour~\cite{popescu}, outputs satisfy
\(a\oplus b=xy\) with probability \(1/2\) on each satisfying pair. This behaviour
is nonsignalling but has no local coupling. Bell nonlocality is therefore a
stronger obstruction than signalling.
\end{remark}

\begin{definition}[Bell functionals, local bounds, and visibility]
\label{def:bell-functional-visibility}
A Bell functional is a real family
\(\beta=(\beta_{\mathbf x}(\mathbf o))_{\mathbf x,\mathbf o}\). Its unweighted value on a behaviour is
\[
\beta(p)=
\sum_{\mathbf x,
\mathbf o}
\beta_{\mathbf x}(\mathbf o)p_{\mathbf x}(\mathbf o),
\]
and its local bound is
\[
B(\beta)=\max_{\ell\in\mathcal L}\beta(\ell),
\]
where \(\mathcal L\) is the finite local polytope. Thus
\(\beta(q)\le B(\beta)\) is valid for every local behaviour \(q\). For a local
noise behaviour \(q\in\mathcal L\), define the critical visibility of \(p\) along
the segment from \(q\) to \(p\) by
\[
v_c(p;q)=\sup\{v\in[0,1]:vp+(1-v)q\in\mathcal L\}.
\]
\end{definition}

\begin{proposition}[Farkas certificates as Bell inequalities]
\label{prop:farkas-bell-functional}
A Farkas certificate in Theorem~\ref{thm:bell-dual} is equivalently a Bell
inequality separating \(p\) from the local polytope, up to sign and affine terms
vanishing on the normalisation constraints of the behaviour space.
\end{proposition}

\begin{proof}
The condition \(A^{\mathsf T}\gamma\ge0\) and \(\langle\gamma,p\rangle<0\) says
that the linear functional \(\gamma\) is nonnegative on every deterministic local
behaviour and negative on \(p\). Equivalently, \(\beta=-\gamma\) has value larger
on \(p\) than on every deterministic local behaviour, hence larger than on every
convex combination of deterministic behaviours. This is a Bell inequality with
local bound \(B(\beta)=\max_{\ell\in\mathcal L}\beta(\ell)\). Adding a functional
that is constant on the affine normalisation subspace changes neither the
validity of the inequality nor its violation by \(p\).
\end{proof}

\begin{theorem}[Visibility from Bell inequalities]
\label{thm:visibility-dual}
For \(q\in\mathcal L\),
\[
v_c(p;q)
=
...
1,
\inf_{\beta(p)>\beta(q)}
\frac{B(\beta)-\beta(q)}{\beta(p)-\beta(q)}
\right),
\]
where the infimum ranges over Bell functionals valid on \(\mathcal L\) and is
understood as \(+\infty\) if the indexing set is empty. If \(p\in\mathcal L\),
then \(v_c(p;q)=1\); if \(p\notin\mathcal L\), then \(v_c(p;q)<1\).
\end{theorem}

\begin{proof}
The feasible set
\[
\{v\in[0,1]:vp+(1-v)q\in\mathcal L\}
\]
is a closed interval containing \(0\), because \(\mathcal L\) is a compact convex
polytope and \(q\in\mathcal L\). Every valid Bell inequality gives
\[
\beta(vp+(1-v)q)
=v\beta(p)+(1-v)\beta(q)
\le B(\beta).
\]
When \(\beta(p)>\beta(q)\), this is exactly the displayed upper bound on \(v\).
Taking the infimum over such inequalities gives an upper bound for \(v_c\).

If \(p\in\mathcal L\), convexity gives feasibility for all \(v\in[0,1]\). If
\(p\notin\mathcal L\), closedness gives a boundary point
\(r=v_c p+(1-v_c)q\in\mathcal L\) with \(v_c<1\). Since the segment immediately
leaves the closed convex set at \(r\) in the direction \(p-q\), that direction is
not in the tangent cone of \(\mathcal L\) at \(r\). The separating-hyperplane
theorem in this pointed direction supplies a supporting Bell functional
\(\beta\) at \(r\) whose local bound is attained at \(r\) and whose value
increases in the direction from \(q\) to \(p\). In particular,
\(\beta(p)>\beta(q)\), so the infimum ranges over a nonempty class of functionals
whenever \(p\notin\mathcal L\). For this \(\beta\), the displayed ratio is
\(v_c\), so the upper bound is tight.
\end{proof}

\begin{example}[CHSH visibility]
\label{ex:chsh-visibility}
In the two-party, two-setting, binary-outcome CHSH scenario, write outcomes as
\(\pm1\) and put
\[
S(p)=E_{00}(p)+E_{01}(p)+E_{10}(p)-E_{11}(p),
\qquad
E_{xy}(p)=\sum_{a,b=\pm1}ab\,p(a,b\mid x,y).
\]
The local bound is \(S\le2\), and the uniformly random local behaviour \(q\) has
\(S(q)=0\). For the standard isotropic CHSH family, where this CHSH facet is the
first local facet met along the ray from \(q\) to \(p\),
\[
v_c(p;q)=\frac{2}{S(p)}
\qquad(S(p)>2).
\]
Thus the PR box, with \(S=4\), has threshold \(v_c=1/2\). A maximal quantum CHSH
correlation has \(S=2\sqrt2\), giving \(v_c=1/\sqrt2\). This equality is a
statement about the isotropic CHSH ray, not about arbitrary behaviours with
\(S(p)>2\).
\end{example}

\begin{theorem}[Finite-sample Bell certification]
\label{thm:finite-sample-bell}
Fix a setting distribution \(\nu\) and a Bell functional \(\beta\). Define the
weighted value and local bound by
\[
\beta_\nu(p)=
\sum_{\mathbf x,
\mathbf o}
\nu(\mathbf x)\beta_{\mathbf x}(\mathbf o)p(\mathbf o\mid\mathbf x),
\qquad
B_\nu(\beta)=
\max_{d\ \mathrm{deterministic}}
\sum_{\mathbf x}
\nu(\mathbf x)\beta_{\mathbf x}(d(\mathbf x)).
\]
Assume \(N\) independent trials, settings sampled independently from \(\nu\), and
single-trial local behaviour. If \(|\beta_{\mathbf x}(\mathbf o)|\le M\) for all \(\mathbf x,\mathbf o\) and
\[
\widehat\beta_N=\frac1N\sum_{k=1}^N
\beta_{\mathbf x_k}(\mathbf o_k),
\]
then every i.i.d. local model satisfies
\[
\Pr\!\left[
\widehat\beta_N\ge B_\nu(\beta)+\delta
\right]
\le
\exp\!\left(-\frac{N\delta^2}{2M^2}\right).
\]
Consequently, the event
\(\widehat\beta_N\ge B_\nu(\beta)+M\sqrt{2\log(1/\alpha)/N}\) has probability at
most \(\alpha\) under every such local model.
\end{theorem}

\begin{proof}
Under the stated sampling convention, the expectation of the single-trial
summand is at most \(B_\nu(\beta)\) for every local model. The summands are
independent and lie in an interval of length at most \(2M\). Hoeffding's
inequality \cite{hoeffding} gives
\[
\Pr\!\left[
\widehat\beta_N-\mathbb E\widehat\beta_N\ge\delta
\right]
\le
\exp\!\left(-\frac{2N\delta^2}{(2M)^2}\right),
\]
and the bound follows from \(\mathbb E\widehat\beta_N\le B_\nu(\beta)\). Solving
for \(\delta\) gives the stated level-\(\alpha\) threshold.
\end{proof}

\begin{remark}[Physical scope of finite-sample certification]
The finite-sample statement assumes independent trials, exogenous setting choice,
and a fixed local model for the trial distribution. Memory, locality,
freedom-of-choice, and detection loopholes require additional presentation
assumptions or martingale-style analyses. Under the stated assumptions, a Farkas
certificate is an experimentally testable Bell inequality with a finite-sample
violation bound.
\end{remark}

\begin{definition}[Bell repair profile]
\label{def:bell-repair-profile}
A repair of a nonlocal Bell behaviour relaxes a specified assumption of the
local-causal presentation. The basic repair coordinates used here are:
communication, where parties exchange messages; measurement dependence, where
\(\lambda\) may depend on \(\mathbf x\); and detector inefficiency, where local
models are allowed no-detection outcomes and the observed behaviour is obtained
by postselection. For fixed quantitative conventions, the corresponding repair
profile records optimal costs such as
\[
(v_c(p;q),\ \kappa_{\mathrm{comm}}(p),\ \kappa_{\mathrm{meas}}(p),\ \eta_c(p)).
\]
Here \(v_c\) is the visibility of Definition~\ref{def:bell-functional-visibility},
\(\kappa_{\mathrm{comm}}\) is the minimum communication cost in a declared
communication model, \(\kappa_{\mathrm{meas}}\) is the minimum dependence cost for
a chosen distance from measurement independence, and \(\eta_c\) is the maximum
detector efficiency still compatible with a local postselected explanation. The
coordinates are model-relative: changing the repair model changes the numerical
profile. Table~\ref{tab:bell-repair} collects the obstruction, certificate, and
repair coordinates for the components used here.
\end{definition}

\begin{table}[htbp]
\centering
\caption{Bell obstruction--certificate--repair coordinates.}
\label{tab:bell-repair}
\begin{tabular}{@{}p{0.18\linewidth}p{0.25\linewidth}p{0.22\linewidth}p{0.24\linewidth}@{}}
\toprule
Component & Fixed local-causal assumption & Relaxation or quantity & Certificate / optimisation\\
\midrule
Local coupling & measurement independence and local response factorisation &
none; require $\operatorname{Coupl}(p)\ne\varnothing$ & Farkas/Bell inequality\\
Noise robustness & fixed local noise $q\in\mathcal L$ & critical visibility
$v_c(p;q)$ & supporting Bell functional\\
Communication repair & no inter-party messages & one-way alphabet cost
$\kappa_{\mathrm{comm}}$ & message lower/upper bounds\\
Measurement-dependence repair & $\lambda$ independent of settings & dependence
cost $\kappa_{\mathrm{meas}}$ & finite dependence optimisation after a distance is fixed\\
Detection repair & observed outcomes without postselection & no-detection
outcome $\bot$ and threshold $\eta_c$ & efficiency Bell inequality\\
Contextual fraction & noncontextual convex decomposition &
$\operatorname{CF}=1-\operatorname{NCF}$ & linear programme and dual noncontextuality inequality\\
\bottomrule
\end{tabular}
\end{table}

\begin{proposition}[One-way communication repair]
\label{prop:communication-repair}
For a finite two-party behaviour \(p(a,b\mid x,y)\) whose first-party marginal is
independent of \(y\), define \(\kappa_{\mathrm{comm}}(p)\) as the minimum value
of \(\log |M|\) among exact worst-case shared-randomness-assisted one-way
simulations from the first party to the second, with
\(\kappa_{\mathrm{comm}}(p)=+\infty\) if none exists. The
following hold:
\begin{enumerate}[label=(\roman*)]
\item \(p\) has a zero-bit communication repair, equivalently a repair with
\(|M|=1\), if and only if \(p\) is local;
\item if \(p\) is nonlocal, every such repair has message alphabet size at least
\(2\);
\item every such behaviour has a repair with \(|M|\le |X_1|\,|O_1|\).
\end{enumerate}
\end{proposition}

\begin{proof}
A zero-bit communication repair, with shared randomness and \(|M|=1\), is
exactly a local hidden-variable model, giving (i), and (ii) follows immediately. For (iii), sample \(a\) from the marginal
\(p(a\mid x)\), which is well defined by the no-signalling condition from the
second party to the first. Send \((x,a)\) to the second party. If
\(p(a\mid x)>0\), the second party samples \(b\) from \(p(b\mid x,y,a)\); for
zero-probability pairs \((x,a)\), choose an arbitrary conditional distribution.
Local randomness used in these sampling steps can be absorbed into an enlarged
hidden variable. The resulting joint distribution is \(p(a,b\mid x,y)\), and the
message alphabet has size at most \(|X_1||O_1|\).
\end{proof}

\begin{proposition}[Measurement-dependent repair]
\label{prop:measurement-dependence-repair}
Every finite Bell behaviour has a local repair if measurement independence is
removed.
\end{proposition}
...
\[
p(\lambda=(\mathbf x,
\mathbf o)\mid\mathbf x)=p(\mathbf o\mid\mathbf x).
\]
The deterministic output rule returns the stored component \(o_i\) at party
\(i\). This reproduces \(p\), but the hidden variable is maximally correlated
with the settings.
\end{proof}

\begin{remark}
The measurement-dependent repair proves sufficiency of unrestricted setting
dependence, not a minimal dependence cost. Such costs can be introduced by
optimising a distance between \(p(\lambda\mid\mathbf x)\) and a
setting-independent hidden-variable distribution.
\end{remark}

\begin{definition}[Detector-efficiency repair]
\label{def:detection-repair}
Extend each outcome set to \(O_i^\bot=O_i\cup\{\bot\}\), where \(\bot\) denotes
no detection. For fixed \(\eta\), an efficiency-\(\eta\) local extension of a
behaviour \(p\) is a local behaviour \(\widetilde p\) on the extended outcomes
such that, for every setting tuple, the probability that all parties detect is
at least \(\eta\), and conditioning on the all-detected event yields
\(p(\cdot\mid\mathbf x)\). This is an all-detection efficiency convention;
party-specific efficiency conventions give related but different linear
programmes. Define
\[
\eta_c(p)=\sup\{\eta\in[0,1]:p\text{ has an efficiency-}\eta
\text{ local extension}\}.
\]
\end{definition}

\begin{proposition}[Detector-efficiency feasibility and threshold]
\label{prop:detector-certificate}
For every finite Bell behaviour \(p\) and fixed \(\eta\), existence of an
efficiency-\(\eta\) local extension is a finite linear feasibility problem.
Moreover, \(\eta_c(p)\) is the optimum of a finite linear programme. Infeasibility
at a specified efficiency has a Farkas dual certificate, an efficiency Bell
inequality.
\end{proposition}

\begin{proof}
Write the extended local behaviour as a convex combination of deterministic
response tables with outcomes in \(O_i^\bot\). Detection probabilities are linear
marginals of this extended behaviour. For each setting tuple \(\mathbf x\),
introduce a variable \(d_{\mathbf x}\) for the probability that all parties
detect, and impose \(d_{\mathbf x}\ge\eta\). The postselection equations are
\[
\widetilde p(\mathbf o\mid\mathbf x)
=d_{\mathbf x}p(\mathbf o\mid\mathbf x)
\qquad(\mathbf o\in\prod_iO_i),
\]
where the left side denotes the extended probability of the all-detected outcome
\(\mathbf o\), with no \(\bot\) coordinate. Since \(p\) is fixed, these equations
are linear in the unknown convex weights and the variables \(d_{\mathbf x}\).
Together with normalisation and the lower bounds \(d_{\mathbf x}\ge\eta\), they
form a finite linear feasibility problem. Maximising \(\eta\) subject to the same
linear constraints gives the linear programme for \(\eta_c(p)\). Farkas' lemma
gives a dual certificate when the system is infeasible at a specified efficiency.
\end{proof}

\begin{remark}[Contextuality counterpart]
Finite contextuality admits the same obstruction--robustness pattern after the
relevant support or probabilistic presentation is fixed. Strong contextuality is
the support-level case: all local supports are nonempty, but no global assignment
lies in all of them. For a finite probabilistic empirical model \(e\), one common
quantitative convention is the noncontextual fraction
\[
\operatorname{NCF}(e)=
\max\{\lambda\in[0,1]: e=\lambda e_{\mathrm{nc}}+(1-\lambda)e'
\text{ for some noncontextual }e_{\mathrm{nc}}\text{ and empirical model }e'\},
\]
with contextual fraction \(\operatorname{CF}(e)=1-\operatorname{NCF}(e)\)
\cite{contextualfraction}. In finite scenarios this is a linear programme, and
dual solutions give noncontextuality inequalities. These are
presentation-relative analogues of the Bell coupling and repair problems above.
\end{remark}
...
policy class, and recall that the bounded realised policy class is \emph{defined}
as the image
\[
\im\!\left(
...
\right).
\]
When this defining image equals $\Pi_{\mathrm{ext}}$, the identification
$\Pi_{\mathcal B}=\Pi_{\mathrm{ext}}$ is definitional rather than an additional
conclusion, and every selector statement for the internal presentation reduces
to the corresponding external-policy problem.
\end{proposition}

\begin{proof}
Only the reduction requires argument: all selector statements depend on
implementations only through the defining image of $\Pi_{\mathcal B}$.
\end{proof}
...
\begin{remark}[Variance convention for prefix diagrams]
In the reduced prefix diagram, arrows go from longer prefixes to shorter
prefixes, $D_n\to D_m$ for $m\le n$, because they are restriction maps. Later
causal patch categories use the same restriction direction, from larger patches
to smaller patches; the difference is only that a full finite tree can be a
largest patch and hence an initial object in the restriction category, while the
chain of finite prefixes has no largest finite prefix. Thus the nested-prefix
``no initial object'' statement is a consequence of unbounded length, not a
change in variance.
\end{remark}
...
\begin{proof}
Since $T$ is infinite, finitely branching, and prefix closed, König's lemma
\cite{konig} gives a path through $T$; in particular, $T$ has nodes of every finite length, and
...
theorem therefore does not apply. The exact
Weihrauch statement concerns the input-uniform operator $T\mapsto[T]$, not any
fixed DNR instance.
\end{remark}

\subsection{A master represented section-selection problem}
\label{subsec:master-weihrauch-section}

The preceding Weihrauch results are instances of one represented selection
problem. The input is not a fixed computable presentation, but a negative
description of the finite obstructions to a total section.

\begin{definition}[Binary forbidden-pattern section selection]
\label{def:forbidden-section-selection}
Let $\mathrm{ForbSec}_2$ be the multivalued problem whose input is an enumeration
of finite binary strings $\sigma\in2^{<\N}$, interpreted as forbidden cylinders
$[\sigma]\subseteq2^\N$, together with the promise that
\[
F=2^\N\setminus\bigcup_{\sigma\ \mathrm{enumerated}}[\sigma]
\]
is nonempty. An output is any $x\in F$.
\end{definition}

\begin{theorem}[Forbidden-pattern selection has Weihrauch degree $\mathsf{WKL}$]
\label{thm:forbidden-section-WKL}
\[
\mathrm{ForbSec}_2\equiv_{\mathrm W}\mathsf{WKL}.
\]
\end{theorem}

\begin{proof}
The enumeration of forbidden cylinders is exactly a negative name for the closed
set $F\subseteq2^\N$. Hence $\mathrm{ForbSec}_2$ is closed choice on Cantor space,
which is Weihrauch-equivalent to $\mathsf{WKL}$ by
Theorem~\ref{thm:fibsel-weihrauch}.

Conversely, an infinite binary tree $T$ determines the closed set $[T]$ by
enumerating every finite string not in $T$ as a forbidden cylinder. A point of the
resulting closed set is precisely an infinite path through $T$. These
translations are computable in the standard negative representations.
\end{proof}

\begin{corollary}[Uniform section-selection templates]
\label{cor:section-selection-templates}
The following represented problems are Weihrauch-reducible to
$\mathrm{ForbSec}_2$, and hence to $\mathsf{WKL}$:
\begin{enumerate}[label=(\roman*)]
\item selection from any nonempty effectively closed subset of $2^\N$;
\item countable binary section selection with uniformly c.e. forbidden finite
patterns;
\item represented patch- or arena-section selection whenever invalidity is
enumerated by finite binary witnesses and nonemptiness is promised.
\end{enumerate}
Each template is Weihrauch-equivalent to $\mathsf{WKL}$ whenever arbitrary
infinite binary trees, equivalently arbitrary nonempty effectively closed subsets
of Cantor space, can be encoded into the corresponding presentation class.
\end{corollary}

\begin{proof}
Each input supplies an enumeration of basic cylinders covering exactly the
invalid total assignments, so it computes an instance of
$\mathrm{ForbSec}_2$. A selected point is a valid section. The hardness statement
is the reverse construction in the proof of Theorem~\ref{thm:forbidden-section-WKL}
whenever the presentation class can realise arbitrary tree-prefix constraints.
\end{proof}

\begin{remark}[Scope of the WKL classification]
The theorem separates three cases. Finite explicit tabular presentations are
solved by finite search. A fixed computable infinite presentation may have a
particular degree spectrum, such as the PA degrees in the DNR example, without
being an input-uniform Weihrauch-complete problem. A represented family whose
invalid total sections are given by c.e. finite patterns has uniform selection
bounded by $\mathsf{WKL}$, and it is exactly $\mathsf{WKL}$ when it can encode
arbitrary binary trees. Thus the represented-space content lies in the input
representation and in the class of finite-pattern constraints allowed.
\end{remark}

\begin{theorem}[$\Pi^0_1$ realisation and oracle spectrum]
\label{thm:Pi01-realisation-spectrum}
For every nonempty $\Pi^0_1$ class $P\subseteq2^\N$, there is a computable
nested-prefix ECD presentation $E_P$ whose set-theoretic global strategy space is
computably homeomorphic to $P$. Consequently, for every oracle $X$,
\[
E_P\text{ has an }X\text{-computable global strategy}
\iff
X\text{ computes a member of }P.
\]
Thus the oracle degrees capable for $E_P$ are exactly the Muchnik upper cone of
$P$.
\end{theorem}

\begin{proof}
Write $P=[T]$ for a computable infinite prefix-closed binary tree $T$. Construct
the nested-prefix presentation $P_T$ of Definition~\ref{def:nested-prefix}. By
Theorem~\ref{thm:nested-distributed-K}, its global strategies are exactly the
paths through $T$. The correspondence sends a global strategy to the binary
sequence obtained by taking the union of its coherent finite prefixes, and sends
a path to its finite-prefix family. Both maps are computable and inverse to one
another. Relativising the same maps to an oracle $X$ gives the stated oracle
spectrum.
\end{proof}

\begin{theorem}[Reverse-mathematical form of nested-prefix selection]
\label{thm:reverse-math-nested-prefix}
Over $\mathsf{RCA}_0$, the following are equivalent:
\begin{enumerate}[label=(\roman*)]
\item $\mathsf{WKL}_0$;
\item every binary nested-prefix presentation $P_T$ arising from a prefix-closed
tree $T$ with nonempty local set $T\cap2^{n+1}$ at every level $n$ has a global
section;
\item every countable binary forbidden-pattern presentation whose every finite
stage is satisfiable has a total section.
\end{enumerate}
\end{theorem}

\begin{proof}
Weak König's lemma \cite{konig,simpson} gives a path through the tree of
coherent finite choices,
proving (ii) and (iii). Conversely, given an infinite binary tree $T$, form the
nested-prefix presentation with local set $T\cap2^{n+1}$ at level $n$. A global
section is exactly a path through $T$, so (ii) implies $\mathsf{WKL}_0$. A tree
can also be represented by forbidding finite strings not in $T$; then (iii)
again gives a path, proving $\mathsf{WKL}_0$. The codings are primitive
recursive and hence available over $\mathsf{RCA}_0$.
\end{proof}

\begin{table}[htbp]
\centering
\caption{Representative Weihrauch degrees of ECD selection variants.}
\label{tab:weihrauch-classification}
\begin{tabular}{@{}p{0.36\linewidth}p{0.28\linewidth}p{0.24\linewidth}@{}}
\toprule
Selection variant & Representation & Weihrauch degree\\
\midrule
finite explicit tabular section selection & finite tables & computable\\
finite alphabet, c.e. forbidden finite patterns, nonempty promise & negative closed-set name in a computably compact product & $\mathsf{WKL}$ when arbitrary binary trees encode\\
binary fibre selection & negative information for each nonempty fibre & $\mathsf{WKL}$\\
nested-prefix path selection & represented infinite binary tree & $\mathsf{WKL}$\\
fixed computable DNR instance & no varying input; oracle spectrum question & PA degrees, not a Weihrauch degree computation\\
countable discrete alphabets & closed subsets of Baire space & closed choice on $\N^\N$ under the standard representation\\
unique valid section & singleton closed sets & unique closed choice for the underlying represented space\\
\bottomrule
\end{tabular}
\end{table}

\begin{remark}[Degree-theoretic contribution]
Table~\ref{tab:weihrauch-classification} separates input-uniform Weihrauch
classification from nonuniform oracle spectra. The realisation theorem above is the nonuniform counterpart of the WKL
classification: every nonempty $\Pi^0_1$ path class, not only the DNR example,
can occur as the global strategy space of a computable ECD presentation. The
represented forbidden-pattern theorem gives the corresponding uniform operator.
\end{remark}
...
\end{cases}
\]
For $r>0$, the condition $\neg E(k,n,r)$ is evaluated at the proposed report
$r$, not at the world stage $t$; this makes a positive report valid uniformly
throughout the fibre exactly when it is the first-entry stage. Report $0$ is
valid throughout the fibre over $n$ exactly when
...
For the full causal patch category of Section~\ref{sec:feedback}, arrows are
restriction arrows from larger patches to smaller patches; with this convention
the full tree is initial and activated validity is a subfunctor. Hence
\[
...
\subsection{Imperfect-information arenas and behavioural coordinates}
\label{subsec:ii-arenas}
...
On the category of nonempty information-saturated prefix-closed patches, arrows
are restriction arrows, and the full tree is initial, so
\[
...
\subsection{Information refinement and recall}
...
\begin{proposition}[Strategic-mixed preservation]
\label{prop:action-recall-mixed-strategic}
Under the hypotheses of Theorem~\ref{thm:action-recall-equivalence}, every
finite-support mixed strategy on the action-recall refinement $\mathcal T^+$ has
a finite-support mixed strategy on $\mathcal T$ inducing the same distribution
on terminal nodes for every world. Consequently any terminal expected-loss or
expected-utility functional, once nature probabilities are supplied, has the same
value under the two mixed strategies.
\end{proposition}

\begin{proof}
The proof of Theorem~\ref{thm:action-recall-equivalence} assigns to each pure
strategy $\sigma$ on $\mathcal T^+$ a pure strategy $\bar\sigma$ on
$\mathcal T$ with the same reached terminal node at every world. Push a
finite-support probability measure on pure strategies of $\mathcal T^+$ forward
along $\sigma\mapsto\bar\sigma$. Conditional on each world, the terminal-node
distribution is unchanged. Taking expectations against any specified nature law
therefore preserves every payoff or loss depending only on the world and the
terminal node.
\end{proof}

\begin{remark}[Boundary with perfect recall and Kuhn equivalence]
\label{rem:action-recall-kuhn-boundary}
Theorem~\ref{thm:action-recall-equivalence} is a capability statement for pure
strategies, and Proposition~\ref{prop:action-recall-mixed-strategic} extends it
only to mixed strategies understood as ex ante randomisation over complete pure
plans. This is not Kuhn's theorem. Kuhn's mixed--behavioural realisation
equivalence is a player-relative theorem for finite extensive games with chance
probabilities, player ownership, utilities, and standard perfect recall; see,
for example, \cite{kuhn,osbornerubinstein}. The present arena formalism has no
player ownership relation at this point, and its global predictor-level
information recall is stronger in one direction and different in another: it can
hold even when a standard player would have forgotten her own earlier action.

Accordingly, action recall is not redundant for independent behavioural
randomisation at information sets. A minimal warning example has one decision
maker, one initial binary move $b\in\{0,1\}$, and a later information set $J$
that merges the two histories following $b=0$ and $b=1$. The two nodes in $J$
have the same predecessor information-set sequence, so global information recall
holds, but action recall fails. Against an adversarially chosen target
$t\in\{0,1\}$, give success exactly when the terminal pair is
$(b,r)=(t,t)$. In the action-recall refinement a behavioural strategy can choose
$b$ uniformly and then copy the recalled value of $b$, obtaining success
probability $1/2$ against each target. In the original arena, a behavioural
strategy chooses $b=1$ with probability $p$ and the later report $r=1$ with an
independent probability $q$ at $J$, so the two target success probabilities are
$pq$ and $(1-p)(1-q)$; their maximum possible minimum is $1/4$. By contrast,
an ex ante mixed strategy on the original arena can randomise equally between
the pure plans $(b,r)=(0,0)$ and $(1,1)$ and again obtains $1/2$ against each
target. Thus the redundancy theorem belongs to pure and strategic-mixed
capability theory, not to behavioural-strategy equivalence.

A game-theoretic development of this observation would require a separate
section or article with standard extensive-form data: players, chance laws,
information partitions by player, mixed and behavioural strategies, expected
utilities, zero-reach conventions, and equilibrium or max--min solution
concepts. Its natural comparison point would be Kuhn's theorem and the theory of
imperfect recall, stochastic games, or team decision problems. In the present
monograph the result is best kept as a scoped structural lemma explaining why
pure action labels need not be added to the interface when global information
histories already determine all pure reachable action histories.
\end{remark}

\subsection{One-shot team arenas and blind-order normal forms}
...
independently of $c_{<k}$. Intermediate losses vanish; here terminal loss means the loss evaluated at the
final predictor stage leading into terminal leaves. This final loss is zero
exactly when
\[
...
\]
\item this bijection preserves every worldwise zero--one final-stage loss and hence
...
\subsection{Distributed compilation of finite arenas}
...
\subsection{One-shot Bell realisation}
...
\subsection{Explicit-table complexity boundary}

\begin{proposition}[Explicit-table complexity boundary]\label{prop:pnp}
On finite perfect-information arenas, backward induction performs
$O(\sum_{h\in\mathcal T_P}|W_h|\,|C_h|)$ predictor loss-table checks and runs in
time linear in the explicit game description, including nature routing tables.
Capability for explicitly represented finite imperfect-information arenas is
NP-complete.
\end{proposition}

\begin{proof}
Membership in NP follows by guessing one choice per information set and checking
all explicitly represented world--node loss constraints. For hardness, reduce the NP-complete variant \cite{tovey} of 3SAT in which
each clause has three distinct variables. Nature selects a clause. The arena then queries, in a
...
\]
Thus the loss profile of every source strategy must factor through $j$ after
applying $K$; if $j(w_0)=j(w_1)$, then any strategy in the domain of the
compilation has equal transported losses at $w_0$ and $w_1$. This factorisation
is part of the well-formedness of the compilation data.
\end{definition}
...
Static compilations compose when the declared policy class of a composite is the
image of its composite strategy transformation. If
\((j_1,K_1):A\to B\) and \((j_2,K_2):B\to C\) preserve the corresponding
worldwise aggregate losses, then
\[
(j_2\circ j_1,\ K_2\circ K_1):A\to C
\]
is a static compilation with declared compiled policy class
\(\operatorname{im}(K_2\circ K_1)\). Identity maps give identity compilations, so
static compilations form a category with this image-as-declared-class
convention.
\end{proposition}

\begin{proof}
The composite of surjections is a surjection, and the declared policy class of
the composite is, by convention, the image of the composite strategy map. For
every strategy \(\pi\) and world \(w\),
\[
...
function composition. If one fixes target policy classes independently of the
maps, closure under composition additionally requires
\(\operatorname{im}(K_2\circ K_1)\) to equal the predeclared target class.
\end{proof}
...
\]
where $\mathbf h_{<\tau}^c(w)$ collects the evaluated predecessor tuples
$(z_\sigma^c(w),m_\sigma^c,a_\sigma,h_\sigma^c(w))_{\sigma\prec\tau}$. For such a complete profile also define
\[
L_{\mathrm{op}}(c,w)=
\max\left(
...
$c_\tau=(r_\tau,a_\tau)$ and
$\mathbf h_{<\tau}=(z_\sigma,m_\sigma,a_\sigma,h_\sigma)_{\sigma\prec\tau}\in
\mathbf{Hist}_{<\tau}$,
\[
...
\cup\{0\}\right).
\]
The added value $0$ fixes the empty-poset convention.
...
$\tau$, every
coordinate of $\mathbf h_{<\tau}$ has already been determined. The resulting
observation lies in $Z_\tau^{\mathrm{act}}$ because the already determined finite
predecessor choice profile extends to a complete choice profile using arbitrary
elements of the nonempty later choice sets. Thus $\pi_\tau(z_\tau)$ is defined,
and the displayed equations determine successively $z_\tau,c_\tau,m_\tau,h_\tau$. Well-founded
...
Figure~\ref{fig:cut-poset-linearisation} illustrates a four-cut poset and two
of its linear-extension evaluations.
\end{remark}
...
\end{array}
\]
\caption{A finite cut poset and two linear-extension evaluations. Incomparable
cuts may be evaluated in either order; confluence says the operational record and
aggregate loss are independent of that order.}
\label{fig:cut-poset-linearisation}
\end{figure}
...
$(\pi,w)$, a configuration over an order ideal $U\in\mathcal I(T)$ records the
execution tuples at cuts in $U$. A cut $\tau\notin U$ is enabled when every
predecessor of $\tau$ lies in $U$; the transition $U\to U\cup\{\tau\}$ computes
its tuple from the recorded strict past.
\end{definition}

\begin{proposition}[Concurrency diamond and confluence]
\label{prop:operational-confluence}
If incomparable cuts $\tau$ and $\sigma$ are both enabled at a configuration over
an order ideal $U\in\mathcal I(T)$, then executing $\tau$ followed by $\sigma$ or
$\sigma$ followed by $\tau$ produces the same configuration over
$U\cup\{\tau,\sigma\}$. Every maximal path
...
$\{\xi:\xi\prec\tau\}\subseteq U$ and writes only the $\tau$ coordinate;
...
$c_{<k}\in\prod_{i<k}C_{\tau_i}$, with choice set $C_{\tau_k}$. Here
$h^k_{w,c_{<k}}$ denotes an arena tree node, distinct from the stage output
values $h_\tau\in H_\tau$. Compute the causal-predecessor record from $w$ and the prefix, ignoring earlier incomparable
...
agree. The choice $c_k$ leads to $h^{k+1}_{w,c_{\le k}}$ for $k<m$, and to a
terminal leaf $h^{m+1}_{w,c_{\le m}}$ at $k=m$, and node loss is
\[
...
\[
1+|W|\sum_{k=1}^{m+1}\prod_{i<k}|C_{\tau_i}|
\]
nodes: the root, all predictor nodes, and all terminals; the term $k=m+1$ counts
terminal leaves. Thus explicit arena
...
\begin{proof}
At stage $k\le m$, a predictor node is determined by a world and one choice for
each of the $k-1$ earlier cuts in $L$, giving
$|W|\prod_{i<k}|C_{\tau_i}|$ nodes. The same product with $k=m+1$ counts the
terminal leaves after a complete choice profile. Adding the nature root proves
the formula. For $m=0$, the empty product is understood as $1$; the formula
counts a nature root with $|W|$ terminal leaves and no predictor information
sets, which may be identified without changing capability. The products can grow
...
\begin{remark}[Concurrency and recall]
When a linear extension orders two incomparable cuts $\tau_1$ and $\tau_2$, the
observation at the later cut does not read the earlier incomparable choice. If
$\tau_1$ has at least two possible choices and $\tau_2$ is placed after it, the
linear-extension arena therefore contains syntactic nodes at $\tau_2$ with the
same current observation but different preceding action histories. In the global
arena-level sense of Definition~\ref{def:information-refinement}, such a
sequentialisation fails perfect recall whenever such an earlier incomparable cut
has at least two choices. Thus Proposition~\ref{prop:operational-perfect-recall}
is nontrivially satisfied only when the cut order is total along the relevant
prefixes, or when all earlier incomparable cuts have singleton choice sets.
\end{remark}
...
\subsection{World-only decoupling}
...
observations, local nonemptiness implies a global family by finite choice
(provable in ZF), so there is no $\mathsf D_\eps$ outcome in that presentation.
\end{theorem}
...
\subsection{Observation-preserving compilation}
...
\subsection{Trace-relative past factorisation}
...
\]
The map $J_\tau^\mu$ is the original observation interface $I_\tau$ on the
past-record space; the superscript records the fixed predecessor trace used to
form $P_\tau^\mu$. The construction is independent of a policy after $\mu$ is
fixed.
\end{proposition}
...
Fix $\tau$ and $\mu$. If two worlds $w_0,w_1\in W$ satisfy
$P_\tau^\mu(w_0)=P_\tau^\mu(w_1)$ and
\[
\bigcap_{i=0}^1
\{c\in C_\tau:
...
\label{def:K-arena}
Let $W=\N\times\N$. The root, denoted $\varepsilon$, is a nature node with moves
...
\section{Functorial and compositional effective causal descent}
\label{sec:functorial-programme}

The preceding sections establish object-level presentations. This section develops
the corresponding morphism-level theory in the cases where the morphisms are
specified explicitly. The guiding separation is
\[
...
\]
A map may preserve the yes--no capability predicate while losing the local
incidence data that distinguish $\mathsf L$ from $\mathsf D$, and a diagnostic
isomorphism may still fail to transport a particular parity, affine, holonomy, or
Farkas certificate unless the algebraic structure used by that certificate is
also preserved.

\subsection{A category of constrained presentations over a fixed base}

Fix a context category $\mathcal C$ and a tolerance level $\eps$. A presentation
object over $\mathcal C$ consists of an assignment functor
$\mathcal A^P:\mathcal C\to\mathbf{Set}$, local admissibility sets
$\LocalAdm_P(c)\subseteq\mathcal A^P(c)$, and effective and bounded realised
images inside the ambient family product, as in Definition~\ref{def:ECD}. Write
\[
\Gamma(P)=
\left(
\Gamma_{\mathrm{set}}(P)
...
\right).
\]
Let $\mathbf{Nested}$ be the category whose objects are nested triples
$X_0\supseteq X_1\supseteq X_2$ and whose morphisms are functions
$h:X_0\to Y_0$ satisfying $h(X_1)\subseteq Y_1$ and
$h(X_2)\subseteq Y_2$.

\begin{definition}[Presentation morphism over a fixed base]
\label{def:presentation-morphism-fixed-base}
For presentations $P,Q$ over $\mathcal C$, a morphism $P\to Q$ consists of a
natural transformation $\alpha:\mathcal A^P\Rightarrow\mathcal A^Q$ such that
\[
\alpha_c(\LocalAdm_P(c))\subseteq\LocalAdm_Q(c)
\qquad(c\in\mathcal C),
\]
and such that componentwise application of $\alpha$ carries the effective and
bounded realised images of $P$ into those of $Q$. Composition is composition of
natural transformations.
\end{definition}

\begin{theorem}[Global-family functor]
\label{thm:global-family-functor-monograph}
The assignment $P\mapsto\Gamma(P)$ extends to a functor
\[
\Gamma:\mathbf{Pres}_{\mathcal C}\longrightarrow\mathbf{Nested}.
\]
On a morphism represented by $\alpha$, the map is
\[
(\Gamma(\alpha)s)_c=\alpha_c(s_c).
\]
\end{theorem}

\begin{proof}
By the hypothesis on $\alpha$,
Proposition~\ref{prop:global-family-functoriality} applies and makes
componentwise application a well-defined map on the set-theoretic global
spaces, preserving the effective and bounded components under the image
hypotheses. Identities and composition are inherited from natural
transformations, giving a functor.
\end{proof}

\begin{remark}[Direction of preservation]
A presentation morphism transports positive success forward: nonempty local
sets, compatible global families, effective realisations, and bounded
realisations map to corresponding witnesses in the target. Failure labels are not
functorial in this forward direction. A target presentation may contain
additional local elements, global families, or implementations not lying in the
image of the source. Preservation and reflection of failure therefore require
isomorphism-strength data.
\end{remark}

\subsection{Changing context bases}

The preceding functoriality result fixes the context category. Structural
comparisons also require controlled changes of base: a presentation may be
renamed, subdivided, or transported along an isomorphic context category without
changing the section problem.

\begin{definition}[Base isomorphism of presentations]
\label{def:base-isomorphism-presentations}
Let $P$ be a presentation over $\mathcal C$ and $Q$ a presentation over
$\mathcal D$. A base isomorphism $P\cong Q$ consists of an isomorphism of
categories $F:\mathcal C\to\mathcal D$, a natural isomorphism
\[
\alpha:\mathcal A^P\Rightarrow \mathcal A^Q\circ F,
\]
objectwise equalities
\[
\alpha_c(\LocalAdm_P(c))=\LocalAdm_Q(Fc),
\]
and bijections of effective and bounded implementation images compatible with
the induced map on global families.
\end{definition}

Unpacking the naturality of $\alpha$: for every arrow $f:c\to c'$ of
$\mathcal C$, the square
\begin{center}
\begin{tikzcd}[sep=small]
\mathcal A^P(c) \ar[r,"\alpha_c"] \ar[d,"\mathcal A^P(f)"'] &
\mathcal A^Q(Fc) \ar[d,"\mathcal A^Q(Ff)"]\\
\mathcal A^P(c') \ar[r,"\alpha_{c'}"'] & \mathcal A^Q(Fc')
\end{tikzcd}
\end{center}
commutes.

\begin{theorem}[Invariance under base isomorphism]
\label{thm:base-isomorphism-invariance}
A base isomorphism of presentations induces an isomorphism of obstruction data.
Consequently it preserves and reflects
\[
\mathsf L_\eps,\quad \mathsf D_\eps,\quad \mathsf K_\eps,\quad
\mathsf R_\eps,\quad \checkmark_\eps .
\]
\end{theorem}

\begin{proof}
Precomposition with an isomorphism of categories only renames the compatibility
equations. Componentwise application of the natural isomorphism $\alpha$ then
carries compatible families of $P$ bijectively to compatible families of $Q$.
The displayed equality of local sets preserves objectwise local nonemptiness and
local admissibility. The implementation-image hypotheses identify the effective
and bounded subspaces. Thus the local-success bit and all three nested global
spaces are preserved and reflected.
\end{proof}

\begin{remark}[Refinements versus base isomorphisms]
A base isomorphism is a safe change of coordinates. Noninvertible refinements are
more delicate. Adding objects that are determined by existing restrictions can
preserve the section problem, while deleting an initial object or quotienting
contexts can change the diagnostic. Such refinements require their own
cofinality, density, or certificate-preservation hypotheses.
\end{remark}

\subsection{Capability, diagnostic, and certificate morphisms}

For behaviourally annotated presentations, a capability morphism preserves the
policy or strategy space together with worldwise loss, possibly up to a declared
loss-overhead map. This is enough to transport capability, but it need not
preserve local incidence. A diagnostic morphism preserves the obstruction datum
of Definition~\ref{def:obstruction-datum}. A certificate morphism additionally
preserves the local objects, local admissibility sets, restriction maps, and any
algebraic or convex structure used to express a certificate.

\begin{definition}[Morphism levels]
\label{def:morphism-levels}
A \emph{capability isomorphism} identifies strategy or policy spaces and
worldwise loss functionals (Definition~\ref{def:equivalence-levels}). A
\emph{diagnostic isomorphism} is an isomorphism of obstruction data
(Definition~\ref{def:obstruction-datum}). A \emph{certificate isomorphism} is a
diagnostic isomorphism induced by an isomorphism of the underlying
local-incidence presentation,
including any linear, affine, convex, torsor, or order structure used by the
certificate language.
\end{definition}

\begin{proposition}[Strictness of morphism levels]
\label{prop:morphism-level-strictness}
Capability equivalence need not imply diagnostic equivalence, and diagnostic
equivalence need not imply certificate equivalence.
\end{proposition}

\begin{proof}
For the first assertion, take a compatibility-failing presentation and its
terminal compression. The nested global spaces, and hence capability at the
set-theoretic, effective, and bounded levels, are preserved, but the outcome
changes from $\mathsf D$ to $\mathsf L$ by
Theorem~\ref{thm:compression-boundary}. For the second assertion, duplicate a
constraint occurrence in a finite incidence diagram. The local-success bit and
all nested global spaces can remain unchanged, while the incidence structure and
minimal finite certificates change. A diagnostic datum records emptiness and
nonemptiness of the distinguished spaces, not the multiplicity or shape of local
constraint occurrences.
\end{proof}

\begin{theorem}[Certificate transport under structured isomorphism]
\label{thm:certificate-transport-monograph}
A certificate isomorphism transports every certificate construction defined from
the preserved structure. In particular, it transports finite local certificates,
finite incompatibility subdiagrams, parity certificates, affine inconsistency
certificates, holonomy fixed-point certificates, and Farkas certificates for
finite polyhedral systems whenever the corresponding finite, algebraic, torsor,
or convex structure is included in the isomorphism data.
\end{theorem}

\begin{proof}
Finite incidence certificates transport by relabelling the referenced objects,
arrows, and local elements. In an affine presentation, a certificate of
inconsistency is a finite linear combination of equations with zero left-hand
side and nonzero right-hand side; affine coordinate and equation isomorphisms
preserve that identity. In a bijective network, an isomorphism conjugates the
loop holonomies, and conjugation preserves common fixed-point emptiness. In a
finite polyhedral presentation, a Farkas certificate is a dual vector satisfying
linear equalities and inequalities; declared affine coordinate changes,
constraint permutations, and positive row scalings preserve those relations. The
same argument applies to any certificate scheme functorially defined from the
preserved local structure.
\end{proof}

\begin{remark}
Noninvertible maps require separate soundness theorems for each certificate
language. A map into a more permissive system can send an unsatisfiable source to
a satisfiable target. Conversely, a quotient can hide the local structure needed
to identify a minimal certificate. Certificate preservation is therefore a
stronger requirement than diagnostic preservation.
\end{remark}

\subsection{Terminal compression as an idempotent functor}

At a fixed tolerance, define a terminal-presentation functor
$\mathrm{Term}:\mathbf{Nested}\to\mathbf{Pres}_{\mathbf 1}$ by sending
$X_0\supseteq X_1\supseteq X_2$ to the terminal-category presentation with
assignment set $X_0$, sole local admissibility set $X_0$, and realised effective
and bounded images $X_1$ and $X_2$.

\begin{theorem}[Terminal retraction]
\label{thm:terminal-retraction-monograph}
At fixed tolerance,
\[
\Gamma\circ\mathrm{Term}=\mathrm{Id}_{\mathbf{Nested}}.
\]
Consequently $\operatorname{ExtComp}=\mathrm{Term}\circ\Gamma$ is idempotent
whenever it is regarded as an endofunctor on a category containing the relevant
context shapes:
\[
\operatorname{ExtComp}^2=\operatorname{ExtComp}.
\]
For a fixed nonterminal base $\mathcal C$, the correctly typed statement is
$\operatorname{ExtComp}_{\mathbf 1}\circ\operatorname{ExtComp}_{\mathcal C}
=\operatorname{ExtComp}_{\mathcal C}$.
\end{theorem}

\begin{proof}
A terminal-category diagram has no nontrivial matching equation, so its
set-theoretic global-family space is its sole local set, namely $X_0$. Its
effective and bounded realised images are $X_1$ and $X_2$ by construction. Thus
$\Gamma(\mathrm{Term}(X_0\supseteq X_1\supseteq X_2))=X_0\supseteq X_1\supseteq
X_2$. Idempotence follows by substitution:
\[
(\mathrm{Term}\Gamma)(\mathrm{Term}\Gamma)
=\mathrm{Term}(\Gamma\mathrm{Term})\Gamma
=\mathrm{Term}\Gamma.
\]
The final assertion records that terminal compression changes the base category
from $\mathcal C$ to $\mathbf 1$.
\end{proof}

The two identities admit the standard retraction picture. On a nested
triple $N=X_0\supseteq X_1\supseteq X_2$, the identity
$\Gamma\circ\mathrm{Term}=\mathrm{Id}$ is the retraction square
\begin{center}
\begin{tikzcd}[sep=small]
N \ar[r,"\mathrm{Term}"] \ar[d,equal] &
\mathrm{Term}(N) \ar[d,"\Gamma"]\\
N \ar[r,equal] & N
\end{tikzcd}
\end{center}
and the idempotence $\operatorname{ExtComp}^2=\operatorname{ExtComp}$ is
the triangle
\begin{center}
\begin{tikzcd}[sep=small]
& P \ar[dl,"\operatorname{ExtComp}"'] \ar[dr,"\operatorname{ExtComp}"]\\
\operatorname{ExtComp}(P) \ar[rr,"\operatorname{ExtComp}"{description}] &&
\operatorname{ExtComp}(P)
\end{tikzcd}
\end{center}
whose bottom edge acts as the identity on the compressed image, as the
typed identity of the theorem records.

\begin{proposition}[Decompression fibres of spectra]
\label{prop:decompression-fibres-monograph}
Let a compressed spectrum have the form
$(S_D,S_D,S_K,S_R)$. At the spectral level, its possible decompressions are
exactly the quadruples
\[
(S_L,S_D,S_K,S_R)
\]
where $S_L$ is upward closed and $S_D\subseteq S_L$. Thus the forgotten spectral
datum is precisely the possible $\mathsf D$-gap $S_L\setminus S_D$.
\end{proposition}

\begin{proof}
Corollary~\ref{cor:compression-spectrum} shows that terminal compression replaces
$T_{\mathsf L}$ by $T_{\mathsf D}$ and leaves the other three success spectra
unchanged. Conversely, the nesting theorem requires only that $S_L$ be an
upward-closed superset of $S_D$ at the level of spectra. Certificate-level
decompression is strictly richer, because many nonisomorphic local-incidence
diagrams can realise the same success spectra.
\end{proof}

\begin{theorem}[Realisation of abstract success spectra]
\label{thm:abstract-spectrum-realisation}
Let $E$ be a tolerance poset and let
\[
S_{\mathsf R}\subseteq S_{\mathsf K}\subseteq S_{\mathsf D}\subseteq S_{\mathsf L}
\]
be upward-closed subsets of $E$. In the unrestricted set-based category of
tolerance-filtered presentations, there is a presentation $P$ with
\[
(T_{\mathsf L}(P),T_{\mathsf D}(P),T_{\mathsf K}(P),T_{\mathsf R}(P))
=(S_{\mathsf L},S_{\mathsf D},S_{\mathsf K},S_{\mathsf R}).
\]
\end{theorem}

\begin{proof}
Use the cospan category $c_0\to u\leftarrow c_1$. Let
\[
\mathcal A(c_0)=\{\ell_0,d,k,r\},\quad
\mathcal A(c_1)=\{\ell_1,d,k,r\},\quad
\mathcal A(u)=\{0,1,d,k,r\},
\]
with restrictions $\ell_0\mapsto0$, $\ell_1\mapsto1$, and $d,k,r$ mapped to the
same-named elements of $\mathcal A(u)$. At tolerance $\eps$, put
\begin{align*}
\LocalAdm_\eps(c_0)&=
\{\ell_0:\eps\in S_{\mathsf L}\}\cup
\{d:\eps\in S_{\mathsf D}\}\cup
\{k:\eps\in S_{\mathsf K}\}\cup
\{r:\eps\in S_{\mathsf R}\},\\
\LocalAdm_\eps(c_1)&=
\{\ell_1:\eps\in S_{\mathsf L}\}\cup
\{d:\eps\in S_{\mathsf D}\}\cup
\{k:\eps\in S_{\mathsf K}\}\cup
\{r:\eps\in S_{\mathsf R}\},\\
\LocalAdm_\eps(u)&=
\{0,1:\eps\in S_{\mathsf L}\}\cup
\{d:\eps\in S_{\mathsf D}\}\cup
\{k:\eps\in S_{\mathsf K}\}\cup
\{r:\eps\in S_{\mathsf R}\}.
\end{align*}
Here $\{x:\phi\}$ denotes $\{x\}$ when $\phi$ holds and $\varnothing$ otherwise,
and $\{0,1:\phi\}$ denotes $\{0,1\}$ when $\phi$ holds and $\varnothing$
otherwise. All local sets are nonempty exactly on $S_{\mathsf L}$. If
$\eps\in S_{\mathsf L}\setminus S_{\mathsf D}$, the only available special
choices are $\ell_0,\ell_1,0,1$, and they cannot form a matching family because
the two restrictions require the $u$-component to be both $0$ and $1$. If
$\eps\in S_{\mathsf D}$, the matching family $(d,d,d)$ exists. Thus
set-theoretic success is exactly $S_{\mathsf D}$.

Let the effective realised image be declared to be exactly the subset consisting
of the matching families $(k,k,k)$ and $(r,r,r)$, and let the bounded realised
image be declared to be exactly $(r,r,r)$. In particular, the set-theoretic family
$(d,d,d)$ is not in the effective image unless it is explicitly included. Since
$S_{\mathsf R}\subseteq S_{\mathsf K}\subseteq S_{\mathsf D}$, effective success
occurs exactly on $S_{\mathsf K}$ and bounded success exactly on
$S_{\mathsf R}$. The local sets are monotone because all four $S$-sets are
upward closed.
\end{proof}

\begin{remark}
Theorem~\ref{thm:abstract-spectrum-realisation} is deliberately set-based. In
represented or finite-parametric subcategories, only spectra admitting the
corresponding descriptions are realisable. Thus the result identifies the
abstract degrees of freedom forgotten by compression; it is not a uniform
effective construction for arbitrary unrepresented subsets of a tolerance poset.
\end{remark}

\subsection{Empirical-support packaging as a functor on isomorphisms}

\begin{definition}[Empirical-support isomorphism]
\label{def:empirical-support-isomorphism-monograph}
An isomorphism between finite empirical support models consists of a measurement
bijection carrying maximal contexts to maximal contexts, outcome bijections on
measurements, and exact preservation of the support relation in every maximal
context.
\end{definition}

\begin{theorem}[Functorial support packaging on the support groupoid]
\label{thm:support-packaging-functoriality-monograph}
The downward-closed packaging of Definition~\ref{def:packaging} is functorial on
the groupoid of finite empirical support models and support isomorphisms. It
preserves and reflects global supported assignments and therefore preserves and
reflects strong contextuality.
\end{theorem}

\begin{proof}
A support isomorphism sends every context $D$ in the downward closure to its
image context and acts coordinatewise on assignments. Coordinatewise maps commute
with restriction, and support preservation on maximal contexts implies support
preservation on all restricted supports. Hence the packaging is carried to an
isomorphic constrained diagram. A global assignment $g$ is supported on every
maximal context exactly when the transported global assignment is supported on
every transported maximal context. Strong contextuality is the nonexistence of
such a global assignment with all local supports nonempty.
\end{proof}

\subsection{Noninvertible empirical reindexing doctrines}
\label{subsec:noninvertible-empirical}

Support isomorphisms preserve all structure. Noninvertible maps do not have a
single canonical variance. The three basic doctrines are direct image, inverse
image, and saturation. Each preserves a different part of the local-to-global
problem.

\begin{definition}[Outcome coarse-graining and direct image]
\label{def:empirical-direct-image}
Fix a finite measurement scenario with contexts $M$, let
$X=\bigcup_{C\in M}C$ be the measurement set, and let
$q_x:O_x\to O'_x$ be outcome maps. For a support model $S=(S_C)_{C\in M}$,
define the direct image support model by
\[
(q_*S)_C=q_C(S_C)\subseteq\prod_{x\in C}O'_x,
\]
where $q_C=\prod_{x\in C}q_x$.
\end{definition}

\begin{proposition}[Direct image preserves global sections forward]
\label{prop:empirical-direct-image}
If $g\in\prod_xO_x$ is a global supported assignment for $S$, then
$q_X(g)\in\prod_xO'_x$ is a global supported assignment for $q_*S$. Thus direct
image preserves noncontextuality forward. It need not preserve strong
contextuality.
\end{proposition}

\begin{proof}
For every context $C$, $g|_C\in S_C$. Applying $q_C$ gives
$q_X(g)|_C=q_C(g|_C)\in q_C(S_C)=(q_*S)_C$. Hence $q_X(g)$ is global for the
direct image. For failure of preservation of strong contextuality, take any
strongly contextual support model and map every outcome set to a singleton. The
direct image then has the unique global assignment.
\end{proof}

\begin{definition}[Inverse-image reindexing]
\label{def:empirical-inverse-image}
For a target support model $T=(T_C)_C$ on outcome sets $O'_x$ and outcome maps
$q_x:O_x\to O'_x$, define
\[
(q^*T)_C=q_C^{-1}(T_C)\subseteq\prod_{x\in C}O_x.
\]
\end{definition}

\begin{proposition}[Inverse image and global sections]
\label{prop:empirical-inverse-image}
There is an equality of global-section sets
\[
\Gamma(q^*T)=q_X^{-1}(\Gamma(T)).
\]
Consequently, if $T$ is strongly contextual then $q^*T$ has no global section.
If all $q_x$ are surjective and the local supports of $T$ are nonempty, then the
local supports of $q^*T$ are nonempty as well, so strong contextuality of $T$
pulls back to a $\mathsf D$ outcome for $q^*T$.
\end{proposition}

\begin{proof}
A global assignment $g\in\prod_xO_x$ lies in $\Gamma(q^*T)$ exactly when
$g|_C\in q_C^{-1}(T_C)$ for every context $C$, equivalently when
$q_X(g)|_C=q_C(g|_C)\in T_C$ for every $C$. This is exactly
$q_X(g)\in\Gamma(T)$. Surjectivity of each $q_x$ makes each product map $q_C$
surjective, so preimages of nonempty local supports are nonempty.
\end{proof}

\begin{definition}[Global-section saturation]
\label{def:empirical-saturation}
For a support model $S$, define its global-section saturation by
\[
S_C^{\mathrm{sat}}=\{g|_C:g\in\Gamma(S)\}.
\]
\end{definition}

\begin{proposition}[Saturation preserves global sections and erases certificates]
\label{prop:empirical-saturation}
For every finite support model,
\[
\Gamma(S^{\mathrm{sat}})=\Gamma(S),
\qquad
(S^{\mathrm{sat}})^{\mathrm{sat}}=S^{\mathrm{sat}}.
\]
If $S$ is strongly contextual, then every nonempty-context support in
$S^{\mathrm{sat}}$ is empty. Thus saturation preserves global-section emptiness
but changes a support-level $\mathsf D$ presentation into a local failure.
\end{proposition}

\begin{proof}
Every $g\in\Gamma(S)$ restricts into $S_C^{\mathrm{sat}}$ for each context, so
$\Gamma(S)\subseteq\Gamma(S^{\mathrm{sat}})$. Conversely,
$S_C^{\mathrm{sat}}\subseteq S_C$ whenever $S$ is the downward support packaging,
so a global assignment supported by $S^{\mathrm{sat}}$ is supported by $S$. The
same equality of global sections gives idempotence. If $\Gamma(S)=\varnothing$,
then no restrictions $g|_C$ exist, so each nonempty-context saturated support is
empty.
\end{proof}

\begin{remark}
Direct image, inverse image, and saturation are all natural operations, but they
answer different questions. Direct image models coarse-graining of recorded
outcomes; inverse image models refinement of outcome labels; saturation records
only what is extendable to a global assignment. None is the canonical
noninvertible morphism of empirical models without an explicitly chosen
semantics.
\end{remark}

\subsection{Distributed selectors and blind-order arenas as a groupoid equivalence}

Let $\mathbf{Dist}_{\mathrm{red}}$ be the groupoid of finite distributed selector
problems whose observation sets are the active observation images, with
isomorphisms given by bijections of worlds, agents, observations, and choices
preserving interfaces, scopes, and relations. Let $\mathbf{Blind}$ be the
groupoid of finite blind-order one-shot arenas in strict normal form, with
isomorphisms preserving the rooted tree, world branches, level order,
information sets, choice sets, and loss tables.

\begin{theorem}[Reduced distributed selectors and blind-order arenas]
\label{thm:dist-blind-groupoid-monograph}
The constructions $\mathsf N$ and $\mathsf{Dist}$ of
Theorem~\ref{thm:one-shot-normal-form} restrict to an equivalence of groupoids
\[
\mathsf N:\mathbf{Dist}_{\mathrm{red}}
\rightleftarrows
\mathbf{Blind}:\mathsf{Dist}.
\]
They preserve pure strategies, worldwise zero--one loss, and pure-strategy
capability.
\end{theorem}

\begin{proof}
In a reduced distributed problem, every observation labels an information set in
$\mathsf N(P)$. A pure strategy in the arena is therefore exactly one map
$\im(I_j)\to C_j$ for each agent. Terminal loss was defined to be the distributed
validity predicate. Reading a strict normal-form arena back recovers its worlds,
agent order, observation labels, choices, scopes, and terminal relations.
Structure-preserving isomorphisms act by the same relabellings on both sides,
so the object-level bijections assemble into an equivalence of groupoids.
\end{proof}

\begin{remark}
This is a capability and loss equivalence. It is not automatically a diagnostic
or certificate equivalence between an incidence presentation and a full arena
patch presentation. Such preservation requires transporting the local-incidence
presentation as additional data.
\end{remark}

\subsection{Arena patch sheaves under arena isomorphisms}

\begin{theorem}[Patch-sheaf functoriality under arena isomorphisms]
\label{thm:patch-sheaf-functoriality-monograph}
An isomorphism of finite imperfect-information arenas preserving worlds, rooted
tree structure, node types, nature maps, information sets, choice sets, and loss
tables induces an isomorphism of information-saturated patch sites and an
isomorphism of the strategy sheaves carrying
$\mathscr V_{\eps,\mathcal I}$ onto $\mathscr V'_{\eps,\mathcal I'}$. Hence it
preserves and reflects global activated-valid strategies and pure
$\eps$-capability.
\end{theorem}

\begin{proof}
The node bijection carries prefix-closed information-saturated patches to such
patches and commutes with intersections and union covers. Choice-set bijections
act coordinatewise on partial strategies and commute with restriction. Because
worlds, nature maps, selected edges, and losses are preserved, a node is reached
under a partial strategy exactly when its image is reached under the transported
partial strategy, and the loss values agree. Thus activated validity is preserved
and reflected. The inverse arena isomorphism supplies the inverse sheaf map.
\end{proof}

\subsection{Constraint-system composition}

\begin{proposition}[Conjunction and disjoint union]
\label{prop:csp-composition-monograph}
For finite CSP presentations over a common variable set, occurrence-preserving
conjunction has solution set
\[
\mathrm{Sol}(P\wedge Q)=\mathrm{Sol}(P)\cap\mathrm{Sol}(Q).
\]
For presentations over disjoint variable sets,
\[
\mathrm{Sol}(P\sqcup Q)\cong\mathrm{Sol}(P)\times\mathrm{Sol}(Q).
\]
If effective and bounded implementation objects factor as products in the
disjoint-union case, the same product law holds at the effective and bounded
levels.
\end{proposition}

\begin{proof}
A valuation satisfies the occurrence-preserving conjunction exactly when it
satisfies both original lists of constraints. For disjoint union, a valuation on
the union of variables is uniquely a pair of valuations, one on each side, and
constraints do not mix the sides. The implementation-level statement is the same
argument applied to the assumed product realisation maps and product resource
filters.
\end{proof}

\begin{proposition}[Shared interfaces and hiding]
\label{prop:pullback-hiding-monograph}
In this proposition, $\mathrm{Sol}(P)$ denotes the set-theoretic global-family
space $\Gamma_{\mathrm{set}}(P)$ of a constraint presentation. If presentations
$P$ and $Q$ have solution maps to a shared interface presentation $I$ and the
pullback presentation is formed semantically, then
\[
\mathrm{Sol}(P\times_I Q)
\cong
\mathrm{Sol}(P)\times_{\mathrm{Sol}(I)}\mathrm{Sol}(Q).
\]
If a set of internal variables is hidden, the visible solution relation is the
projection of the full solution relation. Hiding preserves set-level nonemptiness
but may erase diagnostics and certificates, and implementation-level equality
requires uniform witness lifting.
\end{proposition}

\begin{proof}
The pullback presentation imposes exactly the condition that a solution of $P$
and a solution of $Q$ have the same interface value; this is the defining
property of the fibre product. Hiding existentially quantifies the internal
variables, hence takes the image of the full solution relation under projection.
Projection sends nonempty sets to nonempty sets, but it need not preserve local
incidence, minimal unsatisfiable subdiagrams, or computable access to a hidden
witness.
\end{proof}

\subsection{Parallel, sequential, and guarded-feedback composition}

\begin{proposition}[Parallel capability under independent implementations]
\label{prop:parallel-capability-monograph}
Suppose $P\otimes Q$ is formed with independent worlds, disjoint context
components, componentwise success, and an implementation-pairing operation whose
realisation is the product of realised families. If $P$ and $Q$ are effectively
capable, then $P\otimes Q$ is effectively capable. If the paired implementation
has resource cost bounded by $b_P\oplus b_Q$, bounded capability composes at the
combined budget.
\end{proposition}

\begin{proof}
Pair the two successful implementations. The realised global family is valid on
each component, hence valid for the componentwise success predicate of the
parallel presentation. The resource claim is exactly the assumed cost inequality.
\end{proof}

\begin{remark}
The converse and exact product laws require projection of joint implementations
back to component implementations. Shared memory, shared randomness, preprocessing,
amortisation, or nonfactorising budgets can invalidate such projections. Parallel
composition is therefore doctrine-dependent at the implementation and resource
levels.
\end{remark}

\subsection{Resource-overhead calculus}
\label{subsec:resource-overhead-calculus}

Let $(B,\le,\oplus,0)$ be an ordered resource monoid. A budgeted implementation
doctrine assigns monotone implementation classes
\[
\mathrm{Impl}_b(P)\subseteq\mathrm{Impl}_{\mathrm{eff}}(P)
\qquad(b\in B),
\]
and corresponding realised spaces
\[
\Gamma_b^\eps(P)=\Gamma_{\mathrm{set}}^\eps(P)
\cap \sigma_{\mathrm{eff}}(\mathrm{Impl}_b(P)).
\]
The bounded success spectrum is then the upward-closed subset
\[
T_{\mathsf R}(P)=\{(\eps,b):\Gamma_b^\eps(P)\ne\varnothing\}.
\]

\begin{proposition}[Spectrum transport with overhead]
\label{prop:overhead-spectrum-transport}
Suppose a presentation morphism $F:P\to Q$, with assignment components
$F_c=\alpha_c$, has tolerance overhead $\rho$, an unbudgeted effective
implementation translator, and resource overhead $H$, meaning that
\[
\alpha_c(\LocalAdm_P^\eps(c))\subseteq \LocalAdm_Q^{\rho(\eps)}(c),
\]
there is a translator $\operatorname{trans}_{\mathrm{eff}}$ satisfying
\[
\sigma_{\mathrm{eff}}^Q(\operatorname{trans}_{\mathrm{eff}}(\iota))
=\alpha_*(\sigma_{\mathrm{eff}}^P(\iota)),
\]
and implementations of budget $b$ in $P$ are translated, with the same
realisation compatibility, to implementations of budget $H(b)$ in $Q$. Then
\[
\eps\in T_\chi(P)\Longrightarrow \rho(\eps)\in T_\chi(Q)
\qquad(\chi\in\{\mathsf L,\mathsf D,\mathsf K\}),
\]
and
\[
(\eps,b)\in T_{\mathsf R}(P)
\Longrightarrow
(\rho(\eps),H(b))\in T_{\mathsf R}(Q).
\]
\end{proposition}

\begin{proof}
The proof transports witnesses. Objectwise local witnesses are mapped into the
larger-tolerance local sets of $Q$. Compatible global families are transported by
the global-family functor. Effective witnesses are transported by the effective
implementation translator. A budget-$b$ witness is transported to a
budget-$H(b)$ witness by the resource-overhead hypothesis.
\end{proof}

\begin{proposition}[Parallel resource convolution]
\label{prop:parallel-resource-convolution}
Assume $P\otimes Q$ has independent implementations and cost aggregation by
$\oplus$: pairing a budget-$b_P$ implementation of $P$ with a budget-$b_Q$
implementation of $Q$ gives a budget-$b_P\oplus b_Q$ implementation of
$P\otimes Q$. For a componentwise success predicate,
\[
\bigcup_{b_P\oplus b_Q\le b}
\Gamma_{b_P}^\eps(P)\times\Gamma_{b_Q}^\eps(Q)
\subseteq
\Gamma_b^\eps(P\otimes Q).
\]
If every budget-$b$ implementation of $P\otimes Q$ projects to component
implementations with budgets $b_P,b_Q$ satisfying $b_P\oplus b_Q\le b$, then the
inclusion is equality.
\end{proposition}

\begin{proof}
The inclusion is the stated pairing operation and the componentwise success
criterion. For equality, take a budget-$b$ composite witness and project it to
component witnesses. By hypothesis their budgets combine to at most $b$, and the
componentwise success predicate gives membership in the corresponding product on
the left.
\end{proof}

\begin{remark}
The operation $\oplus$ is doctrine-specific. It may be addition for total work,
maximum for ideal parallel time, a communication-cost sum, or a vector-valued
resource operation. The abstract framework supplies the monotone transport law;
resource additivity must be proved in the chosen implementation model.
\end{remark}

\subsection{Minimax thresholds and resource margins}
\label{subsec:minimax-resource-calculus}

Loss-induced presentations give a quantitative version of the four-level
diagnostic. Suppose each local condition is generated by a loss map
$L_c:\mathcal A(c)\to[0,\infty]$ via
\[
\LocalAdm_\eps(c)=\{a\in\mathcal A(c):L_c(a)\le\eps\}.
\]
Let $\mathcal S_{\mathrm{set}}$ be the structurally compatible family space, and
let $\mathcal S_{\mathrm{eff}}$ and $\mathcal S_{\mathcal B}$ be its intersections
with the effective and bounded realised images. Define
\[
\theta_{\mathsf L}=\sup_c\inf_{a\in\mathcal A(c)}L_c(a),
\]
and, for $X\in\{\mathrm{set},\mathrm{eff},\mathcal B\}$,
\[
\theta_X=\inf_{s\in\mathcal S_X}\sup_c L_c(s_c),
\]
with the convention $\inf\varnothing=+\infty$.

\begin{proposition}[Minimax tower]
\label{prop:minimax-tower-monograph}
For a loss-induced presentation,
\[
\theta_{\mathsf L}\le
\theta_{\mathrm{set}}\le
\theta_{\mathrm{eff}}\le
\theta_{\mathcal B}.
\]
Moreover, endpoint membership is governed by attainment: if the relevant local
or global infimum is attained at value at most $\eps$, then $\eps$ lies in the
corresponding success set; if no minimiser exists, the threshold may be an
infimum not belonging to the success set.
\end{proposition}

\begin{proof}
For every compatible family $s$, the component $s_c$ is a candidate in
$\mathcal A(c)$, so
\[
\inf_{a\in\mathcal A(c)}L_c(a)\le L_c(s_c)
\]
for every $c$. Taking the supremum over $c$ and then the infimum over compatible
families gives $\theta_{\mathsf L}\le\theta_{\mathrm{set}}$. The remaining
inequalities follow from
$\mathcal S_{\mathcal B}\subseteq\mathcal S_{\mathrm{eff}}\subseteq
\mathcal S_{\mathrm{set}}$. The statement about endpoints is the distinction
between an infimum and a realised minimiser.
\end{proof}

\begin{remark}[Interpreting the gaps]
The differences
\[
\theta_{\mathrm{set}}-\theta_{\mathsf L},\quad
\theta_{\mathrm{eff}}-\theta_{\mathrm{set}},\quad
\theta_{\mathcal B}-\theta_{\mathrm{eff}}
\]
when finite measure, respectively, the tolerance cost of gluing, effective
realisation, and bounded realisation. They are threshold gaps, not an additive
physical decomposition. Additivity requires an additional theorem in the chosen
loss and resource doctrine.
\end{remark}

\begin{remark}[Relation to the single-observer degrees]
The quantities $d_{\mathrm{pt}},d_{\mathrm{obs}},d_{\mathrm{eff}},d_{\mathcal B}$
of Definition~\ref{def:degree} are the single-observer prediction-game form of
this minimax tower. In that special case the local pointwise optimum is
$d_{\mathrm{pt}}$, the set-theoretic observation-uniform optimum is
$d_{\mathrm{obs}}$, and the realised optima are $d_{\mathrm{eff}}$ and
$d_{\mathcal B}$. The symbols $\theta$ are used here for the more general
multi-context threshold presentation.
\end{remark}

\begin{proposition}[Affine overhead composition]
\label{prop:affine-overhead-composition}
If two morphisms have affine tolerance overheads
\[
\rho_F(\eps)=L_F\eps+\eta_F,
\qquad
\rho_G(\eps)=L_G\eps+\eta_G,
\]
with $L_F,L_G\ge0$, then the composite overhead is
\[
\rho_{G\circ F}(\eps)=L_GL_F\eps+L_G\eta_F+\eta_G.
\]
The same formula applies to scalar affine cost overheads.
\end{proposition}

\begin{proof}
The composite overhead is the composite function
$\rho_G\circ\rho_F$:
\[
\rho_G(\rho_F(\eps))=L_G(L_F\eps+\eta_F)+\eta_G
=L_GL_F\eps+L_G\eta_F+\eta_G.
\]
\end{proof}

\begin{proposition}[Sequential section composition]
\label{prop:sequential-composition-monograph}
Let $P$ be a first-stage presentation and let $Q[y]$ be continuation
presentations indexed by realised first-stage interface values. If $s$ is a valid
section of $P$ and $t_y$ is a compatible valid continuation section for each
reachable $y$, then the sequential composite has a valid section. Effective
sequential capability additionally requires a uniformly effective map
$y\mapsto t_y$ and an implementation language closed under the corresponding
bind operation.
\end{proposition}

\begin{proof}
The set-theoretic composite section is the dependent pair consisting of the
first-stage section and the family of continuation sections on the interface
values reached by it. Compatibility is imposed by the definition of the
sequential composite. For the effective statement, the first-stage realiser
computes the interface value and then invokes the uniformly supplied continuation
realiser; this is exactly closure under bind.
\end{proof}

\begin{proposition}[Guarded feedback]
\label{prop:guarded-feedback-monograph}
Let $(J,\prec)$ be a finite well-founded dependency order. Suppose each component
$j$ has a nonempty set of admissible outputs for every valid predecessor record.
Then a global feedback section exists. If each admissible output set is a
singleton, the section is unique. Effective and bounded versions require,
respectively, uniform effective selection from the local output sets and a
resource recurrence bounding the well-founded evaluation.
\end{proposition}

\begin{proof}
Choose any linear extension of the finite order. At stage $j$, all predecessor
outputs have already been chosen, and the local admissible set is nonempty by
hypothesis; choose an element of it. This constructs a global section by finite
induction. If every local set is a singleton, the choice at each stage is forced.
The effective and bounded statements add precisely the stated effective selector
and cost recurrence to the same induction.
\end{proof}

\subsection{Finite tabular verifier games}

The certificate transport of Theorem~\ref{thm:certificate-transport-monograph}
also covers the finite tabular certificates; the following representation turns
any finite tabular constrained presentation into a finite verifier game, so
that certificates and refutations become winning strategies and their
absence.

\begin{theorem}[Finite verifier-game representation]
\label{thm:finite-verifier-game-representation}
Every finite tabular constrained presentation has a finite verifier-game
presentation with the same set-theoretic global-family space. If the effective
and bounded realised images are transported as declared subsets of the same
strategy space, the three nested global spaces are preserved.
\end{theorem}

\begin{proof}
Let the original finite presentation have context objects $c$, assignment sets
$\mathcal A(c)$, local sets $\LocalAdm(c)$, and restriction maps
$\mathcal A(f):\mathcal A(c)\to\mathcal A(c')$. The verifier game has one pure
strategy coordinate $x_c\in\mathcal A(c)$ for every context object. It has one
local test world for each object $c$, accepting exactly when
$x_c\in\LocalAdm(c)$, and one compatibility test world for each arrow
$f:c\to c'$, accepting exactly when $\mathcal A(f)(x_c)=x_{c'}$. A pure strategy
wins all verifier worlds exactly when the family $(x_c)_c$ is locally admissible
and satisfies every restriction equation, i.e. exactly when it is an element of
$\Gamma_{\mathrm{set}}$ for the original presentation. Transporting the effective
and bounded images as subsets of this common strategy space preserves the nested
intersections defining $\Gamma_{\mathrm{eff}}$ and $\Gamma_{\mathcal B}$.
\end{proof}

\begin{remark}
The construction is a finite tabular representation theorem, not a physical
realisation theorem. It preserves prediction-relevant finite data by explicitly
turning local admissibility and restriction equations into verifier tests. It may
change certificate size or presentation geometry unless the incidence decoration
is retained.
\end{remark}

\subsection{Source-domain placement principle}

\begin{definition}[Source-domain placement]
\label{def:source-domain-placement}
A placement of a source-domain theorem in the ECD taxonomy consists of a source
class, a target presentation class, a translation from source objects to target
presentations, declared preservation properties, and a proof that the translated
object has the asserted diagnostic outcome. Without these data, no diagnostic
placement is asserted. Table~\ref{tab:source-domain-status} records the current
status of the representative source-domain placements.
\end{definition}

\begin{table}[htbp]
\centering
\caption{Status of representative source-domain placements in the monograph.}
\label{tab:source-domain-status}
\begin{tabular}{@{}p{0.24\linewidth}p{0.28\linewidth}p{0.38\linewidth}@{}}
\toprule
Source domain & Status here & Additional structure needed for broader claims\\
\midrule
Finite Bell scenarios & finite coupling, Farkas, robustness, and repair theory developed & fixed scenario, setting distribution for finite-sample claims, declared repair model\\
Finite empirical support models & support packaging and support-isomorphism functoriality developed & noninvertible reindexing doctrine for broader functoriality\\
Finite source integration & distributed-selector representation and $\#\mathrm P$ counting lower bound developed & chosen repair/edit-cost doctrine for database repair claims\\
Protocol complexes & finite section representation developed & process model, topology, carrier theorem, and protocol legality for impossibility placements\\
Decentralised control & representation target & observation histories, measurability, controller architecture, and performance objective\\
Gödel-style diagonals & theoremhood selector placement developed & truth variants require soundness/Tarski assumptions and internal representability data\\
No-cloning/no-broadcasting & finite-dimensional quantum placement developed & broader process theories require tensor, states, channels, and success predicates\\
Quantum instruments & representation target & completely positive maps, POVMs/instruments, compatibility, and tensor/discarding structure\\
\bottomrule
\end{tabular}
\end{table}

\subsection{Representation targets and boundaries}

\begin{programme}[Representation and application targets]
\label{prog:monograph-targets}
The following directions are mathematically meaningful once their source-domain
semantics and morphisms are fixed.
\begin{enumerate}[label=(\roman*)]
\item Extensions of the finite verifier-game representation theorem to richer
classes of represented games and operational systems.
\item Functorial probabilistic and quantum variants using affine or process-theoretic
assignment categories, with physical realisability separated from set-theoretic
existence.
\item Topological impossibility placements for protocol-complex tasks, building on
the section representation theorem by adding the process model, carrier theorem,
and protocol legality.
\item Decentralised-control and synthesis presentations with observation histories
as contexts, nonanticipation as structural admissibility, and controller
languages as implementation images.
\item Further Gödel-style diagonal placements beyond theoremhood, formulated as
reductions from candidate deciders with explicit distinctions between truth,
consistency, soundness, and internal representability.
\item Process-theoretic extensions of the finite-dimensional no-cloning and
no-broadcasting placements, with physical channels forming an implementation or
physical-realisation layer rather than a local fixed-point contradiction.
\item Reverse-mathematical and Weihrauch classifications for represented causal
selection problems beyond the bounded-height examples, with the input
representation fixed in each statement.
\end{enumerate}
\end{programme}

\subsection{Structural spine theorem}

The monograph-level structure can be summarised as a single spine: sections are
functorial, diagnostics require the obstruction datum, certificates require local
incidence, and composition lifts only under explicit implementation operations.

\begin{theorem}[Structural spine]
\label{thm:structural-spine}
Within the classes and hypotheses fixed in this section:
\begin{enumerate}[label=(\roman*)]
\item admissibility-preserving natural transformations induce morphisms of nested
global-family triples;
\item base isomorphisms of presentations preserve and reflect the complete
first-failure diagnostic;
\item certificate isomorphisms transport finite local, parity, affine, holonomy,
and Farkas certificates when the relevant structure is included;
\item terminal compression is an idempotent compression of presentations onto
terminal presentations and replaces the local-success spectrum by the
set-theoretic success spectrum;
\item support packaging, blind-order normal forms, and arena patch sheaves are
functorial under their stated isomorphism notions;
\item conjunction, disjoint union, pullback, hiding, parallel composition,
sequential composition, and guarded feedback obey the section laws stated above,
with effective and bounded versions exactly when the required realiser operations
and resource recurrences are part of the doctrine.
\end{enumerate}
\end{theorem}

\begin{proof}
Clause (i) is Theorem~\ref{thm:global-family-functor-monograph}. Clause (ii) is
Theorem~\ref{thm:base-isomorphism-invariance}. Clause (iii) is
Theorem~\ref{thm:certificate-transport-monograph}. Clause (iv) is
Theorem~\ref{thm:terminal-retraction-monograph} together with
Corollary~\ref{cor:compression-spectrum}. Clause (v) combines
Theorems~\ref{thm:support-packaging-functoriality-monograph},
\ref{thm:dist-blind-groupoid-monograph}, and
\ref{thm:patch-sheaf-functoriality-monograph}. Clause (vi) is the collection of
Propositions~\ref{prop:csp-composition-monograph},
\ref{prop:pullback-hiding-monograph}, \ref{prop:parallel-capability-monograph},
\ref{prop:sequential-composition-monograph}, and
\ref{prop:guarded-feedback-monograph}.
\end{proof}

\begin{remark}
The theorem is structural rather than classificatory. It identifies the maps and
operations under which the section spaces, diagnostics, certificates, and
implementation filters are transported. It also marks the boundary of the
abstract theory: whenever a composition law mentions effective or bounded
success, the corresponding operation on realisers and its cost law must be
specified.
\end{remark}

\subsection{Finite protocol-complex section representation}
\label{subsec:protocol-complex-representation}

A finite chromatic complex can be read as a presentation of distributed views.
Let $K$ be a finite simplicial complex whose vertices are coloured by process
identities, with at most one vertex of each colour in each simplex. Let $O$ be a
finite output complex with the same colour set. A task carrier assigns to each
simplex $\sigma\in K$ a subcomplex $\Delta(\sigma)\subseteq O$ on the same set of
colours, monotone under faces.

\begin{definition}[Protocol-complex presentation]
\label{def:protocol-complex-presentation}
The context category is the face poset of $K$, with arrows from a simplex to its
faces. For a simplex $\sigma$, let $\mathcal A(\sigma)$ be the set of
colour-preserving vertex maps from $\sigma$ to $O$. Restriction is restriction of
maps to faces. The local admissibility set $\LocalAdm(\sigma)$ consists of those
maps whose image simplex lies in $\Delta(\sigma)$.
\end{definition}

\begin{theorem}[Protocol-complex section theorem]
\label{thm:protocol-complex-section}
Compatible admissible global families of the protocol-complex presentation are
in canonical bijection with chromatic simplicial maps
\[
f:K\longrightarrow O
\]
carried by $\Delta$, i.e. satisfying $f(\sigma)\subseteq\Delta(\sigma)$ for every
simplex $\sigma\in K$.
\end{theorem}

\begin{proof}
A compatible global family gives, on each vertex $v$ of $K$, an output vertex of
the same colour: take the component assigned on the singleton simplex $\{v\}$.
Compatibility forces the assignment on any simplex $\sigma$ to be the restriction
of these vertex assignments, so the vertex map extends to a chromatic simplicial
map. Local admissibility at $\sigma$ is exactly the carrier condition
$f(\sigma)\subseteq\Delta(\sigma)$. Conversely, any carried chromatic simplicial
map restricts to an admissible local assignment on each simplex, and these
restrictions are compatible on faces.
\end{proof}

\begin{proposition}[Functoriality under chromatic carrier isomorphisms]
\label{prop:protocol-complex-functoriality}
An isomorphism of finite protocol-complex tasks, consisting of chromatic
simplicial isomorphisms of input and output complexes that commute with the task
carrier, induces a certificate isomorphism of the associated presentations and
therefore preserves the existence or nonexistence of carried decision maps.
\end{proposition}

\begin{proof}
The simplicial isomorphism sends faces to faces and commutes with taking faces,
so it is an isomorphism of context categories. The output isomorphism transports
colour-preserving local maps bijectively and preserves the carrier subcomplexes
by hypothesis. Hence local admissibility, restrictions, and global sections are
transported isomorphically.
\end{proof}

\begin{remark}
Topological impossibility theorems for consensus and set agreement become
certificates for absence of carried chromatic maps only after the process model,
protocol complex, carrier map, and protocol legality have been fixed. For a
fixed finite complex, all candidate maps are finite tables; effectivity and
resource placements require a uniform protocol family or a restricted
implementation doctrine.
\end{remark}

\subsection{Gödel theoremhood selector placement}
\label{subsec:godel-theoremhood-placement}

The theoremhood placement is an effectivity obstruction, not a local
contradiction. The local choices are singletons, so a set-theoretic selector
exists. What fails is computable realisation.

\begin{definition}[Theoremhood selector]
\label{def:theoremhood-selector}
Let $T$ be a computably axiomatised theory whose theorem set
\[
\operatorname{Thm}_T=\{n:n\text{ codes a theorem of }T\}
\]
is not decidable. The theoremhood selector has observations $n\in\N$, choice set
$\{0,1\}$, and local admissibility sets
\[
\LocalAdm(n)=\{\chi_{\operatorname{Thm}_T}(n)\}.
\]
Effective policies are total computable binary functions.
\end{definition}

\begin{theorem}[Theoremhood selector has outcome $\mathsf K$]
\label{thm:theoremhood-selector-K}
For every computably axiomatised theory $T$ with undecidable theorem set, the
theoremhood selector has a set-theoretic global section and no computable global
section. Hence its diagnostic outcome is $\mathsf K$ for the computable
realisation model.
\end{theorem}

\begin{proof}
Every local set is the singleton containing the theoremhood bit, so the
characteristic function $\chi_{\operatorname{Thm}_T}$ is a set-theoretic global
section. If a computable global section existed, it would compute
$\chi_{\operatorname{Thm}_T}$ and decide the theorem set of $T$, contrary to the
hypothesis.
\end{proof}

\begin{remark}[Assumption hygiene]
Theorem~\ref{thm:theoremhood-selector-K} concerns theoremhood, not truth. It
uses an external undecidability hypothesis about the theorem set. For standard
consistent effectively axiomatised extensions of enough arithmetic, this is the
usual undecidability theorem. The local singleton predicate is not decidable in
this presentation; the primitive-recursive halting-stage construction of
Theorem~\ref{thm:AH} gives a different effective syntax when decidable structural
maps are required. Truth in the standard model requires different assumptions,
and internal representability of a candidate decider is distinct from external
correctness. Gödel, Rosser, and Tarski-style arguments should be placed only
after specifying which predicate is being selected and which consistency,
soundness, or representability assumptions are being used.
\end{remark}

\subsection{Quantum process restrictions: no-cloning and no-broadcasting}
\label{subsec:quantum-process-placement}

Quantum process restrictions require a process doctrine. In finite-dimensional
quantum theory the assignments are states or channels, while physically realised
implementations are completely positive trace-preserving maps. The diagnostic
placement depends on which data are treated as local constraints and which are
treated as implementation restrictions.

\begin{definition}[Quantum cloning presentation]
\label{def:quantum-cloning-presentation}
Let $\mathcal H$ be a finite-dimensional Hilbert space and let $S$ be a finite
set of states. A channel-level cloning requirement asks for one quantum channel
\[
\Phi:\mathcal D(\mathcal H)\to\mathcal D(\mathcal H\otimes\mathcal H)
\]
such that $\Phi(\rho)=\rho\otimes\rho$ for every $\rho\in S$.
\end{definition}

\begin{theorem}[Two-state pure no-cloning]
\label{thm:pure-no-cloning-placement}
No quantum channel exactly clones two distinct nonorthogonal pure states \cite{wootterszurek}.
\end{theorem}

\begin{proof}
Let $0<|\langle\psi,\phi\rangle|<1$ and suppose a channel clones both pure
states. Choose a Stinespring isometry for the channel. Since the required output
on each input is pure, the environment must factor, so for some unit vectors
$e_\psi,e_\phi$,
\[
V\psi=\psi\otimes\psi\otimes e_\psi,
\qquad
V\phi=\phi\otimes\phi\otimes e_\phi.
\]
Taking inner products and using that $V$ is an isometry gives
\[
\langle\psi,
\phi\rangle
=\langle\psi,
\phi\rangle^2\langle e_\psi,e_\phi\rangle.
\]
Taking absolute values yields $c\le c^2$ for $c=|\langle\psi,\phi\rangle|$, a
contradiction when $0<c<1$.
\end{proof}

\begin{proposition}[Two ECD placements of no-cloning]
\label{prop:no-cloning-placements}
The no-cloning obstruction has at least two natural ECD presentations.
\begin{enumerate}[label=(\roman*)]
\item If arbitrary set maps on states are allowed at the set level and quantum
channels form the physical or effective implementation image, then exact cloning
fails at that implementation layer.
\item If the assignment space consists of quantum channels and separate local
contexts require cloning of individual states, then each one-state local context
is inhabited, but the nonorthogonal two-state family has no common global channel;
this is a compatibility presentation.
\end{enumerate}
\end{proposition}

\begin{proof}
For (i), the set map $\rho\mapsto\rho\otimes\rho$ exists, but
Theorem~\ref{thm:pure-no-cloning-placement} excludes its realisation by a quantum
channel on a nonorthogonal pair. For (ii), a single state $\rho$ is cloned by the
constant channel $\sigma\mapsto\rho\otimes\rho$, so every one-state local context
is inhabited. A global section is one channel satisfying all state constraints;
Theorem~\ref{thm:pure-no-cloning-placement} says no such section exists for two
nonorthogonal pure states.
\end{proof}

\begin{theorem}[No-broadcasting placement]
\label{thm:no-broadcasting-placement}
In finite-dimensional quantum theory, a family of density matrices is exactly
broadcastable by one channel if and only if the density matrices commute
pairwise \cite{nobroadcasting}. Thus noncommuting families give the same kind of
implementation or compatibility obstruction, depending on whether physical
channels are placed in the implementation layer or in the assignment space of a
common-channel presentation.
\end{theorem}

\begin{proof}
The equivalence is the standard no-broadcasting theorem. The ECD placement then
follows as in Proposition~\ref{prop:no-cloning-placements}: either a
set-theoretic broadcasting map is compared with the image of physical channels,
or local single-state broadcasting requirements are intersected through one
common channel variable.
\end{proof}

\begin{remark}
These placements do not identify no-cloning with recursion-theoretic diagonal
failure. They identify which layer changes when the presentation changes. A
separate physical stratum
$\Gamma_{\mathrm{set}}\supseteq\Gamma_{\mathrm{phys}}\supseteq
\Gamma_{\mathrm{eff}}\supseteq\Gamma_{\mathcal B}$
can be introduced when physical realisability should be distinguished from
computability.
\end{remark}

\subsection{Topological-order development principle}
\label{subsec:topological-development-principle}

The extension programme is governed by a simple priority rule. Dependency order
is a constraint: no module should assert an invariance, comparison, complexity
classification, or empirical advantage before the source doctrine on which that
claim depends has been fixed. Feasibility then orders the remaining work: among
claims whose prerequisites are available, the preferred next claim is the one
with the shortest path to a checkable theorem, certificate, or reproducible
computation.

In practice this gives the following topological layers.
\begin{enumerate}[label=(\roman*)]
\item \emph{Source doctrine.} Specify the objects under discussion: arenas,
empirical models, protocol complexes, quantum channels, represented spaces,
finite mechanical discretisations, or resource monoids.
\item \emph{Presentation.} Define the local data, interfaces, covers, and global
section space before assigning an ECD diagnostic label.
\item \emph{Morphisms and invariants.} State functoriality, transport, or
compression only after the direction of maps and the preserved data are fixed.
\item \emph{Certificates.} Every obstruction or repair claim should name the
certificate transported or verified by the theorem.
\item \emph{Complexity and effectivity.} Complexity claims require an input
representation; computability claims require represented spaces or indices;
resource claims require an ordered cost doctrine.
\item \emph{Empirical performance.} Practical advantage claims require an
artifact, baselines, benchmark instances, and a rule separating certified
success, certified failure, and inconclusive computation.
\end{enumerate}
This order is not merely expository. It prevents a presentation-relative theorem
from being read as an invariant of a bare capability problem, and it prevents a
certificate theorem from being promoted to a performance claim before the
corresponding computations have been carried out.

\subsection{Dependency ledger for remaining foundational modules}
\label{subsec:foundational-dependency-ledger}

The following ledger fixes where the remaining foundational items may safely be
developed. It is ordered by dependency first and feasibility second.
\begin{center}
\begin{tabular}{@{}p{0.20\linewidth}p{0.31\linewidth}p{0.34\linewidth}@{}}
\toprule
Module & Missing prerequisite & Safe next claim\\
\midrule
Repeated-move games & Player ownership, chance laws, utilities, and mixed or
behavioural strategy semantics & Keep
Theorem~\ref{thm:action-recall-equivalence} as a pure and strategic-mixed
capability theorem; develop Kuhn-style comparisons only in a game-theoretic
source doctrine.\\
Noninvertible arena simulations & Direction of simulation, preservation of
nature maps, information, choices, and losses & First define simulation
morphisms; only then ask whether causal-patch sheaves preserve or reflect
global capability.\\
Empirical reindexing & Chosen class of noninvertible maps and whether support,
possibility, or probability data are primary & Use the already proved direct
image, inverse image, and saturation operations as basic constructors; broader
functoriality must state which global-support property is preserved.\\
Protocol complexes & Chromatic complexes, carrier maps, task legality, and
process-model implementation layer & Prove only the representation theorem that
carried chromatic decision maps are global sections; impossibility theorems
require the process model.\\
Measurable policies and kernels & Measurable spaces, admissible kernels, loss
measurability, and selection theorem hypotheses & State measurable descent only
after the sigma-algebras and selection doctrine are fixed.\\
Quantum process theories & State spaces, channels, tensor product, discarding,
and exact or approximate success equation & Retain finite no-cloning and
no-broadcasting placements; add broader process restrictions only as
source-domain theorems.\\
Resource calculus & Ordered resource monoid, attainment convention, and
composition law for realisers & Apply the existing minimax and overhead lemmas
only after resource addition, hiding, and parallel composition are specified.\\
Infinite arenas & Representation of branching, strategies, termination, and
limit payoff & Keep bounded-height WKL results; do not assert an infinite-arena
hierarchy before the representation is fixed.\\
\bottomrule
\end{tabular}
\end{center}

This ledger also determines placement. General source-domain entries remain in
this foundational monograph. Finite explicit encodings, proof objects, width
algorithms, and mechanical relation generation belong to the companion
algorithmic monograph (version~12, with its artifact appendix, version~9).
A module may move from this ledger into the theorem body
only when its prerequisite column has become part of the formal source doctrine.

\subsection{Finite-algorithmic handoff}
\label{subsec:finite-algorithmic-handoff}

The finite algorithmic theory uses the same section language but has a different
source doctrine. Once a finite instance is declared by variables, finite domains,
explicit relation tables, observation maps, implementation catalogues, costs,
and budgets, the following statements become companion-theorem rather than
foundational-theorem obligations.
\begin{enumerate}[label=(\roman*)]
\item A proposed global section is verified by checking each finite factor.
\item A one-shot observation fibre fails exactly when the intersection of its
scenario-admissible action sets is empty; a conflict certificate has at most one
scenario for each action.
\item Finite history-tree safety compiles to forbidden tuples by adding one
forbidden tuple for each reachable unsafe path and discarding paths inconsistent
with observation-uniformity.
\item Explicit-table complexity statements are relative to explicit relation
encoding; succinct generators, circuits, or finite-element preprocessors are
separate source representations.
\item Width and weighted-width algorithms are certificate-producing algorithms
for the finite source doctrine, not new presentation-invariant classifications of
bare decision problems.
\end{enumerate}

This handoff is an application of presentation relativity. The foundational
monograph supplies the invariants and transport vocabulary; the companion
algorithmic monograph supplies the input format, verifier grammar, algorithms,
and mechanics front end. In particular, a finite solver returning a timeout supplies no ECD
classification unless a checked certificate is also returned.

\begin{proposition}[Finite certificate conservativity]
\label{prop:finite-certificate-conservativity}
Let a finite companion instance be translated into the section presentation whose
variables are the declared policy variables and whose local constraints are the
declared finite factors. Any verified policy certificate is a global section of
that presentation. Any verified factor-wise refutation certificate for the whole
finite table instance is an obstruction to such a global section. These
conclusions are relative to the declared finite source representation.
\end{proposition}

\begin{proof}
A policy certificate assigns one declared domain element to each variable and is
verified by testing that every factor sees an allowed tuple. This is precisely
the definition of a global section of the associated finite presentation. A
refutation certificate is required, by its verifier, to prove that every total
assignment violates at least one declared factor, either directly or through a
sound dynamic-programming, join-tree, or exhaustive certificate. Hence no global
section exists in the declared finite presentation. The final sentence records
that changing the source representation, for example by replacing exact tables
with conservative finite-element inner or outer tables, changes the presentation
being certified.
\end{proof}

\begin{remark}[Certificates as verifier-game strategies]
\label{rem:certificate-verifier-game}
Theorem~\ref{thm:finite-verifier-game-representation} and
Proposition~\ref{prop:finite-certificate-conservativity} carry one shared
reading. A verified policy certificate is exactly a winning pure strategy
in the tabular verifier game of the cited theorem: the certificate
prescribes the strategy, verification is play acceptance, and the checked
factorwise tests are the verifier's accepting worlds. A verified
refutation certificate records that no pure strategy wins --- every
candidate loses some verifier test. The status ``inconclusive'' records
that no winning strategy for either side was produced within the declared
limits; it is the absence of a game-outcome certificate, not a third
outcome. The companion's typed certificate grammar and four-status
discipline make this reading constructive, and it explains the handoff
discipline above: an unfinished play decides nothing, so a bare solver
timeout supplies no ECD classification.
\end{remark}

\begin{proposition}[Inner--outer finite-model handoff]
\label{prop:inner-outer-finite-handoff}
Suppose a source-domain computation supplies finite relations
$R_i^-\subseteq R_i^{\mathrm{true}}\subseteq R_i^+$ for every declared factor
scope. Let $P^-$, $P^{\mathrm{true}}$, and $P^+$ be the corresponding finite
presentations. Then
\[
\Gamma(P^-)
\subseteq
...
\Gamma(P^+).
\]
Consequently an inner-model policy is a certified policy for the true finite
model, while an outer-model refutation is a certified refutation for the true
finite model. The mixed case $\Gamma(P^-)=\varnothing$ and
$\Gamma(P^+)\ne\varnothing$ is inconclusive.
\end{proposition}

\begin{proof}
The inclusions are factorwise. If an assignment satisfies each $R_i^-$, then the
same local row lies in $R_i^{\mathrm{true}}$ for every factor, so the assignment
is a true finite-model section. If an assignment is a true finite-model section,
each local row lies in $R_i^+$, so it is an outer-model section. The two
certificate consequences are the contrapositive directions appropriate to the
inclusions. The final case is inconclusive because the inclusions give neither a
true section nor a true refutation.
\end{proof}
The companion algorithmic monograph proves the mechanics front-end instance of
this proposition, with certified finite rows in place of the relations
$R_i^-$, $R_i^{\mathrm{true}}$, and $R_i^+$, as its inner--outer soundness
proposition.

\begin{remark}[Mechanics boundary]
The preceding proposition is a finite-model statement. In a mechanics source
doctrine, residual or interval computations may certify rows of a discrete
algebraic model. They do not by themselves certify continuum modelling adequacy,
mesh convergence, or unmodelled uncertainty. Those require additional
source-domain hypotheses before the finite certificate can be transported beyond
the declared finite model.
\end{remark}

\begin{proposition}[Artifact status separation]
\label{prop:artifact-status-separation}
For a finite computational implementation of a declared ECD presentation, the
statuses ``certified satisfiable'', ``certified unsatisfiable'', ``certified
optimal'', and ``inconclusive'' are logically distinct. Only the first three
transport to mathematical claims about the declared finite presentation. The
status ``inconclusive'', including timeout, failed certificate verification, or
unresolved numerical rows, transports no existence or nonexistence claim.
\end{proposition}

\begin{proof}
A certified satisfiable status includes a checked section; a certified
unsatisfiable status includes a checked refutation; and a certified optimal
status includes both a feasible object and a checked proof that no better object
exists in the declared finite search space. These are precisely the data needed
to assert the corresponding finite mathematical claim. An inconclusive status is
the absence of such data. Since both satisfiable and unsatisfiable instances may
lead to timeouts, failed searches, or unresolved numerical rows, inconclusiveness
has no sound implication in either direction.
\end{proof}

\begin{remark}[Baseline comparisons]
A solver comparison is an empirical claim about implementations and encodings,
not a theorem of effective causal descent. Such a comparison becomes meaningful
only after the finite instances, baseline encodings, hardware, resource limits,
and certificate-verification criteria are fixed. Without these data, the
foundational theory supports certificate transport and width-sensitive algorithms
but not a claim of practical superiority over SAT, CP-SAT, mixed-integer
linear programming (MILP), or other baselines.
\end{remark}

\subsection{Further development modules}

The remaining modules require additional source-domain doctrines rather than only
abstract section data.

\paragraph{Protocol complexes.}
A protocol-complex chapter should fix chromatic complexes, carrier maps, and task
legality. The target theorem is that carried chromatic decision maps are exactly
global sections of the associated presentation. Consensus and set-agreement
impossibilities can then be placed only after the process model and protocol
legality are included in the implementation layer.

\paragraph{Quantum process restrictions.}
The finite-dimensional quantum placement above treats exact no-cloning and
no-broadcasting. Broader process theories require the same ingredients to be
specified explicitly: states, channels, tensor product, discarding, and a success
equation. A separate physical-realisation stratum
$\Gamma_{\mathrm{phys}}$ may be appropriate between $\Gamma_{\mathrm{set}}$ and
$\Gamma_{\mathrm{eff}}$.

\paragraph{Logical diagonalisation.}
The theoremhood selector gives a scoped $\mathsf K$ placement. Further
Gödel-style placements should distinguish theoremhood from truth, consistency
from soundness, and external correctness from internal representability. The safe
form is a reduction theorem from a candidate total decider satisfying the stated
representability and correctness assumptions.

\paragraph{Reverse mathematics and Weihrauch degrees.}
The existing WKL results give exact degrees for binary fibre selection and nested
prefix selection. Broader arena and patch classes require fixed input
representations, cover locators, and validity encodings before exact degrees can
be stated.

\paragraph{General minimax and resource calculus.}
The minimax tower and resource-overhead laws above apply once a loss doctrine,
attainment convention, ordered resource monoid, and realiser operations are
specified. Further domain-specific work is needed for additive physical
decompositions, amortised costs, parallel repetition, and memory-sensitive
resource laws.

\begin{remark}
The results in this section develop the accompanying algorithmic programme at
the level of specified isomorphisms, fixed-base morphisms, and compositional operations with
explicit hypotheses. Broader noninvertible simulations, measurable or quantum
representation theorems, and resource-specific parallel laws remain
source-doctrine questions rather than consequences of the abstract framework
alone.
\end{remark}

\section{Conjectural extensions}\label{sec:conjectures}

The conjectures below are development targets, not asserted consequences of the
preceding theorems. They are to be read under the topological-order principle of
Subsection~\ref{subsec:topological-development-principle}: first fix the source
doctrine and morphisms, then prove preservation or reflection, and only then add
complexity, effectivity, or performance claims.
...
Beyond the support-isomorphism theorem of
Theorem~\ref{thm:support-packaging-functoriality-monograph}, specify useful
noninvertible morphisms of finite empirical support models and constrained
diagrams for which downward-closed packaging is functorial and preserves or
reflects the intended global-support criterion.
\end{conjecture}

\begin{conjecture}[Arena functoriality]\label{conj:arena-functor}
Beyond the arena-isomorphism theorem of
Theorem~\ref{thm:patch-sheaf-functoriality-monograph}, identify noninvertible
arena simulations that preserve nature dynamics, predictor information, choices,
and losses in a direction strong enough to induce functorial maps of causal-patch
sheaves and to reflect or preserve global capability.
\end{conjecture}
...
computable. The common content of these results is that the location of failure is part of the
...
A final tier records what survives change of presentation. Capability,
diagnostic, and certificate morphisms separate the three levels of
comparison; terminal compression is an idempotent functor erasing precisely
the $\mathsf D$-gap; distributed and one-shot arenas agree up to groupoid
equivalence; and finite tabular verifier games identify certificates and
refutations with winning strategies, so that an unfinished play, like a bare
solver timeout, transports no claim. Source-domain placements follow a
dependency ledger under the topological-order development principle:
dependency order is a constraint, and feasibility orders the remaining work.
Under that principle the finite algorithmic theory is delegated rather than
duplicated: the companion algorithmic monograph (version~14, with its
artifact appendix, version~9) supplies the input format, typed certificate
verifiers, width-based dynamic programs, and the recorded artifact.
...
The formal artifact associated with this monograph contains a 36-part reproduction
...
does not verify the modelling adequacy of the monograph's additional
constructions.

\paragraph*{Artifact availability.}
The Lean and Mathlib sources described above---the twenty-four core Lean files,
\texttt{theorem\_D\_tri.lean}, the separate \texttt{research/mathlib/k\_game.lean}
project, and the twelve deterministic Python experiments, together with the
recorded Lean~4.33.0 and Python~3.13 environment---did not persist and are not
part of the repository deposit. The gate was executed and its checks passed
when the results were verified: the listed formal statements are therefore
machine-checked, but the verifying sources are no longer available for
independent re-execution or audit. The reproduction entry point available in
the present repository, \texttt{https://github.com/\allowbreak MIKEAA2020/\allowbreak halting-physics},
is the companion artifact's \texttt{make reproduce}
gate, the finite-ECD prototype of the companion algorithmic monograph (harness
Python~3.12.14); it is a separate artifact, verifying the finite feasibility
pipeline and its certificates, and does not re-run the Lean checks. The proofs
in the text are self-contained and do not depend on the lost sources.
...
\emph{Journal of the American Statistical Association} \textbf{58} (1963),
13--30.
...
J.~W. Thatcher, editors, \emph{Complexity of Computer Computations}, pages
...
\bibitem{konig}
D.~K\"onig. \"Uber eine Schlussweise aus dem Endlichen ins Unendliche.
\emph{Acta Litterarum ac Scientiarum (Szeged)} \textbf{3} (1927), 121--130.
...
\bibitem{popescu}
S.~Popescu and D.~Rohrlich. Quantum nonlocality as an axiom. \emph{Foundations
of Physics} \textbf{24} (1994), 379--385.
...
\bibitem{simpson}
S.~G. Simpson. \emph{Subsystems of Second Order Arithmetic}. Second edition.
...
\emph{SIAM Journal on Computing} \textbf{8} (1979), 410--421.
...
\bibitem{kuhn}
H.~W. Kuhn. Extensive games and the problem of information. In H.~W. Kuhn and
A.~W. Tucker, editors, \emph{Contributions to the Theory of Games II},
Annals of Mathematics Studies 28, pages 193--216. Princeton University Press,
1953.
...
\bibitem{witsenhausen71}
H.~S. Witsenhausen. On information structures, feedback and causality.
\emph{SIAM Journal on Control}, 9(2):149--160, 1971.

\bibitem{papadimitrioutsitsiklis86}
C.~H. Papadimitriou and J.~N. Tsitsiklis. Intractable problems in control
theory. \emph{SIAM Journal on Control and Optimization}, 24(4):639--654,
1986.

\bibitem{bernstein02}
D.~S. Bernstein, R.~Givan, N.~Immerman, and S.~Zilberstein. The complexity
of decentralized control of Markov decision processes. \emph{Mathematics of
Operations Research}, 27(4):819--840, 2002.

\bibitem{contextualfraction}
S.~Abramsky, R.~S.~Barbosa, and S.~Mansfield. The contextual fraction as a
measure of contextuality. \emph{Physical Review Letters} \textbf{119} (2017),
050504.

\bibitem{wootterszurek}
...
Noncommuting mixed states cannot be broadcast. \emph{Physical Review Letters}
\textbf{76} (1996), 2818--2821.
...
\bibitem{tovey}
C.~A. Tovey. A simplified NP-complete satisfiability problem. \emph{Discrete
Applied Mathematics} \textbf{8} (1984), 85--89.
