...
all worlds with the same realised context have the same realised scope. The local
...
\(r=v_c p+(1-v_c)q\in\mathcal L\) with \(v_c<1\). The separating hyperplane
theorem supplies a supporting Bell functional \(\beta\) at \(r\) whose local bound
is attained at \(r\) and whose value increases in the direction from \(q\) to
\(p\). For this \(\beta\), the displayed ratio is \(v_c\), so the upper bound is
tight.
\end{proof}
...
of \(\log |M|\) among exact worst-case one-way simulations from the first party
to the second, with \(\kappa_{\mathrm{comm}}(p)=+\infty\) if none exists. The
following hold:
\begin{enumerate}[label=(\roman*)]
\item \(p\) has a zero-bit communication repair if and only if \(p\) is local;
...
\begin{proof}
A zero-bit communication repair is exactly a local hidden-variable model, giving (i), and
(ii) follows immediately. For (iii), sample \(a\) from the marginal
...
\begin{enumerate}[label=(\roman*)]
\item $\mathsf{WKL}_0$;
\item every binary nested-prefix presentation with a nonempty local set at every
level has a global section;
...
nested-prefix presentation with local set $T\cap2^n$ at level $n$. A global
...
produces the same configuration over $U\cup\{\tau,\sigma\}$. Every maximal path
...
sequentialisation typically fails perfect recall. Proposition~\ref{prop:operational-perfect-recall}
identifies the exceptional cases where the observation label decodes the whole
preceding information/action sequence.
\end{remark}
...
observations, local nonemptiness implies a global family in the ambient ZFC
foundation, so there is no $\mathsf D_\eps$ outcome in that presentation.
\end{theorem}
...
If presentations $P$ and $Q$ have solution maps to a shared interface
presentation $I$ and the pullback presentation is formed semantically, then
\[
...
\]
effective implementations of $P$ are translated to effective implementations of
$Q$, and implementations of budget $b$ in $P$ are translated to implementations
of budget $H(b)$ in $Q$. Then
\[
