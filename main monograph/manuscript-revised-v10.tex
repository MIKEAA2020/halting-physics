...
\begin{abstract}
This article studies a basic constraint on prediction under limited observation:
worlds with the same observable state must receive the same response. From this
constraint arise four presentation-relative diagnostics: objectwise local
validity, compatibility of a declared diagram, effective realisability, and
bounded realisability. These diagnostics are not invariants of bare capability
predicates; the analysis tracks the additional local-incidence data needed to
preserve them under translation. The static kernel is a fibrewise selector
lemma. Classical
...
loss. Global information recall makes pure action recall capability-redundant,
and acyclic record congruences support quotient backward induction. A countable
height-two arena realises the same DNR class and PA-degree spectrum as the static
effectivity example. Machine-checked artifacts cover the identified discrete and combinatorial
cores.
...
A predictor that observes only an interface cannot choose separately for each
underlying world. Worlds that produce the same observation must receive the same
output. This elementary uniformity requirement is independent of whether the
predictor is a program, a physical device, or an embedded agent. Once this
requirement is imposed, the yes--no question of capability separates into more
informative questions. A choice may fail already on a single observation fibre;
locally valid choices may fail to agree across contexts; a compatible family may
exist but not be computable; and a computable family may exceed a declared
resource bound. Effective causal descent (ECD) keeps these failures distinct
within a declared typed presentation. The diagnostic labels classify
presentations or packagings of a problem, not bare capability predicates
independently of their local incidence data.

The guiding question is therefore not only whether some successful policy exists,
but where a failure first appears in the chosen presentation. A failure may occur
inside one observation fibre, in the attempt to glue locally valid data across
overlapping contexts, in the passage from set-theoretic sections to computable
realisations, or in the imposition of a resource bound. The same capability
predicate can be packaged in more than one way; the diagnostic records the
chosen local-to-global account.

A simple quality-control task gives the intended reading. A visible label is an
observable state; the hidden condition of the item is the world; and a policy
must assign the same decision to all items with the same label. If one label
contains both acceptable and defective hidden cases, the failure is local. If
several locally consistent inspection tables disagree on shared attributes, the
failure is one of compatibility. If a perfect table exists but encodes a
noncomputable set, the failure is effective. If it is computable but outside the
allowed time or memory budget, the failure is resource-bounded.

Two examples serve as informal anchors for the formal definitions. In the
single-observer case, an inspector sees only the exterior record of an item and
must choose a report--action pair. In the distributed case, several inspectors
see different attributes of the same item and their local rules must be
consistent where their views overlap. These examples are not additional
assumptions; they are finite instances of the typed structures defined below.
...
validity relation, and a policy class. Its basic lemma says that a policy
succeeds exactly when, at each realised observation, it chooses an option valid
for every world behind that observation. Thus a global-looking condition over
worlds is converted into a family of fibrewise common-choice conditions. The
prediction-game instantiation then generates validity from dynamics, disclosure,
actions, a query, a decoder, and a loss, separating the logical kernel from the
modelling data.
...
the subfunctorial sense used in Definition~\ref{def:Av}. Set-theoretic,
effective, and resource-admissible global families are subsets of one ambient
family space. The descent kernel contains ordinary restriction agreement;
arbitrary relational compatibility belongs to separate constraint extensions.
...
holonomy fixed points. Bell coupling infeasibility has a Farkas dual certificate. Finite source
integration provides a tabular application, and exact counting of global
integrations is $\#\mathrm P$-complete. The obstruction datum records the minimal
extensional information needed to transport the diagnostic outcome: objectwise
local success and the three nested global spaces. Terminal-category compression proves that the $\mathsf L/\mathsf D$
...
The minimal selector lemma is noncausal. The causal tier starts
...
relations, where acyclicity and join trees explain when separator-consistent
local tables amalgamate into one global table \cite{beerifagin}. The contextual
packaging follows the sheaf-theoretic account of empirical models by Abramsky and
Brandenburger, in which measurement-context data are local sections and
contextuality is detected by the absence of a compatible global assignment
\cite{abramsky}. The Bell result is a finite deterministic-strategy/coupling
characterisation: a local model is a probability measure over deterministic
response tables with the prescribed setting marginals. Fine's theorem is the
...
The computability arguments use standard admissible numberings and index-set
methods \cite{kleene,rogers,soare}. Their interpretation as selection from
represented multivalued maps is related to computable analysis and effective
choice \cite{weihrauch}; in that setting, Weihrauch reducibility compares the
uniform computational content of mathematical existence principles rather than a
single fixed instance. The nonempty co-c.e. selector class is related to the
theory of $\Pi^0_1$ classes without computable members \cite{jockuschsoare}.
The least-fixed-point background for unguarded feedback is standard domain
theory \cite{abramskyjung}. The finite causal core uses the standard
extensive-form distinction between nature moves, decision nodes, and contingent
strategies \cite{osbornerubinstein}; the countable example has uniformly bounded
play length. The finite-poset protocol semantics is related to standard
partial-order models of concurrency, including event structures and Mazurkiewicz
traces; the role here is the loss-preserving translation from universally
executed cut posets to imperfect-information arenas. Past factorisation is
deliberately weaker than a structural causal model; comparison with structural
causal semantics therefore concerns possible extensions rather than an
identification \cite{pearl}.

The contextuality, Bell, compactness, join-tree, affine, DNR, WKL, and
backward-induction statements are standard in their native settings. Their role
in this article is to supply structured reference cases for one presentation-relative
selector formalism, together with explicit recursive, degree-theoretic, and
resource-separation results. This makes clear which distinctions are preserved
by translations and which depend on local incidence data.
...
The minimal kernel contains no causality, computation, probability, or resource
bound. It records only the fact that policies are functions of observations. If
several worlds induce the same observation, a policy has only one opportunity to
choose for all of them.
...
For an observable state $z$, the set $\Adm_V(z)$ contains exactly the choices
that survive all hidden variation behind that observation. If $\Adm_V(z)$ is
empty, no observation-based policy can be correct on the whole fibre
$I^{-1}(z)$. If all such sets are nonempty, the remaining question is whether the
allowed policy class can choose from them uniformly.
\end{remark}
...
quantifier by the fibres of $I$. It replaces the demand that a policy be correct
at every world by the demand that it select from the common-choice set
$\Adm_V(z)$ at every realised observation $z$. All subsequent obstruction and
computability results use these common-choice sets as the basic local objects.
\end{remark}

\section{Prediction-game instantiation}\label{sec:games}

The selector kernel treats validity as primitive. A prediction game supplies the
modelling data from which validity is computed: a report is disclosed, an action
is taken, the world evolves to a history, and a query of that history is compared
...
The closed-loop policy first reports $r=\pi_R(z)$ and then acts by
$a=\pi_A(z,r)$. The report may affect the world through the disclosure map, and
the action may affect the queried history through the dynamics. Exactness says
that zero loss means agreement between the decoded report and the queried value;
positive or infinite losses encode quantitative or hard failures.
\end{remark}
...
In a passive game the action does not change the target. Exact prediction is
therefore possible precisely when the target value is already determined by the
observation, up to the decoder. Two worlds with the same observation and
different target values form the basic information-theoretic obstruction.
\end{remark}
...
The display map packages the admissible choices over each observation as a
family of fibres. Capability asks for a section of this family that is realised
by the declared policy class. This is the single-observer prototype for the
multi-context gluing problems introduced below.
\end{remark}
...
worldwise validity sets. This gives a concrete certificate language: a fibre is
locally impossible when some family of worlds behind the same observation has no
choice valid for all of them. Compactness and convexity yield finite
certificates; past factorisation and finite propagation then supply causal
specialisations.
...
The static contextual tier treats a local assignment as a partial description of
a possible global policy. Compatibility means that two such descriptions give
the same value on their common overlap. Relational CSP constraints can be added
as a separate extension, but the descent kernel itself uses ordinary restriction
agreement. Arrows are oriented from larger contexts to smaller contexts, so
restriction is covariant; reversing all arrows gives the usual presheaf
convention.
...
when all restriction comparisons agree. The diagnostic distinction between
$\mathsf L$ and $\mathsf D$ is therefore the distinction between an empty local
choice set and a nonempty collection of local choice sets that cannot be glued.
\end{remark}
...
This section defines the diagnostic outcomes and the data that preserve them.
The first question is whether every declared local object has at least one
admissible element. If so, the next question is whether those local elements glue
to a compatible global family. The remaining questions ask whether such a family
lies in the effective and bounded realisation images. The local-success bit
therefore separates local failure from compatibility failure, while the nested
global spaces separate set-theoretic, effective, and bounded realisation.
...
The obstruction datum $\mathfrak O_\eps$ is sufficient for transporting the
five diagnostic outcomes. Transporting certificates requires additional
structure, such as the object set, the local admissibility sets, the restriction
maps, and the incidence of local elements with compatible global families. Thus
minimal diagnostic transport and certificate transport are distinct requirements.
\end{remark}
...
\begin{remark}
The compression boundary identifies the compression-invariant part of the
spectrum as $(T_{\mathsf D},T_{\mathsf K},T_{\mathsf R})$. The
compression-sensitive part is $G_{\mathsf D}$, the tolerance region where local
validity holds but compatible gluing fails. Recovering that distinction requires
local incidence and admissibility data in addition to the nested global-family
spaces.
\end{remark}
...
Distributed observation changes the problem by reusing the same local response
across several overlapping scopes. The obstruction is no longer that one fibre
has no admissible value; it is that locally admissible responses cannot be chosen
so as to agree on shared observations.
...
In the distributed setting, an agent's policy is reused at every world producing
the same local observation for that agent. A world relation then tests a tuple of
such local responses. The obstruction is produced by reuse: local response rules
that work on individual scopes may force contradictory values on a shared
...
This section compares finite situations in which local data fail to glue with
situations in which gluing is guaranteed. Parity and support contextuality give
relational obstructions. Join trees give an acyclic positive theorem. Affine and
bijective networks admit complete specialised certificates. Bell locality becomes
a convex coupling problem: the local probability tables must be marginals of one
probability measure on complete response tables.
...
The Bell presentation replaces a deterministic global assignment by a probability
measure on deterministic response tables. The local data are the observed
setting-indexed distributions; compatibility asks whether they are all marginals
of one measure on complete response tables.
...
realisation image is a class of computable maps. The relevant distinction is
between a nonempty set of valid sections and the existence of a uniform procedure
or implementation selecting one of them.
...
The first effectivity example has no local or compatibility failure. A valid
selector exists set-theoretically, but any computable candidate is defeated on
its own index. The construction is the standard diagonally nonrecursive pattern
written as a selector game.
...
The causal tier fixes the arena before a strategy is chosen. In
perfect-information arenas, backward induction characterises capability because
each reached decision node may be solved using the information available at that
node. Compatibility obstructions arise only after choices are shared across
...
A patch contains an initial segment of the decision tree, and a section over that
patch is a partial contingent plan. The sheaf condition says that partial plans
which agree on their common histories glue uniquely to a plan on the union.
Activated validity adds the convention that a loss obligation is checked only at
nodes actually reached by the partial plan.
\end{remark}
...
Active sensing separates two ways of reporting the same incapability. The full
patch site includes a decision skeleton whose validity set is empty. A branch
cover omits that global object; its branch-local sections can be inhabited, but
they cannot be matched because matching fixes a common root action.
...
pointwise computability of every local section is weaker than uniform
computability of the local family. Theorem~\ref{thm:K-arena} is a bounded-height countable instance, not a
