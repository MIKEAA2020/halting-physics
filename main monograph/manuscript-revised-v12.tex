...
\begin{abstract}
This article studies a basic question about prediction under limited
observation: when only part of a world is visible, what does it mean to choose
correctly? The first constraint is simple. Worlds with the same observable state
must receive the same response. Once this constraint is made explicit,
capability separates into four presentation-relative diagnostics: objectwise
local validity, compatibility of a declared diagram, effective realisability,
and bounded realisability. These diagnostics are not invariants of bare
capability predicates. They record where success first fails in a chosen
local-to-global description, and the analysis tracks the local-incidence data
needed to preserve such diagnoses under translation. The static kernel is a
fibrewise selector lemma. Classical
...
measurement record, or a local transcript. The interface may discard information,
and the discarded information cannot be used by the policy. Thus two worlds that
produce the same observation must receive the same output. This small constraint
is the starting point of the paper.

The yes--no question of capability is too coarse to explain the resulting
failures. A policy can fail because one observation hides incompatible worlds. It
can fail because several locally valid rules cannot be stitched into one global
rule. It can fail because the required rule exists set-theoretically but not
computably. It can fail because a computable rule exceeds the declared resource
bound. Effective causal descent (ECD) keeps these failures distinct within a
declared typed presentation. The diagnostic labels classify presentations or
packagings of a problem, not bare capability predicates independently of their
...
is the observable state; the hidden condition of the box is the world. If one
label covers both acceptable and defective boxes, then no single decision works
behind that label. That is local failure. If several inspectors see different
attributes and their individually consistent tables disagree on shared
attributes, the obstruction is no longer inside one label; it lies in the attempt
to combine the tables. That is compatibility failure. If the perfect inspection
table is the graph of a noncomputable set, the obstruction is effective. If the
table is computable but cannot be produced within the budget, the obstruction is
resource-bounded.

The example is mnemonic rather than additional structure. The formal objects are
sets, maps, relations, diagrams, implementations, and loss functions. The four
questions are kept separate throughout: Is there a locally valid choice? Do the
local choices glue? Is a glued choice computable or otherwise realisable? Is it
...
images in the common global-family space.}
