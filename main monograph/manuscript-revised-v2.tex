...
\begin{abstract}
This article studies observation-indexed choice problems through four presentation-relative
...
invariants of bare capability predicates; the analysis tracks the additional
local-incidence data needed to preserve them under translation. The static kernel is a fibrewise selector lemma. Classical
...
diagnostic labels classify presentations or packagings of a problem,
...
The phrase ``effective causal descent'' denotes here a presentation-level
taxonomy of restriction-compatible selection problems with computability and
resource filters. It does not assert effective descent in the Barr--Beck or
Grothendieck-descent sense.
...
contingent strategies \cite{osbornerubinstein}; the countable example has
...
The contextuality, Bell, compactness, join-tree, affine, DNR, WKL, and
backward-induction statements are standard in their native settings. They are
represented in one presentation-relative selector formalism, together with
explicit recursive, degree-theoretic, and resource-separation results, so that
preserved and non-preserved diagnostic distinctions are stated with uniform
types.
...
quantifier by the fibres of $I$. Its value is that subsequent obstruction and
...
predicate is part of the descent kernel. Arrows are oriented from larger contexts
...
team problem of Corollary~\ref{cor:team-decision} has nonempty local relations but no global member, so its outcome
...
This section compares negative and positive finite compatibility criteria. Parity and
...
The cokernel class of Proposition~\ref{prop:affine-cokernel} is complete only for the stated affine subclass. No
...
The $\mathsf D$ classification in Theorem~\ref{thm:bell} is specific to the star
presentation of Definition~\ref{def:bellpack}. If the top local admissibility set
is replaced by $\operatorname{Coupl}(p)$, coupling failure is local emptiness at
that object. The invariant content is coupling feasibility, equivalently
membership in the finite local polytope, rather than the diagnostic letter after
arbitrary repackaging.
\end{remark}
...
Definition~\ref{def:Kgame} is one computable instance of this operator, whose
...
only after choices are shared across information-limited contexts; Corollary~\ref{cor:team-decision} supplies that imperfect-information case. Declared
...
\begin{remark}
A static compilation is a capability equivalence. It preserves the diagnostic
outcome only when the declared local objects, admissibility sets, and realised
global-family spaces are transported as part of the compilation data.
\end{remark}
...
to preserve the operational poset results.
\end{remark}
...
gate: twenty-four core Lean files and twelve deterministic Python experiments. The Lean artifacts check the
...
polarity are proved in the text without dedicated Lean artifacts. Object-level
