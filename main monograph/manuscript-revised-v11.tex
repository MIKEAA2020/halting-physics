...
\begin{abstract}
This article studies a basic question: when a predictor can see only part of a
world, what does it mean to choose correctly? The first constraint is simple:
worlds with the same observable state must receive the same response. Once this
constraint is made explicit, capability separates into four
presentation-relative diagnostics: objectwise local validity, compatibility of a
declared diagram, effective realisability, and bounded realisability. These
diagnostics are not invariants of bare capability predicates; the analysis
tracks the local-incidence data needed to preserve them under translation. The
static kernel is a fibrewise selector lemma. Classical
...
A predictor observes the world through an interface: a sensor, a display, a
measurement record, or a local transcript. It cannot look behind that interface.
If two worlds produce the same observation, the predictor must give them the same
output. This is the small fact from which the rest of the paper starts.

The yes--no question of capability is therefore too coarse. A policy can fail
because one observation hides incompatible worlds. It can fail because several
locally valid rules cannot be stitched together. It can fail because the required
rule exists set-theoretically but not computably. It can fail because a
computable rule exceeds the declared resource bound. Effective causal descent
(ECD) keeps these failures distinct within a declared typed presentation. The
diagnostic labels classify presentations or packagings of a problem, not bare
capability predicates independently of their local incidence data.
...
A running quality-control example fixes the intuition. An inspector sees the
outside of a box and must decide what to report and what to do. The visible label
is the observable state; the hidden condition of the box is the world. If all
boxes with the same label must receive the same decision, then a mixed fibre of
acceptable and defective boxes is a local failure. If several inspectors see
different attributes and their locally valid tables disagree on shared
attributes, the failure is a compatibility failure. If the perfect inspection
table is the graph of a noncomputable set, the failure is effective. If the table
is computable but cannot be produced within the budget, the failure is a resource
failure.

The example is only mnemonic. The formal objects are sets, maps, relations,
diagrams, implementations, and loss functions. The point of the example is to
keep the four questions separate: Is there a locally valid choice? Do local
choices glue? Is a glued choice computable or otherwise realisable? Is it
realisable within the declared bound?
...
bound. It records only one constraint: a policy is a function of what is seen.
All worlds in one observation fibre must therefore be handled by a single
choice.
...
The selector kernel treats validity as primitive. A prediction game explains
where validity comes from. The predictor observes, reports, and acts; the report
may be disclosed to the system; the world evolves; and a query of the resulting
history is compared with the decoded report.
...
The static contextual tier replaces one observation channel by many overlapping
contexts. A local assignment is a partial description of a possible global
policy. Compatibility means that two partial descriptions agree wherever they
both speak. Relational CSP constraints can be added as a separate extension, but
the descent kernel itself uses ordinary restriction agreement. Arrows are
...
Distributed observation is different. Several agents may see different slices of
the same world, and the same local response rule must be reused whenever the
same local observation reappears. The obstruction is no longer that one fibre has
no admissible value; it is that locally admissible response rules cannot be made
to agree on their shared variables.
...
Instead of asking for one deterministic global assignment, it asks for a
probability measure on deterministic response tables. The observed distributions
at each setting are the local data; locality asks whether all of them are
marginals of one global measure.
...
A compatible section may exist in principle and still be unavailable to an
implementation. Abstract effectivity is therefore relative to a realisation map.
Ordinary computability is the special case in which the domains carry effective
representations and the realisation image is a class of computable maps. The
central distinction is between nonempty valid-section space and the existence of
a uniform procedure selecting a valid section.
...
The causal tier adds time without allowing the strategy to choose the arena. The
tree, nature moves, information sets, and losses are fixed first; a strategy then
selects actions at the predictor information states. In perfect-information
arenas, backward induction characterises capability because each reached decision
node can be solved using the information available at that node. Compatibility
obstructions arise only when choices are shared across information-limited
contexts; Corollary~\ref{cor:team-decision} supplies that imperfect-information
case. Declared covers may still exhibit presentation-relative matching failure,
as active sensing shows.
...
Active sensing is the clearest finite example of presentation relativity in the
causal tier. The full patch site includes a decision skeleton whose validity set
is empty, so the failure is local in that presentation. A branch cover omits that
global object. Each branch can then be solved locally by choosing an action that
leaves the branch, but the branch solutions cannot be matched because matching
forces one common root action.
