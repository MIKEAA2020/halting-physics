...
\]
For each realised context $C\subseteq X$, let
\[
\bar R_C=\bigcap_{i:C_i=C}R_i
\]
be the aggregated relation for all sources with that context. Then global
records integrate the original sources exactly when their restrictions lie in
$\bar R_C$ for every realised context $C$.
...
For the canonical integration presentation at tolerance zero, an empty aggregated
realised-context relation $\bar R_C$ gives outcome $\mathsf L_0$; nonempty
aggregated relations with no integrating global record give outcome
...
$\bar R_C$. The three cases are then exactly the local, compatibility, and
...
following hold:
\begin{enumerate}[label=(\roman*)]
\item \(p\) has a zero-message repair if and only if \(p\) is local;
...
\begin{proof}
A zero-message repair is exactly a local hidden-variable model, giving (i), and
...
Let the effective realised image consist of the matching families $(k,k,k)$ and
$(r,r,r)$, and let the bounded realised image consist of $(r,r,r)$. Since
...
Fix a finite measurement scenario with contexts $M$ and outcome maps
$q_x:O_x\to O'_x$. For a support model $S=(S_C)_{C\in M}$, define the direct
image support model by
\[
...
information sets, choice sets, and terminal losses.
...
If presentations $P$ and $Q$ have solution maps to a shared interface $I$ and the
...
Suppose a morphism $F:P\to Q$ has tolerance overhead $\rho$ and resource overhead
$H$, meaning that
\[
F_c(\LocalAdm_P^\eps(c))\subseteq \LocalAdm_Q^{\rho(\eps)}(c)
\]
and that implementations of budget $b$ in $P$ are translated to implementations
