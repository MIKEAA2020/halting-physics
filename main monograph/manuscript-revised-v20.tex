...
The coupling theorem gives the Bell diagram a physical reading. The global object
is a hidden-variable space of complete response tables; the setting objects are
the experimentally observed marginals. A \(\mathsf D\) outcome in this
presentation means that the observed behaviour cannot be embedded in the
local-causal model without relaxing one of its assumptions.
...
local-causal presentation. The basic qualitative repair coordinates are:
...
by postselection. A quantitative repair profile records the optimal cost of such
relaxations whenever the corresponding optimisation problem is specified.
\end{definition}
...
independent of \(y\), the following hold in the exact worst-case one-way alphabet
model from the first party to the second:
\begin{enumerate}[label=(\roman*)]
...
\begin{proposition}[Detector-efficiency feasibility]
\label{prop:detector-certificate}
For every finite Bell behaviour \(p\) and fixed \(\eta\), existence of an
efficiency-\(\eta\) local extension is a finite linear feasibility problem. Hence
infeasibility has a Farkas dual certificate, an efficiency Bell inequality.
\end{proposition}
...
form a finite linear feasibility problem. Farkas' lemma gives a dual certificate
