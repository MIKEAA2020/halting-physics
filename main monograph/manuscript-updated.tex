...
\begin{abstract}
We study observation-indexed choice problems through four presentation-relative
diagnostics: objectwise local validity, compatibility of a declared diagram,
effective realisability, and bounded realisability. These diagnostics are not
invariants of bare capability predicates; we track the additional local-incidence
data needed to preserve them under translation. The static kernel is a fibrewise selector lemma. Classical
compactness, Helly, and join-tree results provide certificate and gluing tools;
parity, support contextuality, and Bell coupling give finite representations. A
recursive selector game yields the binary DNR $\Pi^0_1$ class, with capability
characterised by PA oracle degrees; uniformly selecting from negatively
represented nonempty binary fibres has Weihrauch degree $\mathsf{WKL}$.
Countably nested distributed scopes realise path selection on computable binary
trees and provide a distributed $\mathsf K$ obstruction. A concrete
primitive-recursive game language has a $\Sigma^0_3$-complete capability index
set. The causal tier begins with finite extensive-form arenas fixed independently of
strategies. Backward induction shows that perfect-information node-local
nonemptiness implies global capability. Partial strategies and activated-valid
strategies form sheaves on prefix-closed patches; active sensing distinguishes
full-site local failure from cover-relative matching failure. Distributed
selectors supply a canonical incidence presentation in which compatibility
failures can appear; the private-information XOR team relation is the
deterministic CHSH constraint. Distributed selectors are
equivalent to blind-order one-shot arenas, while every finite pure-strategy
imperfect-information arena also has an extensional distributed compilation.
Mixed strategies on the Bell one-shot skeleton realise exactly the local
behaviours, and Bell coupling failure has a Farkas dual certificate. Finite
causal-poset protocols have unique confluent executions; every linear extension
gives a policy-independent imperfect-information arena with the same operational
loss. Information recall makes pure action recall capability-redundant, and
acyclic record congruences support quotient backward induction. A countable
height-two arena realises the same DNR class and PA-degree spectrum as the static
effectivity example. Existing machine-checked artifacts verify only
the identified discrete and combinatorial cores.
...
A predictor that observes only an interface cannot generally choose separately
for each underlying world: worlds with the same observation must receive the
same output. This elementary uniformity requirement is independent of whether
the predictor is computational, physical, or embedded. Additional structure
creates further questions. Locally valid choices may fail to agree across
contexts; a compatible family may not be computable; and a computable family may
exceed a declared resource bound. The purpose of effective causal descent (ECD)
is to keep these failures distinct within a declared typed presentation. The
diagnostic labels used below classify presentations or packagings of a problem,
not bare capability predicates independently of their local incidence data.
...
The minimal selector problem consists of worlds, observable states, choices, a
validity relation, and a policy class. Its basic lemma regroups the universal
world quantifier by interface fibres. The prediction-game instantiation then
generates validity from dynamics, disclosure, actions, a query, a decoder, and a
loss, separating the logical kernel from the modelling data.

Multiple observation contexts are represented by a diagram of assignment spaces
with standard restriction agreement. Objectwise validity gives a constrained
diagram; when validity is stable under restriction, it gives a descent system in
the usual functorial sense. Set-theoretic, effective, and resource-admissible
global families are subsets of one ambient family space. Arbitrary relational
compatibility is not part of the descent kernel.

The static results include compactness and Helly bounds for local certificates,
a join-tree local-to-global theorem, a six-object parity obstruction,
support-level strong contextuality, and finite Bell coupling. Constraint-datum
isomorphisms preserve compatible families; affine systems have a complete
cokernel obstruction, and bijective constraint networks are governed by common
holonomy fixed points. Bell coupling infeasibility has a Farkas dual certificate.
The obstruction datum records the minimal extensional information needed to
transport the diagnostic outcome: objectwise local success and the three nested
global spaces. Terminal-category compression proves that the $\mathsf L/\mathsf D$
distinction cannot be recovered from global spaces alone. In the computability layer, the selector
set of the halting-stage selector game is a nonempty $\Pi^0_1$ class without
computable members. Nested countable distributed selectors give a second,
overlapping-scope $\mathsf K$ presentation. The DNR instance has the PA oracle
...
Computational-class separation is distinguished from a deterministic-time
resource lower bound. A quantitative interlude separates pointwise,
observation-uniform, effective, and bounded worst-case optima, with a
Chebyshev-radius formula in passive metric prediction.

Causality is not used by the minimal selector lemma. The causal tier starts
from a finite extensive-form arena fixed independently of the strategy. Backward
induction proves that perfect-information node-local nonemptiness already gives
a global strategy. Activated patch semantics represents contingent plans and
cover-relative matching. Section~\ref{subsec:oneshot} proves an exact normal
form for blind-order one-shot arenas and an extensional distributed compilation
for arbitrary finite pure-strategy imperfect-information arenas. On the Bell
one-shot skeleton, probability measures over pure strategies are exactly
couplings of deterministic response tables. Universally executed cuts may be
partially ordered: well-founded recursion gives one confluent operational
execution, and each linear extension yields an arena preserving its worldwise
loss. Contingent stage occurrence remains tree-indexed. A separate bounded-height
countable example transfers the halting-stage effectivity obstruction to the
causal presentation; no general infinite-arena hierarchy is inferred.

Figure~\ref{fig:architecture} summarises the three canonical presentations and the nested realisation filters.

\subsection*{Relation to established frameworks}

The phrase ``effective causal descent'' is used here for a presentation-level
taxonomy of restriction-compatible selection problems with computability and
resource filters. It is not meant to assert effective descent in the Barr--Beck
or Grothendieck-descent sense.

The fibrewise selector problem is a uniform constraint-satisfaction problem. Its
multi-context form is closely related to database joins and consistency of local
relations, where acyclicity controls global amalgamation
\cite{beerifagin}. The contextual packaging follows the sheaf-theoretic account
of empirical models and global assignments of Abramsky and Brandenburger
\cite{abramsky}. The Bell result is a finite deterministic-strategy/coupling
characterisation; Fine's theorem is the historically central result for the
standard $2\times2\times2$ scenario \cite{fine}, while the general finite
statement follows from the local polytope representation. The computability
arguments use standard admissible numberings and index-set methods
\cite{kleene,rogers,soare}. Their interpretation as selection from represented
multivalued maps is related to computable analysis and effective choice
\cite{weihrauch}; the nonempty co-c.e. selector class is related to the theory
of $\Pi^0_1$ classes without computable members \cite{jockuschsoare}. The least-fixed-point background for unguarded feedback is
standard domain theory \cite{abramskyjung}. The finite causal core uses the
standard extensive-form distinction between nature moves, decision nodes, and
contingent strategies \cite{osbornerubinstein}; the countable example later has
uniformly bounded play length. Past factorisation is deliberately
weaker than a structural causal model; comparison with structural causal
semantics therefore concerns possible extensions rather than an identification
\cite{pearl}.

The contextuality and Bell statements below are representation results rather
than new proofs in their native form. The framework places selector,
compatibility, effectivity, and resource failures in a common typed language and
adds explicit recursive, degree-theoretic, and resource-separation results.
...
Effectivity and resource bounds are imposed by nested realisation images in the
common global-family space.}
...
A selector problem consists of nonempty sets $W$ of worlds, $Z$ of observable
states, and $C$ of choices; an interface $I:W\to Z$; a validity relation
$V\subseteq W\times C$; and a policy class $\Pi\subseteq C^Z$. For
$z\in\im(I)$ define the common admissible-choice set
\[
...
\begin{remark}
Lemma~\ref{lem:selector} is a representation lemma: it regroups one universal
quantifier by the fibres of $I$. Its value is that all later obstruction and
computability results can be stated in terms of the common-choice sets
$\Adm_V(z)$.
\end{remark}
...
\]
where $W,Z,R,A,M,H,Y$ are nonempty and
\[
...
\]
The loss acts directly on a report and a query value; in closed loop it is
evaluated as $\ell(r,q(h))$. The game is called \emph{exact} when
$\ell(r,y)=0$ if and only if $e(r)=y$; this condition constrains only the zero
level of the loss. A policy is
$\pi=(\pi_R,\pi_A)$ with
\[
...
Local obstruction is first expressed as emptiness of an intersection of
worldwise validity sets. Compactness and convexity yield finite certificates;
past factorisation and finite propagation then supply causal specialisations.
...
Suppose $R\times A$ is compact and, for every world, the map
\[
(r,a)\longmapsto\ell(r,q(U(w,D(r),a)))
\]
is lower semicontinuous. Equivalently for this use, assume directly that every
sublevel set $\operatorname{Val}_\eps(w)$ is closed. Then
$\Adm_\eps(z)=\varnothing$ if and only if some finite
...
\begin{proof}
Lower semicontinuity makes each sublevel set
$\operatorname{Val}_\eps(w)$ closed. The reverse implication follows because
the full intersection is contained in the intersection over $S$. Conversely, if
...
For the prediction game whose interface is the cut interface $I_\tau$ above
...
The static contextual tier uses ordinary restriction agreement. Relational CSP
constraints can be added as a separate extension, but no arbitrary compatibility
predicate is part of the descent kernel. We orient arrows from larger contexts
to smaller contexts, so restriction is covariant; reversing all arrows gives the
usual presheaf convention.
...
declared diagram and are not invariants of a bare capability predicate. Table~\ref{tab:presentations} summarises the structural cases used later; several rows are roadmap entries whose constructions appear in Sections~\ref{sec:distributed}, \ref{sec:Dtri}, and \ref{sec:feedback}. Repackaging all constraints into one object can move a failure
between descriptions. The terminal-category construction above and the extensive-form
...
This section defines the diagnostic outcomes before stating their invariance and
compression properties. The local-success bit distinguishes local failure from
compatibility failure; the nested global spaces then separate set-theoretic,
effective, and bounded realisation.

\begin{definition}[Well-formed presentation]\label{def:levels}
For each presentation language used below, a raw presentation is a code for the
...
\begin{remark}
Theorem~\ref{thm:compression-boundary} gives a precise limit to
presentation-invariant obstruction labels. Effectivity and resource failure can
be transported by equivalences of the corresponding nested global spaces;
separating $\mathsf L$ from $\mathsf D$ additionally requires local incidence
and admissibility data.
\end{remark}
...
fixed realisation models, not an intrinsic order on physical mechanisms.
Table~\ref{tab:ladder} summarises the outcomes, certificates, and representative inhabitants; representative results are cited where they are proved below.
\end{remark}
...
A single interface generates independent fibre constraints and cannot produce an
intrinsic compatibility obstruction in ZFC. Distributed observation supplies
a canonical packaging in which compatibility obstructions arise: one response
function must serve several overlapping
world scopes.
...
problem below has nonempty local relations but no global member, so its outcome
...
between Boolean outputs and $\{\pm1\}$ outcomes. The Bell coupling theorem later
allows probability measures over the same deterministic tables. Thus the team theorem is the
...
Theorem~\ref{thm:oneshot}. Theorem~\ref{thm:finite-ii-compilation} later gives
...
strategies with parties as agents and settings as local observations. The probability-valued coupling diagram of Definition~\ref{def:bellpack} is
their convex relaxation. The join-tree theorem gives the acyclic, pairwise
projection-consistent positive case for the scope hypergraph.
...
effectivity, and resources. We separate four optima. Let
...
We next compare negative and positive finite compatibility criteria. Parity and
support contextuality provide relational obstructions, join trees provide an
acyclic gluing theorem, affine and bijective networks admit complete specialised
certificates, and Bell locality becomes a convex coupling problem.
...
proved above is internal to a fixed empirical model; functoriality of the
...
The cokernel class above is complete only for the stated affine subclass. No
complete cohomological certificate is claimed for general nonlinear contextual
relations.
...
\]
This is a constrained assignment diagram, not generally a descent subfunctor:
an arbitrary measure at $\top$ need not have the prescribed marginals. Its
...
\begin{proof}
We verify the implication from either standard formulation of locality.
...
Abstract effectivity is relative to a realisation map. Ordinary computability is
a special case in which the domains carry effective representations and the
realisation image is a class of computable maps.

\begin{definition}[Uniform binary fibre selection]
\label{def:fibsel-weihrauch}
Give a closed subset of the discrete space $2=\{0,1\}$ by negative
...
\mathsf{WKL},
\]
where $\mathsf C_X$ denotes closed choice on $X$ and the hat denotes countable
parallelisation.
\end{theorem}
...
representations or countably valued fibres. The halting-stage family introduced in
Definition~\ref{def:Kgame} below is one computable instance of this operator, whose
solution set has the PA-degree spectrum of Corollary~\ref{cor:PAdegree}; that
nonuniform degree statement does not make the fixed instance Weihrauch-complete
...
are fixed. The computability results below use standard acceptable numberings.
\end{remark}
...
For the fixed acceptable numbering $(\varphi_n)$ introduced above, let $\Pi_X$ be the policies computable relative
...
\end{enumerate}
If $T$ is computable, then $[T]$ is a nonempty $\Pi^0_1$ class. If it has no
computable member, computable strategies as the effective image give outcome
$\mathsf K_0$. For the binary DNR tree of Theorem~\ref{thm:Pi01}, an oracle
computes a global strategy exactly when it has PA degree.
\end{theorem}

\begin{proof}
Since $T$ is infinite and finitely branching, it has nodes of every finite
length; hence every $V_n$ is nonempty. A distributed strategy is a binary
...
K\H{o}nig's lemma gives $[T]\ne\varnothing$. An initial object would require
...
The preceding effective-outcome classification concerns a fixed computable tree.
The next theorem treats the tree itself as represented input, which may be
noncomputable.

\begin{theorem}[Uniform degree of nested-prefix selection]
\label{thm:nested-WKL}
Let the input be an infinite prefix-closed binary tree, represented in the
standard tree representation and promised to have a path. The multivalued
operator taking $T$ to a valid strategy of $P_T$ is Weihrauch-equivalent to
$\mathsf{WKL}$. The input need not be computable.
\end{theorem}
...
\begin{remark}
The full canonical incidence category of $P_T$ is not a chain, and adjacent
projection equality can fail when $T$ has dead ends. Thus the finite join-tree
theorem is not used. The exact Weihrauch statement concerns the input-uniform
...
sort-compatible composition and application. Invalid strings are not indices of
$\mathcal L_{\mathrm{PR}}$. Exact zero--one loss uses decidable equality. A policy is coded by a
...
\end{enumerate}
Primitive-recursive structural maps are total by syntax, and their values and
the final equality are decidable once the policy computations have converged.
A single stage code $s$ can pair all finite computation witnesses needed by the
tag. Hence a recursive predicate $R$ satisfies
\[
G\in\mathrm{CAP}_{\mathrm{PR}}
\iff
\exists p\,\forall x\,\exists s\ R(G,p,x,s),
\]
placing the index set in $\Sigma^0_3$.
...
The causal tier begins with perfect-information arenas, where backward induction
completely characterises capability. Intrinsic compatibility obstructions arise
only after choices are shared across information-limited contexts; the
team-decision example below supplies that imperfect-information case. Declared
covers may still exhibit presentation-relative matching failure, as active
sensing shows.
...
from later loss to earlier loss is assumed or needed.
\end{definition}
...
\]
and intrinsic full-site $\mathsf D_\eps$ is impossible.
\end{corollary}
...
information-set hypothesis used below. Perfect recall is automatic in the
one-shot construction of Definition~\ref{def:oneshot} but is not assumed in
...
\]
Thus the full site still has no intrinsic $\mathsf D_\eps$ outcome.
\end{corollary}
...
Let $\mathcal T$ be a finite no-absentminded arena with information recall.
For every tolerance,
\[
...
We first prove by induction on the length of the predecessor information-set
...
presentation and the full arena patch-site presentation. No assertion about
morphisms or functoriality is made.
\end{theorem}
...
capability, but not temporal locality or the original arena tree. By contrast,
...
instead represented by the branching arenas already defined above.
...
induction gives uniqueness. A linear extension merely orders these evaluations;
...
subsheaf and the full tree is initial, so intrinsic full-site
...
Adding the nature root proves the formula. The products can grow exponentially
with the number of cuts. Their partial sums can depend on the order of unequal
choice-alphabet sizes, while the operational equivalence is already proved in
Theorem~\ref{thm:operational-arena}.
\end{proof}
...
decoding map implies constancy. No claim is made merely from the existence of
the observation map.
\end{proof}
...
\begin{proposition}[Trace-relative intrinsic past]
...
Appendix~\ref{sec:domain-feedback} is a separate semantics and is not asserted
to preserve the operational results above.
\end{remark}
...
pointwise computability of every local section does not supply a uniform local
family. Theorem~\ref{thm:K-arena} is a bounded-height countable instance, not a
general complexity classification of infinite arenas. In particular, the full
patch of this arena is computable as a set of nodes but has no computable valid
section.
\end{remark}
...
\begin{enumerate}[label=(\roman*)]
\item single-observer canonical diagrams have no intrinsic $\mathsf D$;
...
why a single-observer canonical presentation has no intrinsic compatibility
...
join trees give a positive local-to-global theorem; the affine cokernel and
bijective-network fixed-point criteria give complete certificates on their
stated subclasses.
...
records instantiate past factorisation. Under information recall, pure action
recall is capability-redundant; finite acyclic record congruences permit quotient
backward induction. Finally, the height-two halting-stage arena shows that
...
\appendix
\section{Translation discipline and scope}\label{sec:translation}

\begin{definition}[Placement]\label{def:transclass}
A placement of a source object in an ECD obstruction consists of specified source
and target categories, a translation, stated preservation properties, and a
proof of the translated obstruction. In the absence of these data no placement
is asserted.
\end{definition}

\begin{definition}[Literal translation class]\label{def:T0}
A literal translation class $\mathcal T_0$ is one whose maps may use only
observer, interface, query, disclosure, dynamics, and policy data already
present in the source; missing fields may not be synthesised.
\end{definition}

\begin{remark}[Relative non-typing]\label{rem:relativeNI}
If a source object lacks a required target field and a declared translation
class forbids synthesis of that field, the object has no translation in that
class. This criterion concerns only the declared translation class; extensions
that supply additional structure constitute different translation problems.
\end{remark}

\begin{remark}[Object-level encoding schemes]\label{rem:encodings}
Finite tables can be read directly as typed games. A finite-range cellular
automaton with a fixed observer window and horizon has an extensive arena whose
nature dynamics is the automaton update and whose accessible record is bounded by
the backward light cone. A Turing machine with a designated observer tape and
step horizon admits an analogous arena. Task-specific choices, queries, and
losses must still be supplied. No functoriality or preservation theorem is
claimed.
\end{remark}
...
\section{Pointwise-validity polarity}

\begin{definition}[Pointwise polarity]\label{def:galois}
Write $w\models\pi$ when $\pi$ has zero loss at $w$. For $W'\subseteq W$ and
$\Pi'\subseteq\Pi_{\mathrm{all}}$ put
\[
\Pi^*(W')=\{\pi\in\Pi_{\mathrm{all}}:\forall w\in W',\ w\models\pi\},\qquad
W^*(\Pi')=\{w:\forall\pi\in\Pi',\ w\models\pi\}.
\]
\end{definition}

\begin{proposition}[Pointwise-validity polarity]\label{prop:galois}
The two assignments are antitone and satisfy
\[
W'\subseteq W^*(\Pi')\iff\Pi'\subseteq\Pi^*(W').
\]
Their composites
\[
W^*\Pi^*:\mathcal P(W)\to\mathcal P(W),
\qquad
\Pi^*W^*:\mathcal P(\Pi_{\mathrm{all}})
\to\mathcal P(\Pi_{\mathrm{all}})
\]
are closure operators. Exact
capability on $W'$ in a class $\Pi$ is equivalent to
$\Pi\cap\Pi^*(W')\ne\varnothing$.
\end{proposition}

\begin{proof}
Both inclusions express the same statement
$\forall w\in W'\,\forall\pi\in\Pi'\,w\models\pi$. Antitonicity follows by
adding universal conditions. For every polarity, the composites are extensive,
monotone, and idempotent: extensivity follows from the displayed equivalence,
monotonicity from two reversals of inclusion, and idempotence from applying the
adjunction twice. The capability statement says exactly that one policy in
$\Pi$ succeeds at every world in $W'$.
\end{proof}

\begin{remark}
A closed world set is one determined by the collection of policies that succeed
throughout it. This is the standard relation-induced polarity of Formal Concept
Analysis: closed world sets and closed policy sets are concept extents and
intents \cite{ganterwille}. For fixed $W'$, $\Pi^*(W')$ is also the version
space of policies consistent with all worlds in $W'$. No identification with
observational equivalence classes is claimed, and the polarity is not used to
establish the obstruction hierarchy.
\end{remark}
...
The accompanying artifact repository, when supplied with the manuscript, contains
a 36-part reproduction gate: twenty-four core Lean files and twelve deterministic
Python experiments. The Lean artifacts check the
discrete selector, robust-selector, section, world-pair, obstruction,
discrete-ECD, finite-representation, parity, and $\mathsf L/\mathsf D/\mathsf K/\mathsf R$ priority-classification cores.
...
and compression boundary, join-tree, affine cokernel, and bijective-network
holonomy theorems, categorical packaging, the binary fibre-selection and nested
prefix Weihrauch classifications, finite Bell
...
results, stochastic-kernel Bell construction, static compilation, and pointwise
polarity are manuscript proofs without dedicated Lean artifacts. Object-level
...
Mathlib project described in \texttt{research/mathlib/README.md}. In that
environment the core command completes in approximately half a minute. A green
build verifies the formal statements in the source files, not the modelling
adequacy of the manuscript's additional constructions.
