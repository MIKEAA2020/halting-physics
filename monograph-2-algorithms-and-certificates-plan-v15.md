...
1. add a repository-ready `make reproduce` or equivalent shell entry point;
2. add a compact artifact paper appendix summarizing file formats and certificate schemas;
3. run external baseline experiments only in an environment where solvers are installed and hardware/time limits are recorded;
4. freeze a release manifest once the user provides the eventual repository or DOI link.
