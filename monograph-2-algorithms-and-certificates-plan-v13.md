...
The next genuinely worthwhile implementation items are now:

1. add solver-output certificate cross-checks when external solvers are available, without trusting solver logs as certificates;
2. add more benchmark families with controlled width growth, such as grids or banded chains, while keeping exact known decompositions;
3. add a small report generator that summarizes instance size, width, weighted width, certificate size, and verification time;
4. only after that, run fair external baseline experiments on fixed hardware.
