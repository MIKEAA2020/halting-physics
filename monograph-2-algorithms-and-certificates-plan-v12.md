...
1. add a small library of nontrivial tree/path benchmark families with known width;
2. add verifier-side checks for sparsification ladder monotonicity across `J\subseteq J'`;
3. add optional external-solver adapters, keeping all solver timeouts as `INCONCLUSIVE`;
4. add larger exact truss/frame finite algebraic examples only after the certificate gate remains stable.
