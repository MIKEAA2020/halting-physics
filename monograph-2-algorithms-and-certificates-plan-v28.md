...
## Round 10 section (plan v28) — unrestricted-synthesis classification, conditional design-side frontier, and the continuum/bicriteria adjudication (companion v14 / main v47)

### Basis

Owner directives: (1) push all previous and future creations to the
repository; (2) classify unrestricted sensor synthesis, and record an
ETH-tight or approximation-classification statement for repair, only if
highly merited and belonging to the same monograph, with the bicriteria
width-and-repair design problem the remaining unposed candidate; (3)
continuum-mechanics validation and discretisation-error certification only
if highly merited and appropriate for either current monograph. Plan v27
remaining item 6 named the first two as the natural next research targets
if the design-side programme were reopened; this round reopened them.

### Research performed

The unrestricted problem was analysed inside the companion's own one-shot
data (W, o, A, (S_w)), sensor families as free functions refining the
observation map, and Theorem 7.1's core-hitting characterisation. Three
results were obtained and verified line-by-line. (i) Free alphabets are
degenerate: a single sensor taking distinct values on the scenarios of each
fibre is nonconstant on every inclusion-minimal core (each core lies in one
fibre and has at least two scenarios once S_w is nonempty), so the
minimum-cardinality unrestricted synthesis is 0 or 1. (ii) Fixed alphabet:
a partition-versus-transversal lemma --- a family of m a-valued sensors
repairs if and only if every realised fibre's admissible family admits a
transversal of cardinality at most a^m (nonempty refined fibres partition a
fibre into intersecting classes, whose chosen actions form a transversal;
conversely a per-fibre transversal of size at most a^m, an ordering, and
rank encoding in m base-a digits define the sensors), so the minimum sensor
count is max over realised fibres of the base-a logarithm of the transversal
number, with the empty family exactly the fibre-criterion case a^0 = 1.
(iii) Hardness: a power-padding reduction from hitting set --- fresh
singleton admissible sets inflate the transversal number by K_0 = 2^t - k
with t = ceiling(log_2 k), so t binary sensors repair exactly when the
source family has a transversal of size at most k, giving NP-completeness
and W[2]-hardness in the sensor count (the parameterised reduction
compresses k to t <= log_2 k + 1); for fixed m the characterisation is
algorithmic by enumerating at most |A|^{a^m} transversal candidates.

For listed repair, two conditional statements transfer through the
budget-preserving bijection already recorded in Appendix B (repairing
subfamilies of size at most k correspond exactly to covers of size at most
k): the greedy harmonic bound H_N <= 1 + ln N on instances whose core list
of size N is part of the finite master problem [johnson74], with no
(1-eps) ln N-approximation unless P = NP through the bijection
[dinursteurer14]; and, under ETH, no |I|^{o(k)} algorithm, via the
closed-neighbourhood encoding from dominating set (parameter preserved)
composed with the article's own budget-preserving construction, against the
dominating-set f(k) n^{o(k)} barrier [impagliazzo-paturi01, cygan15],
tight up to the exponent constant against the trivial |I|^{O(k)}
enumeration. Citations verified by web search: Johnson, JCSS 9(3):256--278,
1974; Dinur--Steurer, STOC 2014 (the set-cover corollary); Cygan, Fomin,
Kowalik, Lokshtanov, Marx, Pilipczuk, Pilipczuk, Saurabh, Parameterized
Algorithms, Springer, 2015.

### Adjudication

(a) Unrestricted synthesis: included --- it closes a boundary Appendix B
itself records as unclassified, the mathematics is elementary and complete,
the free-alphabet degeneracy justifies the listed-candidate scoping of
Proposition 7.3 (the design-side hardness is not an artifact of the
candidate list, since the fixed-alphabet variant carries the same
hardness), and no new apparatus is needed beyond Theorem 7.1 and two
already-cited sources. (b) Approximation and ETH statements: included ---
both transfer through the article's own bijection with exact source
hypotheses, completing the single-problem design-side classification
Proposition 7.3 began; three new bibitems (johnson74, dinursteurer14,
cygan15), all verified. (c) Bicriteria width-and-repair design problem:
declined --- a formally posed problem with no result would be the pair's
first unproved formal object; the asymmetry remark already fixes the
qualitative boundary (evaluation width versus design budget), and the
round's additions extend the design side without new open apparatus. (d)
Continuum-mechanics validation and discretisation-error certification:
declined --- both monographs already carry explicit, accurate boundary
statements (companion section 13: no continuum-mechanics validity claim,
declared rational models rather than discretisation error; main's Mechanics
boundary remark: no continuum modelling adequacy, mesh convergence, or
unmodelled uncertainty), and a validation programme requires the additional
source-domain hypotheses both books deliberately leave unstated.

### Placement decision

Same book: the companion. All new content completes statements the
companion itself records (Appendix B's unclassified boundary, the design
side of section 7, the complexity table of section 5); the main monograph
is untouched except the mutual pin. Separate monograph and supplementary
both declined: the added content is 150 lines against existing apparatus,
not a programme.

### Implementation (companion v14, 2,143 lines; main v47, 7,431 lines)

Companion v14, twelve edits: (1) the pin to the foundational monograph
bumped to version 47; (2) the Fifth contribution extended with the
unrestricted-synthesis clause; (3) the section 5 table gains the row
"Minimum-cardinality unrestricted synthesis (fixed value alphabet) ---
NP-complete"; (4) the section 5 design-side prose extended with the
degeneracy/transversal clause; (5) Remark 6.4 gains the fixed-alphabet
W[2]-hardness clause with the not-an-artifact conclusion; (6) section 7
gains the lead-in sentence and Proposition 7.4 (Unrestricted sensor
synthesis) with the three-part statement and full proof; (7) the section 13
frontier sentence restated for the two design-side results with their
conditional corollaries; (8) Appendix B's Hard rows subsection labelled;
(9) the unrestricted boundary sentence replaced by the classification
pointer; (10) Appendix B gains the subsection "Unrestricted sensor
synthesis" with the padding construction, the duality attribution, the
width-zero clause, and the fixed-m enumeration contrast; (11) Appendix B
gains the subsection "Conditional statements for listed repair" with the
approximation and ETH transfers, both scoped single-problem; (12) the
bibliography gains johnson74, dinursteurer14, and cygan15 (27 items).
Main v47: the companion pin bumped from version 13 to version 14 (appendix
pin unchanged at version 9); no other edit.

### Verification

Tectonic: companion v14 compiles with exactly v13's six-warning underfull
class (identical badness values 3815, 3601, 2762, 10000, 10000, 10000 at
positions shifted by the insertions; both warning sites verified to be the
same pre-existing table rows), zero overfull, zero errors, zero undefined
references or citations. Main v47 compiles with exactly v46's warning
profile at identical positions and badness values, zero errors, zero
undefined references. pdftotext confirms every insertion renders: the
contribution clause, the table row, the section 5 and Remark 6.4 clauses,
the full Proposition 7.4 statement and proof, the section 13 sentence, both
Appendix B subsections with the construction and both conditional
statements, the three new bibliography entries, and both version pins
(companion pinning main v47; main pinning companion v14). Proposition
numbering confirmed ripple-free (7.3 unchanged, the new proposition is
7.4); multi-citations render as combined brackets; no "??" anywhere.
Mechanical checks: 52 labels (49 prior + subsec:hard-rows,
subsec:unrestricted-synthesis, subsec:conditional-repair), zero duplicates,
27 bibliography items all cited.

### Push directive (owner directive 1)

All tex, plan, worklog, and report creations were verified already pushed
and byte-identical to the persistence copies. The compiled PDFs (companion
v11--v14, main v44--v47), previously local-only, are archived in the
repository alongside their tex sources; future rounds push each new PDF
with its round commit.

### Remaining items

1. assign and insert the archival DOI at publication time (unchanged);
2. continuum-mechanics validation and discretisation-error certification
   remain outside both monographs' scope; the existing boundary statements
   in companion section 13 and the main's Mechanics boundary remark are
   accurate and load-bearing, and a validation programme would require the
   additional source-domain hypotheses both books leave unstated (this
   round's adjudicated decline);
3. if the Lean and Mathlib sources are ever recovered, deposit them with
   checksums and gate records and shorten the availability paragraph
   accordingly;
4. the version pins are mutual (main v47 pins companion v14/appendix v9;
   companion v14 pins main v47) and must be re-checked together whenever
   either book is bumped;
5. the zero-underfull profile remains declined (the same cosmetic
   lone-token warning class as v25/v26/v27);
6. the bicriteria width-and-repair design problem remains unposed beyond
   the asymmetry remark (adjudicated decline: a posed problem with no
   result would be the pair's first unproved formal object);
7. the claim-boundary anchor bibitems remain declined as decoration.
